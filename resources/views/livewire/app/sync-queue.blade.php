...
<div class="main-content">
    ...
    <div class="page-body" x-data="jawlaSyncQueue" x-cloak>
        <div class="card text-center text-text-secondary py-8" x-show="loading">
            {{ $isAr ? 'جارٍ تحميل حالة المزامنة…' : 'Loading sync status…' }}
        </div>
        <div x-show="!loading">
        {{-- Summary + retry-all --}}
        <div class="card mb-4 flex items-center justify-between gap-3">
            ...
            <div class="text-center">
                <div class="text-sm text-text-secondary">{{ $isAr ? 'تعارضات' : 'Conflicts' }}</div>
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-xs px-2 py-0.5 rounded-full"
                            :class="item.status === 'failed' ? 'bg-red-100 text-red-700' : (item.status === 'conflict' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700')"
                            x-text="item.status === 'failed' ? '{{ $isAr ? 'فشل' : 'Failed' }}' : (item.status === 'conflict' ? '{{ $isAr ? 'تعارض' : 'Conflict' }}' : '{{ $isAr ? 'معلّق' : 'Pending' }}')"></span>
                    </div>
