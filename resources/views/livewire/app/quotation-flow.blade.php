<div class="main-content" style="padding:16px">
    ...
    @if($errorMessage)
        <div class="card" style="background:#FEF2F2;color:#DC2626;margin-bottom:12px">{{ $errorMessage }}</div>
    @endif

    @if($successMessage)
        <div class="card" style="background:#DCFCE7;color:#15803D;margin-bottom:12px">{{ $successMessage }}</div>
    @endif
    ...
    @if($step === 'list')
        ...
        @forelse($priced as $r)
            <div class="card" style="cursor:pointer" wire:click="selectQuotation({{ $r->id }})">
                <div style="display:flex;justify-content:space-between;align-items:center">
                    <div>
                        <strong>{{ $r->product?->name_ar }}</strong>
                        <small style="display:block;color:#6b7280">{{ $r->customer?->name_ar }} · {{ $r->quantity_requested }} {{ $r->product?->unit ?? '' }}</small>
                    </div>
                    @if($r->quotation)
    ...
    @if($step === 'detail' && $request && $quotation)
        <div class="card">
            ...
            <input type="number" step="0.01" wire:model.live="negotiatedPrice" style="width:100%;padding:10px;border:1px solid #d1d5db;border-radius:8px;font-size:1.1rem">
