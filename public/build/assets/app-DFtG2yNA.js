var e = typeof __SENTRY_DEBUG__ > `u` || __SENTRY_DEBUG__,
  t = globalThis,
  n = `10.67.0`;
function r() {
  return (i(t), t);
}
function i(e) {
  let t = (e.__SENTRY__ = e.__SENTRY__ || {});
  return ((t.version = t.version || `10.67.0`), (t[n] = t[`10.67.0`] || {}));
}
function a(e, r, i = t) {
  let a = (i.__SENTRY__ = i.__SENTRY__ || {}),
    o = (a[n] = a[`10.67.0`] || {});
  return o[e] || (o[e] = r());
}
var o = [`debug`, `info`, `warn`, `error`, `log`, `assert`, `trace`],
  s = `Sentry Logger `,
  c = {};
function l(e) {
  if (!(`console` in t)) return e();
  let n = t.console,
    r = {},
    i = Object.keys(c);
  i.forEach((e) => {
    let t = c[e];
    ((r[e] = n[e]), (n[e] = t));
  });
  try {
    return e();
  } finally {
    i.forEach((e) => {
      n[e] = r[e];
    });
  }
}
function u() {
  _().enabled = !0;
}
function d() {
  _().enabled = !1;
}
function f() {
  return _().enabled;
}
function p(...e) {
  g(`log`, ...e);
}
function m(...e) {
  g(`warn`, ...e);
}
function h(...e) {
  g(`error`, ...e);
}
function g(n, ...r) {
  e &&
    f() &&
    l(() => {
      t.console[n](`${s}[${n}]:`, ...r);
    });
}
function _() {
  return e ? a(`loggerSettings`, () => ({ enabled: !1 })) : { enabled: !1 };
}
var v = { enable: u, disable: d, isEnabled: f, log: p, warn: m, error: h },
  ee = 50,
  te = /\(error: (.*)\)/,
  ne = /captureMessage|captureException/;
function re(...e) {
  let t = e.sort((e, t) => e[0] - t[0]).map((e) => e[1]);
  return (e, n = 0, r = 0) => {
    let i = [],
      a = e.split(`
`);
    for (let e = n; e < a.length; e++) {
      let n = a[e];
      n.length > 1024 && (n = n.slice(0, 1024));
      let o = te.test(n) ? n.replace(te, `$1`) : n;
      if (!o.includes(`Error: `)) {
        for (let e of t) {
          let t = e(o);
          if (t) {
            i.push(t);
            break;
          }
        }
        if (i.length >= ee + r) break;
      }
    }
    return ae(i.slice(r));
  };
}
function ie(e) {
  return Array.isArray(e) ? re(...e) : e;
}
function ae(e) {
  if (!e.length) return [];
  let t = Array.from(e);
  return (
    /sentryWrapped/.test(oe(t).function || ``) && t.pop(),
    t.reverse(),
    ne.test(oe(t).function || ``) &&
      (t.pop(), ne.test(oe(t).function || ``) && t.pop()),
    t
      .slice(0, ee)
      .map((e) => ({
        ...e,
        filename: e.filename || oe(t).filename,
        function: e.function || `?`,
      }))
  );
}
function oe(e) {
  return e[e.length - 1] || {};
}
var se = `<anonymous>`;
function y(e) {
  try {
    return !e || typeof e != `function` ? se : e.name || se;
  } catch {
    return se;
  }
}
function ce(e) {
  let t = e.exception;
  if (t) {
    let e = [];
    try {
      return (
        t.values.forEach((t) => {
          t.stacktrace.frames && e.push(...t.stacktrace.frames);
        }),
        e
      );
    } catch {
      return;
    }
  }
}
var b = {},
  le = {};
function x(e, t) {
  return (
    (b[e] = b[e] || []),
    b[e].push(t),
    () => {
      let n = b[e];
      if (n) {
        let e = n.indexOf(t);
        e !== -1 && n.splice(e, 1);
      }
    }
  );
}
function S(t, n) {
  if (!le[t]) {
    le[t] = !0;
    try {
      n();
    } catch (n) {
      e && v.error(`Error while instrumenting ${t}`, n);
    }
  }
}
function C(t, n) {
  let r = t && b[t];
  if (r)
    for (let i of r)
      try {
        i(n);
      } catch (n) {
        e &&
          v.error(
            `Error while triggering instrumentation handler.
Type: ${t}
Name: ${y(i)}
Error:`,
            n
          );
      }
}
var ue = null;
function de(e) {
  let t = `error`;
  (x(t, e), S(t, fe));
}
function fe() {
  ((ue = t.onerror),
    (t.onerror = function (e, t, n, r, i) {
      return (
        C(`error`, { column: r, error: i, line: n, msg: e, url: t }),
        ue ? ue.apply(this, arguments) : !1
      );
    }),
    (t.onerror.__SENTRY_INSTRUMENTED__ = !0));
}
var pe = null;
function me(e) {
  let t = `unhandledrejection`;
  (x(t, e), S(t, he));
}
function he() {
  ((pe = t.onunhandledrejection),
    (t.onunhandledrejection = function (e) {
      return (C(`unhandledrejection`, e), !pe || pe.apply(this, arguments));
    }),
    (t.onunhandledrejection.__SENTRY_INSTRUMENTED__ = !0));
}
var ge = Object.prototype.toString;
function _e(e) {
  switch (ge.call(e)) {
    case `[object Error]`:
    case `[object Exception]`:
    case `[object DOMException]`:
    case `[object WebAssembly.Exception]`:
      return !0;
    default:
      return Ae(e, Error);
  }
}
function ve(e, t) {
  return ge.call(e) === `[object ${t}]`;
}
function ye(e) {
  return ve(e, `ErrorEvent`);
}
function be(e) {
  return ve(e, `DOMError`);
}
function xe(e) {
  return ve(e, `DOMException`);
}
function Se(e) {
  return ve(e, `String`);
}
function Ce(e) {
  return (
    typeof e == `object` &&
    !!e &&
    `__sentry_template_string__` in e &&
    `__sentry_template_values__` in e
  );
}
function we(e) {
  return (
    e === null || Ce(e) || (typeof e != `object` && typeof e != `function`)
  );
}
function Te(e) {
  return ve(e, `Object`);
}
function Ee(e) {
  return typeof e == `object` && !!e;
}
function De(e) {
  return typeof Event < `u` && Ae(e, Event);
}
function Oe(e) {
  return ve(e, `RegExp`);
}
function ke(e) {
  return !!(e?.then && typeof e.then == `function`);
}
function Ae(e, t) {
  try {
    return e instanceof t;
  } catch {
    return !1;
  }
}
function je(e) {
  return typeof Request < `u` && Ae(e, Request);
}
function w(t, n, r) {
  if (!(n in t)) return;
  let i = t[n];
  if (typeof i != `function`) return;
  let a = r(i);
  typeof a == `function` && Me(a, i);
  try {
    t[n] = a;
  } catch {
    e && v.log(`Failed to replace method "${n}" in object`, t);
  }
}
function T(t, n, r) {
  try {
    Object.defineProperty(t, n, { value: r, writable: !0, configurable: !0 });
  } catch {
    e &&
      v.log(
        `Failed to add non-enumerable property "${String(n)}" to object`,
        t
      );
  }
}
function Me(e, t) {
  try {
    ((e.prototype = t.prototype = t.prototype || {}),
      T(e, `__sentry_original__`, t));
  } catch {}
}
function Ne(e) {
  return e.__sentry_original__;
}
function Pe(e) {
  if (_e(e))
    return { message: e.message, name: e.name, stack: e.stack, ...Fe(e) };
  if (De(e)) {
    let { type: t, target: n, currentTarget: r, detail: i } = e;
    return {
      type: t,
      target: n,
      currentTarget: r,
      ...(i ? { detail: i } : {}),
      ...Fe(e),
    };
  }
  return e;
}
function Fe(e) {
  return Ee(e) ? Object.fromEntries(Object.entries(e)) : {};
}
function Ie(e) {
  let t = Object.keys(Pe(e));
  return (t.sort(), t[0] ? t.join(`, `) : `[object has no keys]`);
}
var Le;
function Re(e) {
  if (Le !== void 0) return Le ? Le(e) : e();
  let n = Symbol.for(`__SENTRY_SAFE_RANDOM_ID_WRAPPER__`),
    r = t;
  return n in r && typeof r[n] == `function`
    ? ((Le = r[n]), Le(e))
    : ((Le = null), e());
}
function ze() {
  return Re(() => Math.random());
}
function Be() {
  return Re(() => Date.now());
}
var Ve = Symbol.for(`sentry.skipNormalization`),
  He = Symbol.for(`sentry.overrideNormalizationDepth`);
function Ue(e) {
  return !!e[Ve];
}
function We(e) {
  let t = e[He];
  return typeof t == `number` ? t : void 0;
}
var Ge;
function Ke(e) {
  Ge = e;
}
function E(e, t = 100, n = 1 / 0) {
  try {
    return Je(``, e, t, n);
  } catch (e) {
    return { ERROR: `**non-serializable** (${e})` };
  }
}
function qe(e, t = 3, n = 100 * 1024) {
  let r = E(e, t);
  return Qe(r) > n ? qe(e, t - 1, n) : r;
}
function Je(e, t, n = 1 / 0, r = 1 / 0, i = $e()) {
  let [a, o] = i;
  if (
    t == null ||
    [`boolean`, `string`].includes(typeof t) ||
    (typeof t == `number` && Number.isFinite(t))
  )
    return t;
  let s = Ye(e, t);
  if (!s.startsWith(`[object `)) return s;
  if (Ue(t)) return t;
  let c = We(t),
    l = c === void 0 ? n : c;
  if (l === 0) return s.replace(`object `, ``);
  if (a(t)) return `[Circular ~]`;
  let u = t;
  if (u && typeof u.toJSON == `function`)
    try {
      return Je(``, u.toJSON(), l - 1, r, i);
    } catch {}
  let d = Array.isArray(t) ? [] : {},
    f = 0,
    p = Pe(t);
  for (let e in p) {
    if (!Object.prototype.hasOwnProperty.call(p, e)) continue;
    if (f >= r) {
      d[e] = `[MaxProperties ~]`;
      break;
    }
    let t = p[e];
    ((d[e] = Je(e, t, l - 1, r, i)), f++);
  }
  return (o(t), d);
}
function Ye(e, t) {
  try {
    if (Ge) {
      let e = Ge(t);
      if (e) return e;
    }
    return typeof global < `u` && t === global
      ? `[Global]`
      : typeof t == `number` && !Number.isFinite(t)
        ? `[${t}]`
        : typeof t == `function`
          ? `[Function: ${y(t)}]`
          : typeof t == `symbol`
            ? `[${String(t)}]`
            : typeof t == `bigint`
              ? `[BigInt: ${String(t)}]`
              : `[object ${Xe(t)}]`;
  } catch (e) {
    return `**non-serializable** (${e})`;
  }
}
function Xe(e) {
  let t = Object.getPrototypeOf(e);
  return t?.constructor ? t.constructor.name : `null prototype`;
}
function Ze(e) {
  return ~-encodeURI(e).split(/%..|./).length;
}
function Qe(e) {
  return Ze(JSON.stringify(e));
}
function $e() {
  let e = new WeakSet();
  function t(t) {
    return e.has(t) ? !0 : (e.add(t), !1);
  }
  function n(t) {
    e.delete(t);
  }
  return [t, n];
}
function et(e, t = 0) {
  return typeof e != `string` || t === 0 || e.length <= t
    ? e
    : `${e.slice(0, t)}...`;
}
function tt(e, t) {
  if (!Array.isArray(e)) return ``;
  let n = [];
  for (let t = 0; t < e.length; t++) {
    let r = e[t];
    we(r)
      ? n.push(String(r))
      : r instanceof Error
        ? n.push(r.message ? `${r.name}: ${r.message}` : r.name)
        : n.push(Ye(void 0, r));
  }
  return n.join(t);
}
function nt(e, t, n = !1) {
  return Se(e)
    ? Oe(t)
      ? t.test(e)
      : Se(t)
        ? n
          ? e === t
          : e.includes(t)
        : typeof t == `function` && t(e)
    : !1;
}
function D(e, t = [], n = !1) {
  for (let r of t) if (nt(e, r, n)) return !0;
  return !1;
}
function rt() {
  let e = t;
  return e.crypto || e.msCrypto;
}
var it;
function at() {
  return ze() * 16;
}
function O(e = rt()) {
  try {
    if (e?.randomUUID) return Re(() => e.randomUUID()).replace(/-/g, ``);
  } catch {}
  return (
    (it ||= `10000000100040008000100000000000`),
    it.replace(/[018]/g, (e) => (e ^ ((at() & 15) >> (e / 4))).toString(16))
  );
}
function ot(e) {
  return e.exception?.values?.[0];
}
function st(e) {
  let { message: t, event_id: n } = e;
  if (t) return t;
  let r = ot(e);
  return r
    ? r.type && r.value
      ? `${r.type}: ${r.value}`
      : r.type || r.value || n || `<unknown>`
    : n || `<unknown>`;
}
function ct(e, t, n) {
  let r = (e.exception = e.exception || {}),
    i = (r.values = r.values || []),
    a = (i[0] = i[0] || {});
  ((a.value ||= t || ``), (a.type ||= n || `Error`));
}
function lt(e, t) {
  let n = ot(e);
  if (!n) return;
  let r = { type: `generic`, handled: !0 },
    i = n.mechanism;
  if (((n.mechanism = { ...r, ...i, ...t }), t && `data` in t)) {
    let e = { ...i?.data, ...t.data };
    n.mechanism.data = e;
  }
}
function ut(e) {
  if (dt(e)) return !0;
  try {
    T(e, `__sentry_captured__`, !0);
  } catch {}
  return !1;
}
function dt(e) {
  try {
    return e.__sentry_captured__;
  } catch {}
}
var ft = 1e3;
function pt() {
  return Be() / ft;
}
function mt() {
  let { performance: e } = t;
  if (!e?.now || !e.timeOrigin) return pt;
  let n = e.timeOrigin;
  return () => (n + Re(() => e.now())) / ft;
}
var ht;
function k() {
  return (ht ??= mt())();
}
var gt = null;
function _t() {
  let { performance: e } = t;
  if (!e?.now) return;
  let n = 3e5,
    r = Re(() => e.now()),
    i = Be(),
    a = e.timeOrigin;
  if (typeof a == `number` && Math.abs(a + r - i) < n) return a;
  let o = e.timing?.navigationStart;
  return typeof o == `number` && Math.abs(o + r - i) < n ? o : i - r;
}
function A() {
  return (gt === null && (gt = _t()), gt);
}
function vt(e) {
  let t = k(),
    n = {
      sid: O(),
      init: !0,
      timestamp: t,
      started: t,
      duration: 0,
      status: `ok`,
      errors: 0,
      ignoreDuration: !1,
      toJSON: () => xt(n),
    };
  return (e && yt(n, e), n);
}
function yt(e, t = {}) {
  if (
    (t.user &&
      (!e.ipAddress && t.user.ip_address && (e.ipAddress = t.user.ip_address),
      !e.did &&
        !t.did &&
        (e.did = t.user.id || t.user.email || t.user.username)),
    (e.timestamp = t.timestamp || k()),
    t.abnormal_mechanism && (e.abnormal_mechanism = t.abnormal_mechanism),
    t.ignoreDuration && (e.ignoreDuration = t.ignoreDuration),
    t.sid && (e.sid = t.sid.length === 32 ? t.sid : O()),
    t.init !== void 0 && (e.init = t.init),
    !e.did && t.did && (e.did = `${t.did}`),
    typeof t.started == `number` && (e.started = t.started),
    e.ignoreDuration)
  )
    e.duration = void 0;
  else if (typeof t.duration == `number`) e.duration = t.duration;
  else {
    let t = e.timestamp - e.started;
    e.duration = t >= 0 ? t : 0;
  }
  (t.release && (e.release = t.release),
    t.environment && (e.environment = t.environment),
    !e.ipAddress && t.ipAddress && (e.ipAddress = t.ipAddress),
    !e.userAgent && t.userAgent && (e.userAgent = t.userAgent),
    typeof t.errors == `number` && (e.errors = t.errors),
    t.status && (e.status = t.status));
}
function bt(e, t) {
  let n = {};
  (t ? (n = { status: t }) : e.status === `ok` && (n = { status: `exited` }),
    yt(e, n));
}
function xt(e) {
  return {
    sid: `${e.sid}`,
    init: e.init,
    started: new Date(e.started * 1e3).toISOString(),
    timestamp: new Date(e.timestamp * 1e3).toISOString(),
    status: e.status,
    errors: e.errors,
    did:
      typeof e.did == `number` || typeof e.did == `string`
        ? `${e.did}`
        : void 0,
    duration: e.duration,
    abnormal_mechanism: e.abnormal_mechanism,
    attrs: {
      release: e.release,
      environment: e.environment,
      ip_address: e.ipAddress,
      user_agent: e.userAgent,
    },
  };
}
function St(e, t, n = 2) {
  if (!t || typeof t != `object` || n <= 0) return t;
  if (e && Object.keys(t).length === 0) return e;
  let r = { ...e };
  for (let e in t)
    Object.prototype.hasOwnProperty.call(t, e) &&
      (r[e] = St(r[e], t[e], n - 1));
  return r;
}
function j() {
  return O();
}
function M() {
  return O().substring(16);
}
function Ct(e) {
  try {
    let n = t.WeakRef;
    if (typeof n == `function`) return new n(e);
  } catch {}
  return e;
}
function wt(e) {
  if (e) {
    if (typeof e == `object` && `deref` in e && typeof e.deref == `function`)
      try {
        return e.deref();
      } catch {
        return;
      }
    return e;
  }
}
var Tt = `_sentrySpan`;
function Et(e, t) {
  t ? T(e, Tt, Ct(t)) : delete e[Tt];
}
function Dt(e) {
  return wt(e[Tt]);
}
var Ot = 100,
  kt = class t {
    constructor() {
      ((this._notifyingListeners = !1),
        (this._scopeListeners = []),
        (this._eventProcessors = []),
        (this._breadcrumbs = []),
        (this._attachments = []),
        (this._user = {}),
        (this._tags = {}),
        (this._attributes = {}),
        (this._extra = {}),
        (this._contexts = {}),
        (this._sdkProcessingMetadata = {}),
        (this._propagationContext = { traceId: j(), sampleRand: ze() }));
    }
    clone() {
      let e = new t();
      return (
        (e._breadcrumbs = [...this._breadcrumbs]),
        (e._tags = { ...this._tags }),
        (e._attributes = { ...this._attributes }),
        (e._extra = { ...this._extra }),
        (e._contexts = { ...this._contexts }),
        this._contexts.flags &&
          (e._contexts.flags = { values: [...this._contexts.flags.values] }),
        (e._user = this._user),
        (e._level = this._level),
        (e._session = this._session),
        (e._transactionName = this._transactionName),
        (e._fingerprint = this._fingerprint),
        (e._eventProcessors = [...this._eventProcessors]),
        (e._attachments = [...this._attachments]),
        (e._sdkProcessingMetadata = { ...this._sdkProcessingMetadata }),
        (e._propagationContext = { ...this._propagationContext }),
        (e._client = this._client),
        (e._lastEventId = this._lastEventId),
        (e._conversationId = this._conversationId),
        Et(e, Dt(this)),
        e
      );
    }
    setClient(e) {
      this._client = e;
    }
    setLastEventId(e) {
      this._lastEventId = e;
    }
    getClient() {
      return this._client;
    }
    lastEventId() {
      return this._lastEventId;
    }
    addScopeListener(e) {
      this._scopeListeners.push(e);
    }
    addEventProcessor(e) {
      return (this._eventProcessors.push(e), this);
    }
    setUser(e) {
      return (
        (this._user = e || {
          email: void 0,
          id: void 0,
          ip_address: void 0,
          username: void 0,
        }),
        this._session && yt(this._session, { user: e }),
        this._notifyScopeListeners(),
        this
      );
    }
    getUser() {
      return this._user;
    }
    setConversationId(e) {
      return (
        (this._conversationId = e || void 0),
        this._notifyScopeListeners(),
        this
      );
    }
    setTags(e) {
      return (
        (this._tags = { ...this._tags, ...e }),
        this._notifyScopeListeners(),
        this
      );
    }
    setTag(e, t) {
      return this.setTags({ [e]: t });
    }
    setAttributes(e) {
      return (
        (this._attributes = { ...this._attributes, ...e }),
        this._notifyScopeListeners(),
        this
      );
    }
    setAttribute(e, t) {
      return this.setAttributes({ [e]: t });
    }
    removeAttribute(e) {
      return (
        e in this._attributes &&
          (delete this._attributes[e], this._notifyScopeListeners()),
        this
      );
    }
    setExtras(e) {
      return (
        (this._extra = { ...this._extra, ...e }),
        this._notifyScopeListeners(),
        this
      );
    }
    setExtra(e, t) {
      return (
        (this._extra = { ...this._extra, [e]: t }),
        this._notifyScopeListeners(),
        this
      );
    }
    setFingerprint(e) {
      return ((this._fingerprint = e), this._notifyScopeListeners(), this);
    }
    setLevel(e) {
      return ((this._level = e), this._notifyScopeListeners(), this);
    }
    setTransactionName(e) {
      return ((this._transactionName = e), this._notifyScopeListeners(), this);
    }
    setContext(e, t) {
      return (
        t === null ? delete this._contexts[e] : (this._contexts[e] = t),
        this._notifyScopeListeners(),
        this
      );
    }
    setSession(e) {
      return (
        e ? (this._session = e) : delete this._session,
        this._notifyScopeListeners(),
        this
      );
    }
    getSession() {
      return this._session;
    }
    update(e) {
      if (!e) return this;
      let n = typeof e == `function` ? e(this) : e,
        {
          tags: r,
          attributes: i,
          extra: a,
          user: o,
          contexts: s,
          level: c,
          fingerprint: l = [],
          propagationContext: u,
          conversationId: d,
        } = (n instanceof t ? n.getScopeData() : Te(n) ? e : void 0) || {};
      return (
        (this._tags = { ...this._tags, ...r }),
        (this._attributes = { ...this._attributes, ...i }),
        (this._extra = { ...this._extra, ...a }),
        (this._contexts = { ...this._contexts, ...s }),
        o && Object.keys(o).length && (this._user = o),
        c && (this._level = c),
        l.length && (this._fingerprint = l),
        u && (this._propagationContext = u),
        d && (this._conversationId = d),
        this
      );
    }
    clear() {
      return (
        (this._breadcrumbs = []),
        (this._tags = {}),
        (this._attributes = {}),
        (this._extra = {}),
        (this._user = {}),
        (this._contexts = {}),
        (this._level = void 0),
        (this._transactionName = void 0),
        (this._fingerprint = void 0),
        (this._session = void 0),
        (this._conversationId = void 0),
        Et(this, void 0),
        (this._attachments = []),
        this.setPropagationContext({ traceId: j(), sampleRand: ze() }),
        this._notifyScopeListeners(),
        this
      );
    }
    addBreadcrumb(e, t) {
      let n = typeof t == `number` ? t : Ot;
      if (n <= 0) return this;
      let r = {
        timestamp: pt(),
        ...e,
        message: e.message ? et(e.message, 2048) : e.message,
      };
      return (
        this._breadcrumbs.push(r),
        this._breadcrumbs.length > n &&
          ((this._breadcrumbs = this._breadcrumbs.slice(-n)),
          this._client?.recordDroppedEvent(`buffer_overflow`, `log_item`)),
        this._notifyScopeListeners(),
        this
      );
    }
    getLastBreadcrumb() {
      return this._breadcrumbs[this._breadcrumbs.length - 1];
    }
    clearBreadcrumbs() {
      return ((this._breadcrumbs = []), this._notifyScopeListeners(), this);
    }
    addAttachment(e) {
      return (this._attachments.push(e), this);
    }
    clearAttachments() {
      return ((this._attachments = []), this);
    }
    getScopeData() {
      return {
        breadcrumbs: this._breadcrumbs,
        attachments: this._attachments,
        contexts: this._contexts,
        tags: this._tags,
        attributes: this._attributes,
        extra: this._extra,
        user: this._user,
        level: this._level,
        fingerprint: this._fingerprint || [],
        eventProcessors: this._eventProcessors,
        propagationContext: this._propagationContext,
        sdkProcessingMetadata: this._sdkProcessingMetadata,
        transactionName: this._transactionName,
        span: Dt(this),
        conversationId: this._conversationId,
      };
    }
    setSDKProcessingMetadata(e) {
      return (
        (this._sdkProcessingMetadata = St(this._sdkProcessingMetadata, e, 2)),
        this
      );
    }
    setPropagationContext(e) {
      return ((this._propagationContext = e), this);
    }
    getPropagationContext() {
      return this._propagationContext;
    }
    captureException(t, n) {
      let r = n?.event_id || O();
      if (!this._client)
        return (
          e &&
            v.warn(
              `No client configured on scope - will not capture exception!`
            ),
          r
        );
      let i = Error(`Sentry syntheticException`);
      return (
        this._client.captureException(
          t,
          { originalException: t, syntheticException: i, ...n, event_id: r },
          this
        ),
        r
      );
    }
    captureMessage(t, n, r) {
      let i = r?.event_id || O();
      if (!this._client)
        return (
          e &&
            v.warn(`No client configured on scope - will not capture message!`),
          i
        );
      let a = r?.syntheticException ?? Error(t);
      return (
        this._client.captureMessage(
          t,
          n,
          { originalException: t, syntheticException: a, ...r, event_id: i },
          this
        ),
        i
      );
    }
    captureEvent(t, n) {
      let r = t.event_id || n?.event_id || O();
      return this._client
        ? (this._client.captureEvent(t, { ...n, event_id: r }, this), r)
        : (e &&
            v.warn(`No client configured on scope - will not capture event!`),
          r);
    }
    _notifyScopeListeners() {
      this._notifyingListeners ||=
        ((this._notifyingListeners = !0),
        this._scopeListeners.forEach((e) => {
          e(this);
        }),
        !1);
    }
  };
function At() {
  return a(`defaultCurrentScope`, () => new kt());
}
function jt() {
  return a(`defaultIsolationScope`, () => new kt());
}
var Mt = (e) => e instanceof Promise && !e[Nt],
  Nt = Symbol(`chained PromiseLike`),
  Pt = (e, t, n) => {
    let r = e.then(
      (e) => (t(e), e),
      (e) => {
        throw (n(e), e);
      }
    );
    return Mt(r) && Mt(e) ? r : Ft(e, r);
  },
  Ft = (e, t) => {
    if (!t) return e;
    let n = !1;
    for (let r in e) {
      if (r in t) continue;
      n = !0;
      let i = e[r];
      typeof i == `function`
        ? Object.defineProperty(t, r, {
            value: (...t) => i.apply(e, t),
            enumerable: !0,
            configurable: !0,
            writable: !0,
          })
        : (t[r] = i);
    }
    return (n && Object.assign(t, { [Nt]: !0 }), t);
  },
  It = class {
    constructor(e, t) {
      let n;
      n = e || new kt();
      let r;
      ((r = t || new kt()),
        (this._stack = [{ scope: n }]),
        (this._isolationScope = r));
    }
    withScope(e) {
      let t = this._pushScope(),
        n;
      try {
        n = e(t);
      } catch (e) {
        throw (this._popScope(), e);
      }
      return ke(n)
        ? Pt(
            n,
            () => this._popScope(),
            () => this._popScope()
          )
        : (this._popScope(), n);
    }
    getClient() {
      return this.getStackTop().client;
    }
    getScope() {
      return this.getStackTop().scope;
    }
    getIsolationScope() {
      return this._isolationScope;
    }
    getStackTop() {
      return this._stack[this._stack.length - 1];
    }
    _pushScope() {
      let e = this.getScope().clone();
      return (this._stack.push({ client: this.getClient(), scope: e }), e);
    }
    _popScope() {
      return this._stack.length <= 1 ? !1 : !!this._stack.pop();
    }
  };
function Lt() {
  let e = i(r());
  return (e.stack = e.stack || new It(At(), jt()));
}
function Rt(e) {
  return Lt().withScope(e);
}
function zt(e, t) {
  let n = Lt();
  return n.withScope(() => ((n.getStackTop().scope = e), t(e)));
}
function Bt(e) {
  return Lt().withScope(() => e(Lt().getIsolationScope()));
}
function Vt() {
  return {
    withIsolationScope: Bt,
    withScope: Rt,
    withSetScope: zt,
    withSetIsolationScope: (e, t) => Bt(t),
    getCurrentScope: () => Lt().getScope(),
    getIsolationScope: () => Lt().getIsolationScope(),
  };
}
function Ht(e) {
  let t = i(e);
  return t.acs ? t.acs : Vt();
}
function Ut(e) {
  return (
    typeof e == `object` &&
    !!e &&
    !Array.isArray(e) &&
    Object.keys(e).includes(`value`)
  );
}
function Wt(e, t) {
  let { value: n, unit: r } = Ut(e) ? e : { value: e, unit: void 0 },
    i = Kt(n),
    a = r && typeof r == `string` ? { unit: r } : {};
  if (i) return { ...i, ...a };
  if (!t || (t === `skip-undefined` && n === void 0)) return;
  let o = ``;
  try {
    o = JSON.stringify(n) ?? ``;
  } catch {}
  return { value: o, type: `string`, ...a };
}
function Gt(e, t = !1) {
  let n = {};
  for (let [r, i] of Object.entries(e ?? {})) {
    let e = Wt(i, t);
    e && (n[r] = e);
  }
  return n;
}
function Kt(e) {
  if (Array.isArray(e)) return { value: e, type: `array` };
  let t =
    typeof e == `string`
      ? `string`
      : typeof e == `boolean`
        ? `boolean`
        : typeof e == `number` && !Number.isNaN(e)
          ? Number.isInteger(e)
            ? `integer`
            : `double`
          : null;
  if (t) return { value: e, type: t };
}
var qt;
function Jt() {
  return qt?.();
}
function Yt() {
  return qt !== void 0;
}
function N() {
  return Ht(r()).getCurrentScope();
}
function P() {
  return Ht(r()).getIsolationScope();
}
function Xt() {
  return a(`globalScope`, () => new kt());
}
function Zt(...e) {
  let t = Ht(r());
  if (e.length === 2) {
    let [n, r] = e;
    return n ? t.withSetScope(n, r) : t.withScope(r);
  }
  return t.withScope(e[0]);
}
function F() {
  return N().getClient();
}
function Qt(e) {
  let t = Jt();
  if (t) return { trace_id: t.traceId, span_id: t.spanId };
  let {
      traceId: n,
      parentSpanId: r,
      propagationSpanId: i,
    } = e.getPropagationContext(),
    a = { trace_id: n, span_id: i || M() };
  return (r && (a.parent_span_id = r), a);
}
var $t = `sentry.source`,
  en = `sentry.sample_rate`,
  tn = `sentry.previous_trace_sample_rate`,
  I = `sentry.op`,
  L = `sentry.origin`,
  nn = `sentry.idle_span_finish_reason`,
  rn = `sentry.measurement_unit`,
  an = `sentry.measurement_value`,
  on = `sentry.custom_span_name`,
  sn = `sentry.profile_id`,
  cn = `sentry.exclusive_time`,
  ln = `url.full`,
  un = `sentry.link.type`,
  dn = `gen_ai.conversation.id`;
function fn(e) {
  if (e < 400 && e >= 100) return { code: 1 };
  if (e >= 400 && e < 500)
    switch (e) {
      case 401:
        return { code: 2, message: `unauthenticated` };
      case 403:
        return { code: 2, message: `permission_denied` };
      case 404:
        return { code: 2, message: `not_found` };
      case 409:
        return { code: 2, message: `already_exists` };
      case 413:
        return { code: 2, message: `failed_precondition` };
      case 429:
        return { code: 2, message: `resource_exhausted` };
      case 499:
        return { code: 2, message: `cancelled` };
      default:
        return { code: 2, message: `invalid_argument` };
    }
  if (e >= 500 && e < 600)
    switch (e) {
      case 501:
        return { code: 2, message: `unimplemented` };
      case 503:
        return { code: 2, message: `unavailable` };
      case 504:
        return { code: 2, message: `deadline_exceeded` };
      default:
        return { code: 2, message: `internal_error` };
    }
  return { code: 2, message: `internal_error` };
}
function pn(e, t) {
  e.setAttribute(`http.response.status_code`, t);
  let n = fn(t);
  n.message !== `unknown_error` && e.setStatus(n);
}
var mn = `_sentryScope`,
  hn = `_sentryIsolationScope`,
  gn = Symbol.for(`sentry.otelSourceInference`),
  _n = Symbol.for(`sentry.otelSourceExplicitlySet`),
  vn = Symbol.for(`sentry.tracerProviderSpan`);
function yn(e, t, n) {
  e && (T(e, hn, Ct(n)), T(e, mn, t));
}
function bn(e) {
  let t = e;
  return { scope: t[mn], isolationScope: wt(t[hn]) };
}
function xn(e) {
  return e[gn] === !0;
}
function Sn(e) {
  T(e, _n, !0);
}
function Cn(e) {
  return e[vn] === !0;
}
var wn = `sentry-`;
function Tn(e) {
  let t = Dn(e);
  if (!t) return;
  let n = Object.entries(t).reduce((e, [t, n]) => {
    if (t.startsWith(`sentry-`)) {
      let r = t.slice(7);
      e[r] = n;
    }
    return e;
  }, {});
  if (Object.keys(n).length > 0) return n;
}
function En(e) {
  if (e)
    return kn(
      Object.entries(e).reduce(
        (e, [t, n]) => (n && (e[`${wn}${t}`] = n), e),
        {}
      )
    );
}
function Dn(e) {
  if (!(!e || (!Se(e) && !Array.isArray(e))))
    return Array.isArray(e)
      ? e.reduce((e, t) => {
          let n = On(t);
          return (
            Object.entries(n).forEach(([t, n]) => {
              e[t] = n;
            }),
            e
          );
        }, {})
      : On(e);
}
function On(e) {
  return e
    .split(`,`)
    .map((e) => {
      let t = e.indexOf(`=`);
      return t === -1
        ? []
        : [e.slice(0, t), e.slice(t + 1)].map((e) => {
            try {
              return decodeURIComponent(e.trim());
            } catch {
              return;
            }
          });
    })
    .reduce((e, [t, n]) => (t && n && (e[t] = n), e), {});
}
function kn(t) {
  if (Object.keys(t).length !== 0)
    return Object.entries(t).reduce((t, [n, r], i) => {
      let a = `${encodeURIComponent(n)}=${encodeURIComponent(r)}`,
        o = i === 0 ? a : `${t},${a}`;
      return o.length > 8192
        ? (e &&
            v.warn(
              `Not adding key: ${n} with val: ${r} to baggage header due to exceeding baggage size limits.`
            ),
          t)
        : o;
    }, ``);
}
var An = /^o(\d+)\./,
  jn =
    /^(?:(\w+):)\/\/(?:(\w+)(?::(\w+)?)?@)((?:\[[:.%\w]+\]|[\w.-]+))(?::(\d+))?\/(.+)/;
function Mn(e) {
  return e === `http` || e === `https`;
}
function Nn(e, t = !1) {
  let {
    host: n,
    path: r,
    pass: i,
    port: a,
    projectId: o,
    protocol: s,
    publicKey: c,
  } = e;
  return `${s}://${c}${t && i ? `:${i}` : ``}@${n}${a ? `:${a}` : ``}/${r && `${r}/`}${o}`;
}
function Pn(e) {
  let t = jn.exec(e);
  if (!t) {
    l(() => {
      console.error(`Invalid Sentry Dsn: ${e}`);
    });
    return;
  }
  let [n, r, i = ``, a = ``, o = ``, s = ``] = t.slice(1),
    c = ``,
    u = s,
    d = u.split(`/`);
  if ((d.length > 1 && ((c = d.slice(0, -1).join(`/`)), (u = d.pop())), u)) {
    let e = u.match(/^\d+/);
    e && (u = e[0]);
  }
  return Fn({
    host: a,
    pass: i,
    path: c,
    projectId: u,
    port: o,
    protocol: n,
    publicKey: r,
  });
}
function Fn(e) {
  return {
    protocol: e.protocol,
    publicKey: e.publicKey || ``,
    pass: e.pass || ``,
    host: e.host,
    port: e.port || ``,
    path: e.path || ``,
    projectId: e.projectId,
  };
}
function In(t) {
  if (!e) return !0;
  let { port: n, projectId: r, protocol: i } = t;
  return [`protocol`, `publicKey`, `host`, `projectId`].find((e) =>
    t[e] ? !1 : (v.error(`Invalid Sentry Dsn: ${e} missing`), !0)
  )
    ? !1
    : r.match(/^\d+$/)
      ? Mn(i)
        ? n && isNaN(parseInt(n, 10))
          ? (v.error(`Invalid Sentry Dsn: Invalid port ${n}`), !1)
          : !0
        : (v.error(`Invalid Sentry Dsn: Invalid protocol ${i}`), !1)
      : (v.error(`Invalid Sentry Dsn: Invalid projectId ${r}`), !1);
}
function Ln(e) {
  return e.match(An)?.[1];
}
function Rn(e) {
  let t = e.getOptions(),
    { host: n } = e.getDsn() || {},
    r;
  return (t.orgId ? (r = String(t.orgId)) : n && (r = Ln(n)), r);
}
function zn(e) {
  let t = typeof e == `string` ? Pn(e) : Fn(e);
  if (!(!t || !In(t))) return t;
}
function Bn(e) {
  if (typeof e == `boolean`) return Number(e);
  let t = typeof e == `string` ? parseFloat(e) : e;
  if (!(typeof t != `number` || isNaN(t) || t < 0 || t > 1)) return t;
}
var Vn = RegExp(`^[ \\t]*([0-9a-f]{32})?-?([0-9a-f]{16})?-?([01])?[ \\t]*$`);
function Hn(e) {
  if (!e) return;
  let t = e.match(Vn);
  if (!t) return;
  let n;
  return (
    t[3] === `1` ? (n = !0) : t[3] === `0` && (n = !1),
    { traceId: t[1], parentSampled: n, parentSpanId: t[2] }
  );
}
function Un(e, t) {
  let n = Hn(e),
    r = Tn(t);
  if (!n?.traceId) return { traceId: j(), sampleRand: ze() };
  let i = Kn(n, r);
  r && (r.sample_rand = i.toString());
  let { traceId: a, parentSpanId: o, parentSampled: s } = n;
  return {
    traceId: a,
    parentSpanId: o,
    sampled: s,
    dsc: r || {},
    sampleRand: i,
  };
}
function Wn(e = j(), t = M(), n) {
  let r = ``;
  return (n !== void 0 && (r = n ? `-1` : `-0`), `${e}-${t}${r}`);
}
function Gn(e = j(), t = M(), n) {
  return `00-${e}-${t}-${n ? `01` : `00`}`;
}
function Kn(e, t) {
  let n = Bn(t?.sample_rand);
  if (n !== void 0) return n;
  let r = Bn(t?.sample_rate);
  return r && e?.parentSampled !== void 0
    ? e.parentSampled
      ? ze() * r
      : r + ze() * (1 - r)
    : ze();
}
var qn = !1;
function Jn(e) {
  let { spanId: t, traceId: n } = e.spanContext(),
    {
      data: r,
      op: i,
      parent_span_id: a,
      status: o,
      origin: s,
      links: c,
    } = z(e);
  return {
    parent_span_id: a,
    span_id: t,
    trace_id: n,
    data: r,
    op: i,
    status: o,
    origin: s,
    links: c,
  };
}
function Yn(e) {
  let { spanId: t, traceId: n, isRemote: r } = e.spanContext(),
    i = r ? t : z(e).parent_span_id,
    a = bn(e).scope;
  return {
    parent_span_id: i,
    span_id: r ? a?.getPropagationContext().propagationSpanId || M() : t,
    trace_id: n,
  };
}
function Xn(e) {
  let { traceId: t, spanId: n } = e.spanContext();
  return Wn(t, n, or(e));
}
function Zn(e) {
  let { traceId: t, spanId: n } = e.spanContext();
  return Gn(t, n, or(e));
}
function Qn(e) {
  if (e && e.length > 0)
    return e.map(
      ({
        context: { spanId: e, traceId: t, traceFlags: n, ...r },
        attributes: i,
      }) => ({ span_id: e, trace_id: t, sampled: n === 1, attributes: i, ...r })
    );
}
function $n(e) {
  if (e?.length)
    return e.map(
      ({
        context: { spanId: e, traceId: t, traceFlags: n },
        attributes: r,
      }) => ({ span_id: e, trace_id: t, sampled: n === 1, attributes: r })
    );
}
function R(e) {
  return typeof e == `number`
    ? er(e)
    : Array.isArray(e)
      ? e[0] + e[1] / 1e9
      : e instanceof Date
        ? er(e.getTime())
        : k();
}
function er(e) {
  return e > 9999999999 ? e / 1e3 : e;
}
function z(e) {
  if (ar(e)) return e.getSpanJSON();
  let { spanId: t, traceId: n } = e.spanContext();
  if (ir(e)) {
    let {
      attributes: r,
      startTime: i,
      name: a,
      endTime: o,
      status: s,
      links: c,
    } = e;
    return {
      span_id: t,
      trace_id: n,
      data: r,
      description: a,
      parent_span_id: nr(e),
      start_timestamp: R(i),
      timestamp: R(o) || void 0,
      status: sr(s),
      op: r[I],
      origin: r[L],
      links: Qn(c),
    };
  }
  return { span_id: t, trace_id: n, start_timestamp: 0, data: {} };
}
function tr(e) {
  if (ar(e)) return e.getStreamedSpanJSON();
  let { spanId: t, traceId: n } = e.spanContext();
  if (ir(e)) {
    let {
      attributes: r,
      startTime: i,
      name: a,
      endTime: o,
      status: s,
      links: c,
    } = e;
    return {
      name: a,
      span_id: t,
      trace_id: n,
      parent_span_id: nr(e),
      start_timestamp: R(i),
      end_timestamp: R(o),
      is_segment: e === hr(e),
      status: cr(s),
      attributes: lr(r, s),
      links: $n(c),
    };
  }
  return {
    span_id: t,
    trace_id: n,
    start_timestamp: 0,
    name: ``,
    end_timestamp: 0,
    status: `ok`,
    is_segment: e === hr(e),
  };
}
function nr(e) {
  return `parentSpanId` in e
    ? e.parentSpanId
    : `parentSpanContext` in e
      ? e.parentSpanContext?.spanId
      : void 0;
}
function rr(e) {
  return {
    ...e,
    attributes: Gt(e.attributes),
    links: e.links?.map((e) => ({ ...e, attributes: Gt(e.attributes) })),
  };
}
function ir(e) {
  let t = e;
  return (
    !!t.attributes && !!t.startTime && !!t.name && !!t.endTime && !!t.status
  );
}
function ar(e) {
  return typeof e.getSpanJSON == `function`;
}
function or(e) {
  let { traceFlags: t } = e.spanContext();
  return t === 1;
}
function sr(e) {
  if (!(!e || e.code === 0))
    return e.code === 1 ? `ok` : e.message || `internal_error`;
}
function cr(e) {
  return !e || e.code === 1 || e.code === 0 || e.message === `cancelled`
    ? `ok`
    : `error`;
}
function lr(e, t) {
  let n = cr(t) === `error` ? t?.message : void 0;
  return { ...(n && { "sentry.status.message": n }), ...e };
}
var ur = `_sentryChildSpans`,
  dr = `_sentryRootSpan`;
function fr(e, t) {
  (T(t, dr, e[dr] || e), e[ur] ? e[ur].add(t) : T(e, ur, new Set([t])));
}
function pr(e, t) {
  e[ur] && e[ur].delete(t);
}
function mr(e) {
  let t = new Set();
  function n(e) {
    if (!t.has(e) && or(e)) {
      t.add(e);
      let r = e[ur] ? Array.from(e[ur]) : [];
      for (let e of r) n(e);
    }
  }
  return (n(e), Array.from(t));
}
var B = hr;
function hr(e) {
  return e[dr] || e;
}
function V() {
  let e = Ht(r());
  return e.getActiveSpan ? e.getActiveSpan() : Dt(N());
}
function gr() {
  qn ||=
    (l(() => {
      console.warn(
        "[Sentry] Returning null from `beforeSendSpan` is disallowed. To drop certain spans, configure the respective integrations directly or use `ignoreSpans`."
      );
    }),
    !0);
}
var _r = !1;
function vr() {
  if (_r) return;
  function t() {
    let t = V(),
      n = t && B(t);
    if (n) {
      let t = `internal_error`;
      (e && v.log(`[Tracing] Root span: ${t} -> Global error occurred`),
        n.setStatus({ code: 2, message: t }));
    }
  }
  ((_r = !0), de(t), me(t));
}
function H(e) {
  if (typeof __SENTRY_TRACING__ == `boolean` && !__SENTRY_TRACING__) return !1;
  let t = e || F()?.getOptions();
  return !!t && (t.tracesSampleRate != null || !!t.tracesSampler);
}
function yr(e) {
  v.log(
    `Ignoring span ${e.op} - ${e.description} because it matches \`ignoreSpans\`.`
  );
}
function br(t, n) {
  if (!n?.length) return !1;
  for (let r of n) {
    if (Cr(r)) {
      if (t.description && nt(t.description, r)) return (e && yr(t), !0);
      continue;
    }
    let n = !!r.attributes && Object.keys(r.attributes).length > 0;
    if (!r.name && !r.op && !n) continue;
    let i = !r.name || (t.description && nt(t.description, r.name)),
      a = !r.op || (t.op && nt(t.op, r.op)),
      o =
        !r.attributes ||
        Object.entries(r.attributes).every(([e, n]) =>
          xr(t.attributes?.[e], n)
        );
    if (i && a && o) return (e && yr(t), !0);
  }
  return !1;
}
function xr(e, t) {
  return typeof e == `string` && (typeof t == `string` || t instanceof RegExp)
    ? nt(e, t)
    : Array.isArray(e) && Array.isArray(t)
      ? e.length === t.length && e.every((e, n) => e === t[n])
      : e === t;
}
function Sr(e, t) {
  let n = t.parent_span_id,
    r = t.span_id;
  if (n) for (let t of e) t.parent_span_id === r && (t.parent_span_id = n);
}
function Cr(e) {
  return typeof e == `string` || e instanceof RegExp;
}
var wr = Symbol.for(`sentry.nonRecordingSpan`),
  Tr = class {
    constructor(e = {}) {
      ((this._traceId = e.traceId || j()),
        (this._spanId = e.spanId || M()),
        (this.dropReason = e.dropReason),
        T(this, wr, !0));
    }
    spanContext() {
      return { spanId: this._spanId, traceId: this._traceId, traceFlags: 0 };
    }
    end(e) {}
    setAttribute(e, t) {
      return this;
    }
    setAttributes(e) {
      return this;
    }
    setStatus(e) {
      return this;
    }
    updateName(e) {
      return this;
    }
    isRecording() {
      return !1;
    }
    addEvent(e, t, n) {
      return this;
    }
    addLink(e) {
      return this;
    }
    addLinks(e) {
      return this;
    }
    recordException(e, t) {}
  };
function Er(e) {
  return !!e && e[wr] === !0;
}
var Dr = `production`,
  Or = `_frozenDsc`;
function kr(e, t) {
  T(e, Or, t);
}
function Ar(e, t) {
  let n = t.getOptions(),
    { publicKey: r } = t.getDsn() || {},
    i = {
      environment: n.environment || `production`,
      release: n.release,
      public_key: r,
      trace_id: e,
      org_id: Rn(t),
    };
  return (t.emit(`createDsc`, i), i);
}
function jr(e, t) {
  let n = t.getPropagationContext();
  return n.dsc || Ar(n.traceId, e);
}
function Mr(e) {
  let t = F();
  if (!t) return {};
  let n = B(e),
    r = z(n),
    i = r.data,
    a = n.spanContext().traceState,
    o =
      a?.get(`sentry.sample_rate`) ??
      i[`sentry.sample_rate`] ??
      i[`sentry.previous_trace_sample_rate`];
  function s(e) {
    return (
      (typeof o == `number` || typeof o == `string`) &&
        (e.sample_rate = `${o}`),
      e
    );
  }
  let c = n[Or];
  if (c) return s(c);
  let l = Er(n),
    u = l && n.dropReason === `ignored`;
  if (l && (!H(t.getOptions()) || u)) {
    let e = bn(n).scope;
    if (e) {
      let n = { ...jr(t, e) };
      return (u && (n.sampled = `false`), s(n));
    }
  }
  let d = a?.get(`sentry.dsc`),
    f = d && Tn(d);
  if (f) return s(f);
  let p = Ar(e.spanContext().traceId, t),
    m = i[`sentry.source`] ?? i[`sentry.segment.name.source`],
    h = r.description;
  return (
    m !== `url` && h && (p.transaction = h),
    H() &&
      ((p.sampled = String(or(n))),
      (p.sample_rand =
        a?.get(`sentry.sample_rand`) ??
        bn(n).scope?.getPropagationContext().sampleRand.toString())),
    s(p),
    t.emit(`createDsc`, p, n),
    p
  );
}
function Nr(e) {
  return !!e && typeof e == `function` && `_streamed` in e && !!e._streamed;
}
function Pr(e, t = []) {
  return [e, t];
}
function Fr(e, t) {
  let [n, r] = e;
  return [n, [...r, t]];
}
function Ir(e, t) {
  let n = e[1];
  for (let e of n) {
    let n = e[0].type;
    if (t(e, n)) return !0;
  }
  return !1;
}
function Lr(e, t) {
  return Ir(e, (e, n) => t.includes(n));
}
function Rr(e) {
  let n = i(t);
  return n.encodePolyfill ? n.encodePolyfill(e) : new TextEncoder().encode(e);
}
function zr(e) {
  let [t, n] = e,
    r = JSON.stringify(t);
  function i(e) {
    typeof r == `string`
      ? (r = typeof e == `string` ? r + e : [Rr(r), e])
      : r.push(typeof e == `string` ? Rr(e) : e);
  }
  for (let e of n) {
    let [t, n] = e;
    if (
      (i(`
${JSON.stringify(t)}
`),
      typeof n == `string` || n instanceof Uint8Array)
    )
      i(n);
    else {
      let e;
      try {
        e = JSON.stringify(n);
      } catch {
        e = JSON.stringify(E(n));
      }
      i(e);
    }
  }
  return typeof r == `string` ? r : Br(r);
}
function Br(e) {
  let t = e.reduce((e, t) => e + t.length, 0),
    n = new Uint8Array(t),
    r = 0;
  for (let t of e) (n.set(t, r), (r += t.length));
  return n;
}
function Vr(e) {
  return [{ type: `span` }, e];
}
function Hr(e) {
  let t = typeof e.data == `string` ? Rr(e.data) : e.data;
  return [
    {
      type: `attachment`,
      length: t.length,
      filename: e.filename,
      content_type: e.contentType,
      attachment_type: e.attachmentType,
    },
    t,
  ];
}
var Ur = {
  sessions: `session`,
  event: `error`,
  client_report: `internal`,
  user_report: `default`,
  profile_chunk: `profile`,
  replay_event: `replay`,
  replay_recording: `replay`,
  check_in: `monitor`,
  raw_security: `security`,
  log: `log_item`,
  trace_metric: `metric`,
};
function Wr(e) {
  return e in Ur;
}
function Gr(e) {
  return Wr(e) ? Ur[e] : e;
}
function Kr(e) {
  if (!e?.sdk) return;
  let { name: t, version: n } = e.sdk;
  return { name: t, version: n };
}
function qr(e, t, n, r) {
  let i = e.sdkProcessingMetadata?.dynamicSamplingContext;
  return {
    event_id: e.event_id,
    sent_at: new Date(Be()).toISOString(),
    ...(t && { sdk: t }),
    ...(!!n && r && { dsn: Nn(r) }),
    ...(i && { trace: i }),
  };
}
function Jr(e, t) {
  if (!t) return e;
  let n = e.sdk || {};
  return (
    (e.sdk = {
      ...n,
      name: n.name || t.name,
      version: n.version || t.version,
      integrations: [...(e.sdk?.integrations || []), ...(t.integrations || [])],
      packages: [...(e.sdk?.packages || []), ...(t.packages || [])],
      settings:
        e.sdk?.settings || t.settings
          ? { ...e.sdk?.settings, ...t.settings }
          : void 0,
    }),
    e
  );
}
function Yr(e, t, n, r) {
  let i = Kr(n);
  return Pr(
    {
      sent_at: new Date(Be()).toISOString(),
      ...(i && { sdk: i }),
      ...(!!r && t && { dsn: Nn(t) }),
    },
    [
      `aggregates` in e
        ? [{ type: `sessions` }, e]
        : [{ type: `session` }, e.toJSON()],
    ]
  );
}
function Xr(e, t, n, r) {
  let i = Kr(n),
    a = e.type && e.type !== `replay_event` ? e.type : `event`;
  Jr(e, n?.sdk);
  let o = qr(e, i, r, t);
  return (delete e.sdkProcessingMetadata, Pr(o, [[{ type: a }, e]]));
}
function Zr(e, t) {
  function n(e) {
    return !!e.trace_id && !!e.public_key;
  }
  let r = Mr(e[0]),
    i = t?.getDsn(),
    a = t?.getOptions().tunnel,
    o = {
      sent_at: new Date(Be()).toISOString(),
      ...(n(r) && { trace: r }),
      ...(!!a && i && { dsn: Nn(i) }),
    },
    { beforeSendSpan: s, ignoreSpans: c } = t?.getOptions() || {},
    l = c?.length
      ? e.filter((e) => {
          let t = z(e);
          return !br(
            { description: t.description, op: t.op, attributes: t.data },
            c
          );
        })
      : e,
    u = e.length - l.length;
  u && t?.recordDroppedEvent(`before_send`, `span`, u);
  let d = s
      ? (e) => {
          let t = z(e);
          return (Nr(s) ? t : s(t)) || (gr(), t);
        }
      : z,
    f = [];
  for (let e of l) {
    let t = d(e);
    t && f.push(Vr(t));
  }
  return Pr(o, f);
}
function Qr(t) {
  if (!e) return;
  let {
      description: n = `< unknown name >`,
      op: r = `< unknown op >`,
      parent_span_id: i,
    } = z(t),
    { spanId: a } = t.spanContext(),
    o = or(t),
    s = B(t),
    c = s === t,
    l = `[Tracing] Starting ${o ? `sampled` : `unsampled`} ${c ? `root ` : ``}span`,
    u = [`op: ${r}`, `name: ${n}`, `ID: ${a}`];
  if ((i && u.push(`parent ID: ${i}`), !c)) {
    let { op: e, description: t } = z(s);
    (u.push(`root ID: ${s.spanContext().spanId}`),
      e && u.push(`root op: ${e}`),
      t && u.push(`root description: ${t}`));
  }
  v.log(`${l}
  ${u.join(`
  `)}`);
}
function $r(t) {
  if (!e) return;
  let { description: n = `< unknown name >`, op: r = `< unknown op >` } = z(t),
    { spanId: i } = t.spanContext(),
    a = `[Tracing] Finishing "${r}" ${B(t) === t ? `root ` : ``}span "${n}" with ID ${i}`;
  v.log(a);
}
function ei(t, n, r, i = V()) {
  let a = i && B(i);
  a &&
    (e &&
      v.log(`[Measurement] Setting measurement on root span: ${t} = ${n} ${r}`),
    a.addEvent(t, { [an]: n, [rn]: r }));
}
function ti(e) {
  if (!e || e.length === 0) return;
  let t = {};
  return (
    e.forEach((e) => {
      let n = e.attributes || {},
        r = n[rn],
        i = n[an];
      typeof r == `string` &&
        typeof i == `number` &&
        (t[e.name] = { value: i, unit: r });
    }),
    t
  );
}
function ni() {
  return i(r()).segmentSpanCaptureStrategy;
}
function U(e) {
  return e.getOptions().traceLifecycle === `stream`;
}
var ri = 1e3,
  ii = class {
    constructor(e = {}) {
      ((this._traceId = e.traceId || j()),
        (this._spanId = e.spanId || M()),
        (this._startTime = e.startTimestamp || k()),
        (this._links = e.links),
        (this._attributes = {}),
        this.setAttributes({ [L]: `manual`, [I]: e.op, ...e.attributes }),
        (this._name = e.name),
        e.parentSpanId && (this._parentSpanId = e.parentSpanId),
        `sampled` in e && (this._sampled = e.sampled),
        e.endTimestamp && (this._endTime = e.endTimestamp),
        (this._events = []),
        (this._isStandaloneSpan = e.isStandalone),
        this._endTime && this._onSpanEnded());
    }
    addLink(e) {
      return (
        this._frozen ||
          (this._links ? this._links.push(e) : (this._links = [e])),
        this
      );
    }
    addLinks(e) {
      return (
        this._frozen ||
          (this._links ? this._links.push(...e) : (this._links = e)),
        this
      );
    }
    recordException(e, t) {}
    spanContext() {
      let { _spanId: e, _traceId: t, _sampled: n } = this;
      return { spanId: e, traceId: t, traceFlags: +!!n };
    }
    setAttribute(e, t) {
      return this._frozen
        ? this
        : (t === void 0
            ? delete this._attributes[e]
            : (this._attributes[e] = t),
          e === `sentry.source` && t !== void 0 && xn(this) && Sn(this),
          this);
    }
    setAttributes(e) {
      return (Object.keys(e).forEach((t) => this.setAttribute(t, e[t])), this);
    }
    updateStartTime(e) {
      this._frozen || (this._startTime = R(e));
    }
    setStatus(e) {
      return (this._frozen || (this._status = e), this);
    }
    updateName(e) {
      return this._frozen
        ? this
        : ((this._name = e), xn(this) || this.setAttribute($t, `custom`), this);
    }
    end(e) {
      if (this._endTime) {
        this._frozen = Cn(this);
        return;
      }
      ((this._endTime = R(e)),
        $r(this),
        this._onSpanEnded(),
        (this._frozen = Cn(this)));
    }
    getSpanJSON() {
      return {
        data: this._attributes,
        description: this._name,
        op: this._attributes[I],
        parent_span_id: this._parentSpanId,
        span_id: this._spanId,
        start_timestamp: this._startTime,
        status: sr(this._status),
        timestamp: this._endTime,
        trace_id: this._traceId,
        origin: this._attributes[L],
        profile_id: this._attributes[sn],
        exclusive_time: this._attributes[cn],
        measurements: ti(this._events),
        is_segment: (this._isStandaloneSpan && B(this) === this) || void 0,
        segment_id: this._isStandaloneSpan
          ? B(this).spanContext().spanId
          : void 0,
        links: Qn(this._links),
      };
    }
    getStreamedSpanJSON() {
      return {
        name: this._name ?? ``,
        span_id: this._spanId,
        trace_id: this._traceId,
        parent_span_id: this._parentSpanId,
        start_timestamp: this._startTime,
        end_timestamp: this._endTime ?? this._startTime,
        is_segment: this._isStandaloneSpan || this === B(this),
        status: cr(this._status),
        attributes: lr(this._attributes, this._status),
        links: $n(this._links),
      };
    }
    isRecording() {
      return !this._endTime && !!this._sampled;
    }
    addEvent(t, n, r) {
      if (this._frozen) return this;
      e && v.log(`[Tracing] Adding an event to span:`, t);
      let i = ai(n) ? n : r || k(),
        a = ai(n) ? {} : n || {},
        o = { name: t, time: R(i), attributes: a };
      return (this._events.push(o), this);
    }
    isStandaloneSpan() {
      return !!this._isStandaloneSpan;
    }
    _onSpanEnded() {
      let t = F();
      t &&
        (t.emit(`spanEnd`, this),
        this._isStandaloneSpan || t.emit(`afterSpanEnd`, this));
      let n = B(this),
        r = this._isStandaloneSpan || this === n;
      if (this._isStandaloneSpan) {
        this._sampled
          ? ci(Zr([this], t))
          : (e &&
              v.log(
                `[Tracing] Discarding standalone span because its trace was not chosen to be sampled.`
              ),
            t && t.recordDroppedEvent(`sample_rate`, `span`));
        return;
      }
      if (!r) {
        let e = ni();
        if (e) {
          let t = bn(this).scope || N();
          e.onChildSpanEnded(
            this,
            n,
            (e) => this._convertSpanToTransaction(e),
            t
          );
        }
        return;
      }
      if (t && U(t)) {
        t.emit(`afterSegmentSpanEnd`, this);
        return;
      }
      let i = bn(this).scope || N(),
        a = ni();
      if (a) a.onSegmentSpanEnded((e) => this._convertSpanToTransaction(e), i);
      else {
        let e = this._convertSpanToTransaction();
        e && i.captureEvent(e);
      }
    }
    _convertSpanToTransaction(t = {}) {
      if (!oi(z(this))) return;
      this._name ||=
        (e &&
          v.warn(
            "Transaction has no name, falling back to `<unlabeled transaction>`."
          ),
        `<unlabeled transaction>`);
      let { scope: n, isolationScope: r } = bn(this),
        i = n?.getScopeData().sdkProcessingMetadata?.normalizedRequest;
      if (this._sampled !== !0) return;
      t.onSpanCaptured?.(this);
      let a = [];
      for (let e of mr(this)) {
        if (e === this || si(e) || t.isSpanAlreadyCaptured?.(e)) continue;
        let n = z(e);
        oi(n) && (t.onSpanCaptured?.(e), a.push(n));
      }
      let o = this._attributes[$t];
      delete this._attributes[on];
      let s = !1;
      a.forEach((e) => {
        (delete e.data[on], e.op?.startsWith(`gen_ai.`) && (s = !0));
      });
      let c = {
          contexts: { trace: Jn(this) },
          spans:
            a.length > ri
              ? a
                  .sort((e, t) => e.start_timestamp - t.start_timestamp)
                  .slice(0, ri)
              : a,
          start_timestamp: this._startTime,
          timestamp: this._endTime,
          transaction: this._name,
          type: `transaction`,
          sdkProcessingMetadata: {
            capturedSpanScope: n,
            capturedSpanIsolationScope: r,
            dynamicSamplingContext: Mr(this),
            hasGenAiSpans: s,
          },
          request: i,
          ...(o && { transaction_info: { source: o } }),
        },
        l = ti(this._events);
      return (
        l &&
          Object.keys(l).length &&
          (e &&
            v.log(
              `[Measurements] Adding measurements to transaction event`,
              JSON.stringify(l, void 0, 2)
            ),
          (c.measurements = l)),
        c
      );
    }
  };
function ai(e) {
  return (e && typeof e == `number`) || e instanceof Date || Array.isArray(e);
}
function oi(e) {
  return !!e.start_timestamp && !!e.timestamp && !!e.span_id && !!e.trace_id;
}
function si(e) {
  return e instanceof ii && e.isStandaloneSpan();
}
function ci(e) {
  let t = F();
  if (!t) return;
  let n = e[1];
  if (!n || n.length === 0) {
    t.recordDroppedEvent(`before_send`, `span`);
    return;
  }
  t.sendEnvelope(e);
}
function li(t, n, r) {
  if (!H(t)) return [!1];
  let i, a;
  typeof t.tracesSampler == `function`
    ? ((a = t.tracesSampler({
        ...n,
        inheritOrSampleWith: (e) =>
          typeof n.parentSampleRate == `number`
            ? n.parentSampleRate
            : typeof n.parentSampled == `boolean`
              ? Number(n.parentSampled)
              : e,
      })),
      (i = !0))
    : n.parentSampled === void 0
      ? t.tracesSampleRate !== void 0 && ((a = t.tracesSampleRate), (i = !0))
      : (a = n.parentSampled);
  let o = Bn(a);
  if (o === void 0)
    return (
      e &&
        v.warn(
          `[Tracing] Discarding root span because of invalid sample rate. Sample rate must be a boolean or a number between 0 and 1. Got ${JSON.stringify(a)} of type ${JSON.stringify(typeof a)}.`
        ),
      [!1]
    );
  if (!o)
    return (
      e &&
        v.log(
          `[Tracing] Discarding transaction because ${typeof t.tracesSampler == `function` ? `tracesSampler returned 0 or false` : `a negative sampling decision was inherited or tracesSampleRate is set to 0`}`
        ),
      [!1, o, i]
    );
  let s = r < o;
  return (
    s ||
      (e &&
        v.log(
          `[Tracing] Discarding transaction because it's not included in the random sample (sampling rate = ${Number(a)})`
        )),
    [s, o, i]
  );
}
var ui = `__SENTRY_SUPPRESS_TRACING__`;
function di(e) {
  let t = vi();
  return t.startInactiveSpan ? t.startInactiveSpan(e) : fi(e);
}
function fi(e) {
  let t = _i(e),
    { forceTransaction: n, parentSpan: r } = e;
  return (
    e.scope
      ? (t) => Zt(e.scope, t)
      : r === void 0
        ? (e) => e()
        : (e) => pi(r, e)
  )(() => {
    let i = N(),
      a = xi(i, r),
      o = F();
    return e.onlyIfParent && !a
      ? hi(i, o)
      : gi({ parentSpan: a, spanArguments: t, forceTransaction: n, scope: i });
  });
}
function pi(e, t) {
  let n = vi();
  return n.withActiveSpan
    ? n.withActiveSpan(e, t)
    : Zt((n) => (Et(n, e || void 0), t(n)));
}
function mi(e = N()) {
  let t = vi();
  return t.isTracingSuppressed
    ? t.isTracingSuppressed(e)
    : e.getScopeData().sdkProcessingMetadata[ui] === !0;
}
function hi(e, t) {
  t?.recordDroppedEvent(`no_parent_span`, `span`);
  let n = new Tr({ traceId: e.getPropagationContext().traceId });
  return (yn(n, e, P()), n);
}
function gi({
  parentSpan: e,
  spanArguments: t,
  forceTransaction: n,
  scope: r,
}) {
  let i = P();
  if (!H()) {
    let t = { ...i.getPropagationContext(), ...r.getPropagationContext() },
      a = new Tr({ traceId: e ? e.spanContext().traceId : t.traceId });
    return (e && !n && fr(e, a), yn(a, r, i), a);
  }
  let a = F();
  if (Si(a, t)) {
    mi(r) || a?.recordDroppedEvent(`ignored`, `span`);
    let t = new Tr({
      dropReason: `ignored`,
      traceId: e?.spanContext().traceId ?? r.getPropagationContext().traceId,
    });
    return (e && !n && fr(e, t), yn(t, r, i), t);
  }
  let o;
  if (e && !n) ((o = bi(e, r, t, i)), fr(e, o));
  else if (e) {
    let n = Mr(e),
      { traceId: a, spanId: s } = e.spanContext(),
      c = or(e);
    ((o = yi({ traceId: a, parentSpanId: s, ...t }, r, i, c)), kr(o, n));
  } else {
    let {
      traceId: e,
      dsc: n,
      parentSpanId: a,
      sampled: s,
    } = { ...i.getPropagationContext(), ...r.getPropagationContext() };
    ((o = yi({ traceId: e, parentSpanId: a, ...t }, r, i, s)), n && kr(o, n));
  }
  return (Qr(o), o);
}
function _i(e) {
  let t = { isStandalone: (e.experimental || {}).standalone, ...e };
  if (e.startTime) {
    let n = { ...t };
    return ((n.startTimestamp = R(e.startTime)), delete n.startTime, n);
  }
  return t;
}
function vi() {
  return Ht(r());
}
function yi(t, n, r, i) {
  let a = F(),
    o = a?.getOptions() || {},
    { name: s = `` } = t,
    c = { spanAttributes: { ...t.attributes }, spanName: s, parentSampled: i };
  a?.emit(`beforeSampling`, c, { decision: !1 });
  let l = c.parentSampled ?? i,
    u = c.spanAttributes,
    d = n.getPropagationContext(),
    f = mi(n),
    [p, m, h] = f
      ? [!1]
      : li(
          o,
          {
            name: s,
            parentSampled: l,
            attributes: u,
            normalizedRequest:
              r.getScopeData().sdkProcessingMetadata.normalizedRequest,
            parentSampleRate: Bn(d.dsc?.sample_rate),
          },
          d.sampleRand
        ),
    g = new ii({
      ...t,
      attributes: {
        [$t]: `custom`,
        [en]: m !== void 0 && h ? m : void 0,
        ...u,
      },
      sampled: p,
    });
  return (
    !p &&
      a &&
      !f &&
      (e &&
        v.log(
          `[Tracing] Discarding root span because its trace was not chosen to be sampled.`
        ),
      a.recordDroppedEvent(`sample_rate`, U(a) ? `span` : `transaction`)),
    yn(g, n, r),
    a && a.emit(`spanStart`, g),
    g
  );
}
function bi(e, t, n, r) {
  let { spanId: i, traceId: a } = e.spanContext(),
    o = mi(t),
    s = !o && or(e),
    c = s
      ? new ii({ ...n, parentSpanId: i, traceId: a, sampled: s })
      : new Tr({ traceId: a });
  (fr(e, c), yn(c, t, r));
  let l = F();
  return l
    ? (U(l) &&
        Er(c) &&
        (Er(e) && e.dropReason
          ? ((c.dropReason = e.dropReason),
            l.recordDroppedEvent(e.dropReason, `span`))
          : o ||
            ((c.dropReason = `sample_rate`),
            l.recordDroppedEvent(`sample_rate`, `span`))),
      l.emit(`spanStart`, c),
      n.endTimestamp && (l.emit(`spanEnd`, c), l.emit(`afterSpanEnd`, c)),
      c)
    : c;
}
function xi(e, t) {
  if (t) return t;
  if (t === null) return;
  let n = Dt(e);
  if (!n) return;
  let r = F();
  return (r ? r.getOptions() : {}).parentSpanIsAlwaysRootSpan ? B(n) : n;
}
function Si(e, t) {
  let n = e?.getOptions().ignoreSpans;
  return !e || !U(e) || !n?.length
    ? !1
    : br(
        {
          description: t.name || ``,
          op: t.attributes?.[`sentry.op`] || t.op,
          attributes: t.attributes,
        },
        n
      );
}
function Ci(e) {
  return Er(e) && e.dropReason === `ignored`;
}
var wi = { idleTimeout: 1e3, finalTimeout: 3e4, childSpanTimeout: 15e3 },
  Ti = `heartbeatFailed`,
  Ei = `idleTimeout`,
  Di = `finalTimeout`,
  Oi = `externalFinish`;
function ki(t, n = {}) {
  let r = new Map(),
    i = !1,
    a,
    o = Oi,
    s = !n.disableAutoFinish,
    c = [],
    {
      idleTimeout: l = wi.idleTimeout,
      finalTimeout: u = wi.finalTimeout,
      childSpanTimeout: d = wi.childSpanTimeout,
      beforeSpanEnd: f,
      trimIdleSpanEndTimestamp: p = !0,
    } = n,
    m = F(),
    h = N();
  if (!m || !H()) {
    let e = new Tr({ traceId: h.getPropagationContext().traceId });
    return (yn(e, h, P()), e);
  }
  let g = V(),
    _ = Ai(t);
  _.end = new Proxy(_.end, {
    apply(e, t, n) {
      if ((f && f(_), Er(t))) return;
      let [r, ...i] = n,
        a = R(r || k()),
        o = mr(_).filter((e) => e !== _),
        s = z(_);
      if (!o.length || !p) return (ae(a), Reflect.apply(e, t, [a, ...i]));
      let c = m.getOptions().ignoreSpans,
        l = o?.reduce(
          (e, t) => {
            let n = z(t);
            return !n.timestamp ||
              (c &&
                br(
                  { description: n.description, op: n.op, attributes: n.data },
                  c
                ))
              ? e
              : e
                ? Math.max(e, n.timestamp)
                : n.timestamp;
          },
          void 0
        ),
        d = s.start_timestamp,
        h = Math.min(
          d ? d + u / 1e3 : 1 / 0,
          Math.max(d || -1 / 0, Math.min(a, l || 1 / 0))
        );
      return (ae(h), Reflect.apply(e, t, [h, ...i]));
    },
  });
  function ee() {
    a &&= (clearTimeout(a), void 0);
  }
  function te(e) {
    (ee(),
      (a = setTimeout(() => {
        !i && r.size === 0 && s && ((o = Ei), _.end(e));
      }, l)));
  }
  function ne(e) {
    a = setTimeout(() => {
      !i && s && ((o = Ti), _.end(e));
    }, d);
  }
  function re(e) {
    (ee(), r.set(e, !0), ne(k() + d / 1e3));
  }
  function ie(e) {
    (r.has(e) && r.delete(e), r.size === 0 && te(k() + l / 1e3));
  }
  function ae(t) {
    ((i = !0), r.clear(), c.forEach((e) => e()), Et(h, g));
    let n = z(_),
      { start_timestamp: a } = n;
    if (!a) return;
    n.data[`sentry.idle_span_finish_reason`] || _.setAttribute(nn, o);
    let s = n.status;
    ((!s || s === `unknown`) && _.setStatus({ code: 1 }),
      v.log(`[Tracing] Idle span "${n.op}" finished`));
    let d = mr(_).filter((e) => e !== _),
      f = 0;
    (d.forEach((n) => {
      n.isRecording() &&
        (n.setStatus({ code: 2, message: `cancelled` }),
        n.end(t),
        e &&
          v.log(
            `[Tracing] Cancelling span since span ended early`,
            JSON.stringify(n, void 0, 2)
          ));
      let { timestamp: r = 0, start_timestamp: i = 0 } = z(n),
        a = i <= t,
        o = (u + l) / 1e3,
        s = r - i <= o;
      if (e) {
        let e = JSON.stringify(n, void 0, 2);
        a
          ? s ||
            v.log(
              `[Tracing] Discarding span since it finished after idle span final timeout`,
              e
            )
          : v.log(
              `[Tracing] Discarding span since it happened after idle span was finished`,
              e
            );
      }
      (!s || !a) && (pr(_, n), f++);
    }),
      f > 0 && _.setAttribute(`sentry.idle_span_discarded_spans`, f));
  }
  return (
    c.push(
      m.on(`spanStart`, (e) => {
        i ||
          e === _ ||
          z(e).timestamp ||
          (e instanceof ii && e.isStandaloneSpan()) ||
          (mr(_).includes(e) && re(e.spanContext().spanId));
      })
    ),
    c.push(
      m.on(`spanEnd`, (e) => {
        i || ie(e.spanContext().spanId);
      })
    ),
    c.push(
      m.on(`idleSpanEnableAutoFinish`, (e) => {
        e === _ && ((s = !0), te(), r.size && ne());
      })
    ),
    n.disableAutoFinish || te(),
    setTimeout(() => {
      i ||
        (_.setStatus({ code: 2, message: `deadline_exceeded` }),
        (o = Di),
        _.end());
    }, u),
    _
  );
}
function Ai(t) {
  let n = di(t);
  return (Et(N(), n), e && v.log(`[Tracing] Started span is an idle span`), n);
}
function ji(e, t) {
  let { fingerprint: n, span: r, breadcrumbs: i, sdkProcessingMetadata: a } = t;
  (Fi(e, t), r && Ri(e, r), zi(e, n), Ii(e, i), Li(e, a));
}
function Mi(e, t) {
  let {
    extra: n,
    tags: r,
    attributes: i,
    user: a,
    contexts: o,
    level: s,
    sdkProcessingMetadata: c,
    breadcrumbs: l,
    fingerprint: u,
    eventProcessors: d,
    attachments: f,
    propagationContext: p,
    transactionName: m,
    span: h,
  } = t;
  (Ni(e, `extra`, n),
    Ni(e, `tags`, r),
    Ni(e, `attributes`, i),
    Ni(e, `user`, a),
    Ni(e, `contexts`, o),
    (e.sdkProcessingMetadata = St(e.sdkProcessingMetadata, c, 2)),
    s && (e.level = s),
    m && (e.transactionName = m),
    h && (e.span = h),
    l.length && (e.breadcrumbs = [...e.breadcrumbs, ...l]),
    u.length && (e.fingerprint = [...e.fingerprint, ...u]),
    d.length && (e.eventProcessors = [...e.eventProcessors, ...d]),
    f.length && (e.attachments = [...e.attachments, ...f]),
    (e.propagationContext = { ...e.propagationContext, ...p }));
}
function Ni(e, t, n) {
  e[t] = St(e[t], n, 1);
}
function Pi(e, t) {
  let n = Xt().getScopeData();
  return (e && Mi(n, e.getScopeData()), t && Mi(n, t.getScopeData()), n);
}
function Fi(e, t) {
  let {
    extra: n,
    tags: r,
    user: i,
    contexts: a,
    level: o,
    transactionName: s,
  } = t;
  (Object.keys(n).length && (e.extra = { ...n, ...e.extra }),
    Object.keys(r).length && (e.tags = { ...r, ...e.tags }),
    Object.keys(i).length && (e.user = { ...i, ...e.user }),
    Object.keys(a).length && (e.contexts = { ...a, ...e.contexts }),
    o && (e.level = o),
    s && e.type !== `transaction` && (e.transaction = s));
}
function Ii(e, t) {
  let n = [...(e.breadcrumbs || []), ...t];
  e.breadcrumbs = n.length ? n : void 0;
}
function Li(e, t) {
  e.sdkProcessingMetadata = { ...e.sdkProcessingMetadata, ...t };
}
function Ri(e, t) {
  ((e.contexts = { trace: Yn(t), ...e.contexts }),
    (e.sdkProcessingMetadata = {
      dynamicSamplingContext: Mr(t),
      ...e.sdkProcessingMetadata,
    }));
  let n = z(B(t)).description;
  n && !e.transaction && e.type === `transaction` && (e.transaction = n);
}
function zi(e, t) {
  ((e.fingerprint = e.fingerprint
    ? Array.isArray(e.fingerprint)
      ? e.fingerprint
      : [e.fingerprint]
    : []),
    t && (e.fingerprint = e.fingerprint.concat(t)),
    e.fingerprint.length || delete e.fingerprint);
}
var Bi = `sentry.segment.name`,
  Vi = `sentry.transaction`;
function Hi(e, t) {
  let n = (e.attributes ??= {});
  Object.entries(t).forEach(([e, t]) => {
    t != null && !(e in n) && (n[e] = t);
  });
}
var Ui = 0,
  Wi = 1,
  Gi = 2;
function Ki(e) {
  return new Ji((t) => {
    t(e);
  });
}
function qi(e) {
  return new Ji((t, n) => {
    n(e);
  });
}
var Ji = class e {
  constructor(e) {
    ((this._state = Ui), (this._handlers = []), this._runExecutor(e));
  }
  then(t, n) {
    return new e((e, r) => {
      (this._handlers.push([
        !1,
        (n) => {
          if (!t) e(n);
          else
            try {
              e(t(n));
            } catch (e) {
              r(e);
            }
        },
        (t) => {
          if (!n) r(t);
          else
            try {
              e(n(t));
            } catch (e) {
              r(e);
            }
        },
      ]),
        this._executeHandlers());
    });
  }
  catch(e) {
    return this.then((e) => e, e);
  }
  finally(t) {
    return new e((e, n) => {
      let r, i;
      return this.then(
        (e) => {
          ((i = !1), (r = e), t && t());
        },
        (e) => {
          ((i = !0), (r = e), t && t());
        }
      ).then(() => {
        if (i) {
          n(r);
          return;
        }
        e(r);
      });
    });
  }
  _executeHandlers() {
    if (this._state === Ui) return;
    let e = this._handlers.slice();
    ((this._handlers = []),
      e.forEach((e) => {
        e[0] ||=
          (this._state === Wi && e[1](this._value),
          this._state === Gi && e[2](this._value),
          !0);
      }));
  }
  _runExecutor(e) {
    let t = (e, t) => {
        if (this._state === Ui) {
          if (ke(t)) {
            t.then(n, r);
            return;
          }
          ((this._state = e), (this._value = t), this._executeHandlers());
        }
      },
      n = (e) => {
        t(Wi, e);
      },
      r = (e) => {
        t(Gi, e);
      };
    try {
      e(n, r);
    } catch (e) {
      r(e);
    }
  }
};
function Yi(e, t, n, r = 0) {
  try {
    let i = Xi(t, n, e, r);
    return ke(i) ? i : Ki(i);
  } catch (e) {
    return qi(e);
  }
}
function Xi(t, n, r, i) {
  let a = r[i];
  if (!t || !a) return t;
  let o = a({ ...t }, n);
  return (
    e && o === null && v.log(`Event processor "${a.id || `?`}" dropped event`),
    ke(o) ? o.then((e) => Xi(e, n, r, i + 1)) : Xi(o, n, r, i + 1)
  );
}
var Zi, Qi, $i, ea;
function ta(e) {
  let n = t._sentryDebugIds,
    r = t._debugIds;
  if (!n && !r) return {};
  let i = n ? Object.keys(n) : [],
    a = r ? Object.keys(r) : [];
  if (ea && i.length === Qi && a.length === $i) return ea;
  ((Qi = i.length), ($i = a.length), (ea = {}), (Zi ||= {}));
  let o = (t, n) => {
    for (let r of t) {
      let t = n[r],
        i = Zi?.[r];
      if (i && ea && t) ((ea[i[0]] = t), Zi && (Zi[r] = [i[0], t]));
      else if (t) {
        let n = e(r);
        for (let e = n.length - 1; e >= 0; e--) {
          let i = n[e]?.filename;
          if (i && ea && Zi) {
            ((ea[i] = t), (Zi[r] = [i, t]));
            break;
          }
        }
      }
    }
  };
  return (n && o(i, n), r && o(a, r), ea);
}
function na(e, t, n, r, i, a) {
  let { normalizeDepth: o = 3, normalizeMaxBreadth: s = 1e3 } = e,
    c = {
      ...t,
      event_id: t.event_id || n.event_id || O(),
      timestamp: t.timestamp || pt(),
    },
    l = n.integrations || e.integrations.map((e) => e.name);
  (ra(c, e),
    oa(c, l),
    i && i.emit(`applyFrameMetadata`, t),
    t.type === void 0 && ia(c, e.stackParser));
  let u = ca(r, n.captureContext);
  n.mechanism && lt(c, n.mechanism);
  let d = i ? i.getEventProcessors() : [],
    f = Pi(a, u),
    p = [...(n.attachments || []), ...f.attachments];
  (p.length && (n.attachments = p), ji(c, f));
  let m = [...d, ...f.eventProcessors];
  return (n.data && n.data.__sentry__ === !0 ? Ki(c) : Yi(m, c, n)).then(
    (e) => (e && aa(e), typeof o == `number` && o > 0 ? sa(e, o, s) : e)
  );
}
function ra(e, t) {
  let { environment: n, release: r, dist: i, maxValueLength: a } = t;
  ((e.environment = e.environment || n || `production`),
    !e.release && r && (e.release = r),
    !e.dist && i && (e.dist = i));
  let o = e.request;
  (o?.url && a && (o.url = et(o.url, a)),
    a &&
      e.exception?.values?.forEach((e) => {
        e.value &&= et(e.value, a);
      }));
}
function ia(e, t) {
  let n = ta(t);
  e.exception?.values?.forEach((e) => {
    e.stacktrace?.frames?.forEach((e) => {
      e.filename && (e.debug_id = n[e.filename]);
    });
  });
}
function aa(e) {
  let t = {};
  if (
    (e.exception?.values?.forEach((e) => {
      e.stacktrace?.frames?.forEach((e) => {
        e.debug_id &&
          (e.abs_path
            ? (t[e.abs_path] = e.debug_id)
            : e.filename && (t[e.filename] = e.debug_id),
          delete e.debug_id);
      });
    }),
    Object.keys(t).length === 0)
  )
    return;
  ((e.debug_meta = e.debug_meta || {}),
    (e.debug_meta.images = e.debug_meta.images || []));
  let n = e.debug_meta.images;
  Object.entries(t).forEach(([e, t]) => {
    n.push({ type: `sourcemap`, code_file: e, debug_id: t });
  });
}
function oa(e, t) {
  t.length > 0 &&
    ((e.sdk = e.sdk || {}),
    (e.sdk.integrations = [...(e.sdk.integrations || []), ...t]));
}
function sa(e, t, n) {
  if (!e) return null;
  let r = {
    ...e,
    ...(e.breadcrumbs && {
      breadcrumbs: e.breadcrumbs.map((e) => ({
        ...e,
        ...(e.data && { data: E(e.data, t, n) }),
      })),
    }),
    ...(e.user && { user: E(e.user, t, n) }),
    ...(e.contexts && { contexts: E(e.contexts, t, n) }),
    ...(e.extra && { extra: E(e.extra, t, n) }),
  };
  return (
    e.contexts?.trace &&
      r.contexts &&
      ((r.contexts.trace = e.contexts.trace),
      e.contexts.trace.data &&
        (r.contexts.trace.data = E(e.contexts.trace.data, t, n))),
    e.spans &&
      (r.spans = e.spans.map((e) => ({
        ...e,
        ...(e.data && { data: E(e.data, t, n) }),
      }))),
    e.contexts?.flags &&
      r.contexts &&
      (r.contexts.flags = E(e.contexts.flags, 3, n)),
    r
  );
}
function ca(e, t) {
  if (!t) return e;
  let n = e ? e.clone() : new kt();
  return (n.update(t), n);
}
function la(e) {
  if (e) return ua(e) || fa(e) ? { captureContext: e } : e;
}
function ua(e) {
  return e instanceof kt || typeof e == `function`;
}
var da = [
  `user`,
  `level`,
  `extra`,
  `contexts`,
  `tags`,
  `fingerprint`,
  `propagationContext`,
];
function fa(e) {
  return Object.keys(e).some((e) => da.includes(e));
}
function pa(e, t) {
  return N().captureException(e, la(t));
}
function ma(e, t) {
  return N().captureEvent(e, t);
}
function ha() {
  let e = F();
  return e?.getOptions().enabled !== !1 && !!e?.getTransport();
}
function ga(e) {
  let n = P(),
    { user: r } = Pi(n, N()),
    { userAgent: i } = t.navigator || {},
    a = vt({ user: r, ...(i && { userAgent: i }), ...e }),
    o = n.getSession();
  return (
    o?.status === `ok` && yt(o, { status: `exited` }),
    _a(),
    n.setSession(a),
    a
  );
}
function _a() {
  let e = P(),
    t = N().getSession() || e.getSession();
  (t && bt(t), va(), e.setSession());
}
function va() {
  let e = P(),
    t = F(),
    n = e.getSession();
  n && t && t.captureSession(n);
}
function ya(e = !1) {
  if (e) {
    _a();
    return;
  }
  va();
}
function ba(e) {
  return (typeof e == `object` && typeof e.unref == `function` && e.unref(), e);
}
function xa(e) {
  let t = e.protocol ? `${e.protocol}:` : ``,
    n = e.port ? `:${e.port}` : ``;
  return `${t}//${e.host}${n}${e.path ? `/${e.path}` : ``}/api/`;
}
function Sa(e) {
  return `${xa(e)}${e.projectId}/envelope/`;
}
function Ca(e, t) {
  let n = { sentry_version: `7` };
  return (
    e.publicKey && (n.sentry_key = e.publicKey),
    t && (n.sentry_client = `${t.name}/${t.version}`),
    new URLSearchParams(n).toString()
  );
}
function wa(e, t, n) {
  return t || `${Sa(e)}?${Ca(e, n)}`;
}
var Ta = [];
function Ea(e) {
  let t = {};
  return (
    e.forEach((e) => {
      let { name: n } = e,
        r = t[n];
      (r && !r.isDefaultInstance && e.isDefaultInstance) || (t[n] = e);
    }),
    Object.values(t)
  );
}
function Da(e) {
  let t = e.defaultIntegrations || [],
    n = e.integrations;
  t.forEach((e) => {
    e.isDefaultInstance = !0;
  });
  let r;
  if (Array.isArray(n)) r = [...t, ...n];
  else if (typeof n == `function`) {
    let e = n(t);
    r = Array.isArray(e) ? e : [e];
  } else r = t;
  return Ea(r);
}
function Oa(e, t) {
  let n = {};
  return (
    t.forEach((t) => {
      t?.beforeSetup && t.beforeSetup(e);
    }),
    t.forEach((t) => {
      t && Aa(e, t, n);
    }),
    n
  );
}
function ka(e, t) {
  for (let n of t) n?.afterAllSetup && n.afterAllSetup(e);
}
function Aa(t, n, r) {
  if (r[n.name]) {
    e &&
      v.log(`Integration skipped because it was already installed: ${n.name}`);
    return;
  }
  if (
    ((r[n.name] = n),
    !Ta.includes(n.name) &&
      typeof n.setupOnce == `function` &&
      (n.setupOnce(), Ta.push(n.name)),
    n.setup && typeof n.setup == `function` && n.setup(t),
    typeof n.preprocessEvent == `function`)
  ) {
    let e = n.preprocessEvent.bind(n);
    t.on(`preprocessEvent`, (n, r) => e(n, r, t));
  }
  if (typeof n.processEvent == `function`) {
    let e = n.processEvent.bind(n),
      r = Object.assign((n, r) => e(n, r, t), { id: n.name });
    t.addEventProcessor(r);
  }
  ([`processSpan`, `processSegmentSpan`].forEach((e) => {
    let r = n[e];
    typeof r == `function` && t.on(e, (e) => r.call(n, e, t));
  }),
    e && v.log(`Integration installed: ${n.name}`));
}
function W(e) {
  return e;
}
function ja() {
  return typeof __SENTRY_BROWSER_BUNDLE__ < `u` && !!__SENTRY_BROWSER_BUNDLE__;
}
function Ma() {
  return `npm`;
}
function Na() {
  return (
    !ja() &&
    Object.prototype.toString.call(typeof process < `u` ? process : 0) ===
      `[object process]`
  );
}
function Pa() {
  return typeof window < `u` && (!Na() || Fa());
}
function Fa() {
  return t.process?.type === `renderer`;
}
function Ia(e, t) {
  let n = t ? `auto` : `never`;
  return [
    {
      type: `log`,
      item_count: e.length,
      content_type: `application/vnd.sentry.items.log+json`,
    },
    {
      version: 2,
      ...(Pa() && { ingest_settings: { infer_ip: n, infer_user_agent: n } }),
      items: e,
    },
  ];
}
function La(e, t, n, r, i) {
  let a = {};
  return (
    t?.sdk && (a.sdk = { name: t.sdk.name, version: t.sdk.version }),
    n && r && (a.dsn = Nn(r)),
    Pr(a, [Ia(e, i)])
  );
}
function Ra(e, t) {
  let n = t ?? za(e) ?? [];
  if (n.length === 0) return;
  let r = e.getOptions(),
    i = La(
      n,
      r._metadata,
      r.tunnel,
      e.getDsn(),
      e.getDataCollectionOptions().userInfo
    );
  (Ba().set(e, []), e.emit(`flushLogs`), e.sendEnvelope(i));
}
function za(e) {
  return Ba().get(e);
}
function Ba() {
  return a(`clientToLogBufferMap`, () => new WeakMap());
}
function Va(e, t) {
  let n = t ? `auto` : `never`;
  return [
    {
      type: `trace_metric`,
      item_count: e.length,
      content_type: `application/vnd.sentry.items.trace-metric+json`,
    },
    {
      version: 2,
      ...(Pa() && { ingest_settings: { infer_ip: n, infer_user_agent: n } }),
      items: e,
    },
  ];
}
function Ha(e, t, n, r, i) {
  let a = {};
  return (
    t?.sdk && (a.sdk = { name: t.sdk.name, version: t.sdk.version }),
    n && r && (a.dsn = Nn(r)),
    Pr(a, [Va(e, i)])
  );
}
function Ua(e, t) {
  let n = t ?? Wa(e) ?? [];
  if (n.length === 0) return;
  let r = e.getOptions(),
    i = Ha(
      n,
      r._metadata,
      r.tunnel,
      e.getDsn(),
      e.getDataCollectionOptions().userInfo
    );
  (Ga().set(e, []), e.emit(`flushMetrics`), e.sendEnvelope(i));
}
function Wa(e) {
  return Ga().get(e);
}
function Ga() {
  return a(`clientToMetricBufferMap`, () => new WeakMap());
}
function Ka(e) {
  return rr({
    trace_id: e.trace_id,
    span_id: e.span_id,
    parent_span_id: e.parent_span_id,
    name: e.description || ``,
    start_timestamp: e.start_timestamp,
    end_timestamp: e.timestamp || e.start_timestamp,
    status:
      !e.status || e.status === `ok` || e.status === `cancelled`
        ? `ok`
        : `error`,
    is_segment: !1,
    attributes: { ...e.data },
    links: e.links,
  });
}
function qa(e, t) {
  if (
    e.type !== `transaction` ||
    !e.spans?.length ||
    !e.sdkProcessingMetadata?.hasGenAiSpans ||
    t.getOptions().streamGenAiSpans === !1 ||
    U(t)
  )
    return;
  let n = [],
    r = [];
  for (let t of e.spans)
    t.op?.startsWith(`gen_ai.`) ? n.push(Ka(t)) : r.push(t);
  if (n.length === 0) return;
  e.spans = r;
  let i = t.getDataCollectionOptions().userInfo ? `auto` : `never`;
  return [
    {
      type: `span`,
      item_count: n.length,
      content_type: `application/vnd.sentry.items.span.v2+json`,
    },
    {
      version: 2,
      ...(Pa() && { ingest_settings: { infer_ip: i, infer_user_agent: i } }),
      items: n,
    },
  ];
}
var Ja = Symbol.for(`SentryBufferFullError`);
function Ya(e = 100) {
  let t = new Set();
  function n() {
    return t.size < e;
  }
  function r(e) {
    t.delete(e);
  }
  function i(e) {
    if (!n()) return qi(Ja);
    let i = e();
    return (
      t.add(i),
      i.then(
        () => r(i),
        () => r(i)
      ),
      i
    );
  }
  function a(e) {
    if (!t.size) return Ki(!0);
    let n = Promise.allSettled(Array.from(t)).then(() => !0);
    if (!e) return n;
    let r = [n, new Promise((t) => ba(setTimeout(() => t(!1), e)))];
    return Promise.race(r);
  }
  return {
    get $() {
      return Array.from(t);
    },
    add: i,
    drain: a,
  };
}
var Xa = 60 * 1e3;
function Za(e, t = Be()) {
  let n = parseInt(`${e}`, 10);
  if (!isNaN(n)) return n * 1e3;
  let r = Date.parse(`${e}`);
  return isNaN(r) ? Xa : r - t;
}
function Qa(e, t) {
  return e[t] || e.all || 0;
}
function $a(e, t, n = Be()) {
  return Qa(e, t) > n;
}
function eo(e, { statusCode: t, headers: n }, r = Be()) {
  let i = { ...e },
    a = n?.[`x-sentry-rate-limits`],
    o = n?.[`retry-after`];
  if (a)
    for (let e of a.trim().split(`,`)) {
      let [t, n, , , a] = e.split(`:`, 5),
        o = parseInt(t, 10),
        s = (isNaN(o) ? 60 : o) * 1e3;
      if (!n) i.all = r + s;
      else
        for (let e of n.split(`;`))
          e === `metric_bucket`
            ? (!a || a.split(`;`).includes(`custom`)) && (i[e] = r + s)
            : (i[e] = r + s);
    }
  else o ? (i.all = r + Za(o, r)) : t === 429 && (i.all = r + 60 * 1e3);
  return i;
}
function to(t, n, r = Ya(t.bufferSize || 64)) {
  let i = {},
    a = (e) => r.drain(e);
  function o(a) {
    let o = [];
    if (
      (Ir(a, (e, n) => {
        let r = Gr(n);
        $a(i, r) ? t.recordDroppedEvent(`ratelimit_backoff`, r) : o.push(e);
      }),
      o.length === 0)
    )
      return Promise.resolve({});
    let s = Pr(a[0], o),
      c = (n) => {
        if (Lr(s, [`client_report`])) {
          e &&
            v.warn(
              `Dropping client report. Will not send outcomes (reason: ${n}).`
            );
          return;
        }
        Ir(s, (e, r) => {
          t.recordDroppedEvent(n, Gr(r));
        });
      };
    return r
      .add(() =>
        n({ body: zr(s) }).then(
          (t) =>
            t.statusCode === 413
              ? (e &&
                  v.error(
                    `Sentry responded with status code 413. Envelope was discarded due to exceeding size limits.`
                  ),
                c(`send_error`),
                t)
              : (e &&
                  t.statusCode !== void 0 &&
                  (t.statusCode < 200 || t.statusCode >= 300) &&
                  v.warn(
                    `Sentry responded with status code ${t.statusCode} to sent event.`
                  ),
                (i = eo(i, t)),
                t),
          (t) => {
            throw (
              c(`network_error`),
              e && v.error(`Encountered error running transport request:`, t),
              t
            );
          }
        )
      )
      .then(
        (e) => e,
        (t) => {
          if (t === Ja)
            return (
              e && v.error(`Skipped sending event because buffer is full.`),
              c(`queue_overflow`),
              Promise.resolve({})
            );
          throw t;
        }
      );
  }
  return { send: o, flush: a };
}
function no(e, t, n) {
  let r = [
    { type: `client_report` },
    { timestamp: n || pt(), discarded_events: e },
  ];
  return Pr(t ? { dsn: t } : {}, [r]);
}
function ro(e) {
  let t = [];
  e.message && t.push(e.message);
  try {
    let n = e.exception.values[e.exception.values.length - 1];
    n?.value && (t.push(n.value), n.type && t.push(`${n.type}: ${n.value}`));
  } catch {}
  return t;
}
function io(e) {
  let {
    trace_id: t,
    parent_span_id: n,
    span_id: r,
    status: i,
    origin: a,
    data: o,
    op: s,
  } = e.contexts?.trace ?? {};
  return {
    data: o ?? {},
    description: e.transaction,
    op: s,
    parent_span_id: n,
    span_id: r ?? ``,
    start_timestamp: e.start_timestamp ?? 0,
    status: i,
    timestamp: e.timestamp,
    trace_id: t ?? ``,
    origin: a,
    profile_id: o?.[sn],
    exclusive_time: o?.[cn],
    measurements: e.measurements,
    is_segment: !0,
  };
}
function ao(e) {
  return {
    type: `transaction`,
    timestamp: e.timestamp,
    start_timestamp: e.start_timestamp,
    transaction: e.description,
    contexts: {
      trace: {
        trace_id: e.trace_id,
        span_id: e.span_id,
        parent_span_id: e.parent_span_id,
        op: e.op,
        status: e.status,
        origin: e.origin,
        data: {
          ...e.data,
          ...(e.profile_id && { "sentry.profile_id": e.profile_id }),
          ...(e.exclusive_time && {
            "sentry.exclusive_time": e.exclusive_time,
          }),
        },
      },
    },
    measurements: e.measurements,
  };
}
var oo = [`forwarded`, `-ip`, `remote-`, `via`, `-user`];
function so(e) {
  return e === !0
    ? {
        userInfo: !0,
        cookies: !0,
        httpHeaders: { request: !0, response: !0 },
        httpBodies: [
          `incomingRequest`,
          `outgoingRequest`,
          `incomingResponse`,
          `outgoingResponse`,
        ],
        urlQueryParams: !0,
        graphQL: { document: !0, variables: !0 },
        genAI: { inputs: !0, outputs: !0 },
        databaseQueryData: !0,
        stackFrameVariables: !0,
        frameContextLines: 7,
      }
    : {
        userInfo: !1,
        cookies: { deny: oo },
        httpHeaders: { request: { deny: oo }, response: { deny: oo } },
        httpBodies: [],
        urlQueryParams: { deny: oo },
        graphQL: { document: !0, variables: !0 },
        genAI: { inputs: !1, outputs: !1 },
        databaseQueryData: !1,
        stackFrameVariables: !0,
        frameContextLines: 7,
      };
}
var co = {
  userInfo: !0,
  cookies: !0,
  httpHeaders: { request: !0, response: !0 },
  httpBodies: [
    `incomingRequest`,
    `outgoingRequest`,
    `incomingResponse`,
    `outgoingResponse`,
  ],
  urlQueryParams: !0,
  graphQL: { document: !0, variables: !0 },
  genAI: { inputs: !0, outputs: !0 },
  databaseQueryData: !0,
  stackFrameVariables: !0,
  frameContextLines: 5,
};
function lo(e) {
  let t = e.dataCollection == null ? so(e.sendDefaultPii) : co,
    n = e.dataCollection ?? {};
  return {
    userInfo: n.userInfo ?? t.userInfo,
    cookies: n.cookies ?? t.cookies,
    httpHeaders: {
      request: n.httpHeaders?.request ?? t.httpHeaders.request,
      response: n.httpHeaders?.response ?? t.httpHeaders.response,
    },
    httpBodies: n.httpBodies ?? t.httpBodies,
    urlQueryParams: n.urlQueryParams ?? n.queryParams ?? t.urlQueryParams,
    graphQL: {
      document: n.graphQL?.document ?? t.graphQL.document,
      variables: n.graphQL?.variables ?? t.graphQL.variables,
    },
    genAI: {
      inputs: n.genAI?.inputs ?? t.genAI.inputs,
      outputs: n.genAI?.outputs ?? t.genAI.outputs,
    },
    databaseQueryData: n.databaseQueryData ?? t.databaseQueryData,
    stackFrameVariables: n.stackFrameVariables ?? t.stackFrameVariables,
    frameContextLines: n.frameContextLines ?? t.frameContextLines,
  };
}
var uo = `Not capturing exception because it's already been captured.`,
  fo = `Discarded session because of missing or non-string release`,
  po = Symbol.for(`SentryInternalError`),
  mo = Symbol.for(`SentryDoNotSendEventError`),
  ho = 5e3;
function go(e) {
  return { message: e, [po]: !0 };
}
function _o(e) {
  return { message: e, [mo]: !0 };
}
function vo(e) {
  return Ee(e) && po in e;
}
function yo(e) {
  return Ee(e) && mo in e;
}
function bo(e, t, n, r, i) {
  let a = 0,
    o,
    s = !1;
  (e.on(n, () => {
    ((a = 0), clearTimeout(o), (s = !1));
  }),
    e.on(t, (t) => {
      if (((a += r(t)), a >= 8e5)) i(e);
      else if (!s) {
        let t = e.getOptions()._flushInterval ?? ho;
        t > 0 &&
          ((s = !0),
          (o = ba(
            setTimeout(() => {
              i(e);
            }, t)
          )));
      }
    }),
    e.on(`flush`, () => {
      i(e);
    }));
}
var xo = class {
  constructor(t) {
    if (
      ((this._options = t),
      (this._integrations = {}),
      (this._numProcessing = 0),
      (this._outcomes = {}),
      (this._hooks = {}),
      (this._eventProcessors = []),
      (this._promiseBuffer = Ya(t.transportOptions?.bufferSize ?? 64)),
      (this._dataCollection = lo(t)),
      t.dsn
        ? (this._dsn = zn(t.dsn))
        : e && v.warn(`No DSN provided, client will not send events.`),
      this._dsn)
    ) {
      let e = wa(this._dsn, t.tunnel, t._metadata ? t._metadata.sdk : void 0);
      this._transport = t.transport({
        tunnel: this._options.tunnel,
        recordDroppedEvent: this.recordDroppedEvent.bind(this),
        ...t.transportOptions,
        url: e,
      });
    }
    ((this._options.enableLogs =
      this._options.enableLogs ?? this._options._experiments?.enableLogs),
      this._options.enableLogs &&
        bo(this, `afterCaptureLog`, `flushLogs`, Oo, Ra),
      (this._options.enableMetrics ??
        this._options._experiments?.enableMetrics ??
        !0) &&
        bo(this, `afterCaptureMetric`, `flushMetrics`, Do, Ua));
  }
  captureException(t, n, r) {
    let i = O();
    if (ut(t)) return (e && v.log(uo), i);
    let a = { event_id: i, ...n };
    return (
      this._process(
        () =>
          this.eventFromException(t, a)
            .then((e) => this._captureEvent(e, a, r))
            .then((e) => e),
        `error`
      ),
      a.event_id
    );
  }
  captureMessage(e, t, n, r) {
    let i = { event_id: O(), ...n },
      a = Ce(e) ? e : String(e),
      o = we(e),
      s = o ? this.eventFromMessage(a, t, i) : this.eventFromException(e, i);
    return (
      this._process(
        () => s.then((e) => this._captureEvent(e, i, r)),
        o ? `unknown` : `error`
      ),
      i.event_id
    );
  }
  captureEvent(t, n, r) {
    let i = O();
    if (n?.originalException && ut(n.originalException))
      return (e && v.log(uo), i);
    let a = { event_id: i, ...n },
      o = t.sdkProcessingMetadata || {},
      s = o.capturedSpanScope,
      c = o.capturedSpanIsolationScope,
      l = So(t.type);
    return (
      this._process(() => this._captureEvent(t, a, s || r, c), l),
      a.event_id
    );
  }
  captureSession(e) {
    (this.sendSession(e), yt(e, { init: !1 }));
  }
  getDsn() {
    return this._dsn;
  }
  getOptions() {
    return this._options;
  }
  getDataCollectionOptions() {
    return this._dataCollection;
  }
  getSdkMetadata() {
    return this._options._metadata;
  }
  getTransport() {
    return this._transport;
  }
  async flush(e) {
    let t = this._transport;
    if ((this.emit(`flush`), !t)) return !0;
    let n = await this._isClientDoneProcessing(e),
      r = await t.flush(e);
    return n && r;
  }
  async close(e) {
    let t = await this.flush(e);
    return ((this.getOptions().enabled = !1), this.emit(`close`), t);
  }
  getEventProcessors() {
    return this._eventProcessors;
  }
  addEventProcessor(e) {
    this._eventProcessors.push(e);
  }
  init() {
    (this._isEnabled() ||
      this._options.integrations.some(({ name: e }) =>
        e.startsWith(`Spotlight`)
      )) &&
      this._setupIntegrations();
  }
  getIntegrationByName(e) {
    return this._integrations[e];
  }
  getIntegrationNames() {
    return Object.keys(this._integrations);
  }
  addIntegration(e) {
    let t = this._integrations[e.name];
    (!t && e.beforeSetup && e.beforeSetup(this),
      Aa(this, e, this._integrations),
      t || ka(this, [e]));
  }
  sendEvent(e, t = {}) {
    this.emit(`beforeSendEvent`, e, t);
    let n = qa(e, this),
      r = Xr(e, this._dsn, this._options._metadata, this._options.tunnel);
    for (let e of t.attachments || []) r = Fr(r, Hr(e));
    (n && (r = Fr(r, n)),
      this.sendEnvelope(r).then((t) => this.emit(`afterSendEvent`, e, t)));
  }
  sendSession(t) {
    let { release: n, environment: r = Dr } = this._options;
    if (`aggregates` in t) {
      let i = t.attrs || {};
      if (!i.release && !n) {
        e && v.warn(fo);
        return;
      }
      ((i.release = i.release || n),
        (i.environment = i.environment || r),
        (t.attrs = i));
    } else {
      if (!t.release && !n) {
        e && v.warn(fo);
        return;
      }
      ((t.release = t.release || n), (t.environment = t.environment || r));
    }
    this.emit(`beforeSendSession`, t);
    let i = Yr(t, this._dsn, this._options._metadata, this._options.tunnel);
    this.sendEnvelope(i);
  }
  recordDroppedEvent(t, n, r = 1) {
    if (this._options.sendClientReports) {
      let i = `${t}:${n}`;
      (e && v.log(`Recording outcome: "${i}"${r > 1 ? ` (${r} times)` : ``}`),
        (this._outcomes[i] = (this._outcomes[i] || 0) + r));
    }
  }
  on(e, t) {
    let n = (this._hooks[e] = this._hooks[e] || new Set()),
      r = (...e) => t(...e);
    return (
      n.add(r),
      () => {
        n.delete(r);
      }
    );
  }
  emit(e, ...t) {
    let n = this._hooks[e];
    n && n.forEach((e) => e(...t));
  }
  async sendEnvelope(t) {
    if ((this.emit(`beforeEnvelope`, t), this._isEnabled() && this._transport))
      try {
        return await this._transport.send(t);
      } catch (t) {
        return (e && v.error(`Error while sending envelope:`, t), {});
      }
    return (e && v.error(`Transport disabled`), {});
  }
  registerCleanup(e) {}
  dispose() {}
  _setupIntegrations() {
    let { integrations: e } = this._options;
    ((this._integrations = Oa(this, e)), ka(this, e));
  }
  _updateSessionFromEvent(e, t) {
    let n = t.level === `fatal`,
      r = !1,
      i = t.exception?.values;
    if (i) {
      ((r = !0), (n = !1));
      for (let e of i)
        if (e.mechanism?.handled === !1) {
          n = !0;
          break;
        }
    }
    let a = e.status === `ok`;
    ((a && e.errors === 0) || (a && n)) &&
      (yt(e, {
        ...(n && { status: `crashed` }),
        errors: e.errors || Number(r || n),
      }),
      this.captureSession(e));
  }
  async _isClientDoneProcessing(e) {
    let t = 0;
    for (; !e || t < e;) {
      if ((await new Promise((e) => setTimeout(e, 1)), !this._numProcessing))
        return !0;
      t++;
    }
    return !1;
  }
  _isEnabled() {
    return this.getOptions().enabled !== !1 && this._transport !== void 0;
  }
  _prepareEvent(e, t, n, r) {
    let i = this.getOptions(),
      a = this.getIntegrationNames();
    return (
      !t.integrations && a.length && (t.integrations = a),
      this.emit(`preprocessEvent`, e, t),
      e.type || r.setLastEventId(e.event_id || t.event_id),
      na(i, e, t, n, this, r).then((e) =>
        e === null
          ? e
          : (this.emit(`postprocessEvent`, e, t),
            (e.contexts = {
              trace: { ...e.contexts?.trace, ...Qt(n) },
              ...e.contexts,
            }),
            (e.sdkProcessingMetadata = {
              dynamicSamplingContext: jr(this, n),
              ...e.sdkProcessingMetadata,
            }),
            e)
      )
    );
  }
  _captureEvent(t, n = {}, r = N(), i = P()) {
    return (
      e &&
        To(t) &&
        v.log(`Captured error event \`${ro(t)[0] || `<unknown>`}\``),
      this._processEvent(t, n, r, i).then(
        (e) => e.event_id,
        (t) => {
          e &&
            (yo(t) ? v.log(t.message) : vo(t) ? v.warn(t.message) : v.warn(t));
        }
      )
    );
  }
  _processEvent(e, t, n, r) {
    let i = this.getOptions(),
      { sampleRate: a } = i,
      o = Eo(e),
      s = To(e),
      c = `before send for type \`${e.type || `error`}\``,
      l = a === void 0 ? void 0 : Bn(a);
    if (s && typeof l == `number` && ze() > l)
      return (
        this.recordDroppedEvent(`sample_rate`, `error`),
        qi(
          _o(
            `Discarding event because it's not included in the random sample (sampling rate = ${a})`
          )
        )
      );
    let u = So(e.type);
    return this._prepareEvent(e, t, n, r)
      .then((e) => {
        if (e === null)
          throw (
            this.recordDroppedEvent(`event_processor`, u),
            _o("An event processor returned `null`, will not send event.")
          );
        return t.data?.__sentry__ === !0 ? e : Co(wo(this, i, e, t), c);
      })
      .then((i) => {
        if (i === null) {
          if ((this.recordDroppedEvent(`before_send`, u), o)) {
            let t = 1 + (e.spans || []).length;
            this.recordDroppedEvent(`before_send`, `span`, t);
          }
          throw _o(`${c} returned \`null\`, will not send event.`);
        }
        let a = n.getSession() || r.getSession();
        if ((s && a && this._updateSessionFromEvent(a, i), o)) {
          let e =
            (i.sdkProcessingMetadata?.spanCountBeforeProcessing || 0) -
            (i.spans ? i.spans.length : 0);
          e > 0 && this.recordDroppedEvent(`before_send`, `span`, e);
        }
        let l = i.transaction_info;
        return (
          o &&
            l &&
            i.transaction !== e.transaction &&
            (i.transaction_info = { ...l, source: `custom` }),
          this.sendEvent(i, t),
          i
        );
      })
      .then(null, (e) => {
        throw yo(e) || vo(e)
          ? e
          : (this.captureException(e, {
              mechanism: { handled: !1, type: `internal` },
              data: { __sentry__: !0 },
              originalException: e,
            }),
            go(`Event processing pipeline threw an error, original event will not be sent. Details have been sent as a new event.
Reason: ${e}`));
      });
  }
  _process(e, t) {
    (this._numProcessing++,
      this._promiseBuffer.add(e).then(
        (e) => (this._numProcessing--, e),
        (e) => (
          this._numProcessing--,
          e === Ja && this.recordDroppedEvent(`queue_overflow`, t),
          e
        )
      ));
  }
  _clearOutcomes() {
    let e = this._outcomes;
    return (
      (this._outcomes = {}),
      Object.entries(e).map(([e, t]) => {
        let [n, r] = e.split(`:`);
        return { reason: n, category: r, quantity: t };
      })
    );
  }
  _flushOutcomes() {
    e && v.log(`Flushing outcomes...`);
    let t = this._clearOutcomes();
    if (t.length === 0) {
      e && v.log(`No outcomes to send`);
      return;
    }
    if (!this._dsn) {
      e && v.log(`No dsn provided, will not send outcomes`);
      return;
    }
    e && v.log(`Sending outcomes:`, t);
    let n = no(t, this._options.tunnel && Nn(this._dsn));
    this.sendEnvelope(n);
  }
};
function So(e) {
  return e === `replay_event` ? `replay` : e || `error`;
}
function Co(e, t) {
  let n = `${t} must return \`null\` or a valid event.`;
  if (ke(e))
    return e.then(
      (e) => {
        if (!Te(e) && e !== null) throw go(n);
        return e;
      },
      (e) => {
        throw go(`${t} rejected with ${e}`);
      }
    );
  if (!Te(e) && e !== null) throw go(n);
  return e;
}
function wo(e, t, n, r) {
  let { beforeSend: i, beforeSendTransaction: a, ignoreSpans: o } = t,
    s = !Nr(t.beforeSendSpan) && t.beforeSendSpan,
    c = n;
  if (To(c) && i) return i(c, r);
  if (Eo(c)) {
    if (s || o) {
      let t = io(c);
      if (
        o?.length &&
        br({ description: t.description, op: t.op, attributes: t.data }, o)
      )
        return null;
      if (s) {
        let e = s(t);
        e ? (c = St(n, ao(e))) : gr();
      }
      if (c.spans) {
        let t = [],
          n = c.spans;
        for (let e of n) {
          if (
            o?.length &&
            br({ description: e.description, op: e.op, attributes: e.data }, o)
          ) {
            Sr(n, e);
            continue;
          }
          if (s) {
            let n = s(e);
            n ? t.push(n) : (gr(), t.push(e));
          } else t.push(e);
        }
        let r = c.spans.length - t.length;
        (r && e.recordDroppedEvent(`before_send`, `span`, r), (c.spans = t));
      }
    }
    if (a) {
      if (c.spans) {
        let e = c.spans.length;
        c.sdkProcessingMetadata = {
          ...n.sdkProcessingMetadata,
          spanCountBeforeProcessing: e,
        };
      }
      return a(c, r);
    }
  }
  return c;
}
function To(e) {
  return e.type === void 0;
}
function Eo(e) {
  return e.type === `transaction`;
}
function Do(e) {
  let t = 0;
  return (e.name && (t += e.name.length * 2), (t += 8), t + ko(e.attributes));
}
function Oo(e) {
  let t = 0;
  return (e.message && (t += e.message.length * 2), t + ko(e.attributes));
}
function ko(e) {
  if (!e) return 0;
  let t = 0;
  return (
    Object.values(e).forEach((e) => {
      Array.isArray(e)
        ? (t += e.length * Ao(e[0]))
        : we(e)
          ? (t += Ao(e))
          : (t += 100);
    }),
    t
  );
}
function Ao(e) {
  return typeof e == `string`
    ? e.length * 2
    : typeof e == `number`
      ? 8
      : typeof e == `boolean`
        ? 4
        : 0;
}
function jo(t, n) {
  (n.debug === !0 &&
    (e
      ? v.enable()
      : l(() => {
          console.warn(
            "[Sentry] Cannot initialize SDK with `debug` option using a non-debug bundle."
          );
        })),
    N().update(n.initialScope));
  let r = new t(n);
  return (Mo(r), r.init(), r);
}
function Mo(e) {
  N().setClient(e);
}
var No = `thismessage:/`;
function Po(e) {
  return `isRelative` in e;
}
function Fo(e, t) {
  let n = e.indexOf(`://`) <= 0 && e.indexOf(`//`) !== 0,
    r = t ?? (n ? No : void 0);
  try {
    if (`canParse` in URL && !URL.canParse(e, r)) return;
    let t = new URL(e, r);
    return n
      ? { isRelative: n, pathname: t.pathname, search: t.search, hash: t.hash }
      : t;
  } catch {}
}
function Io(e) {
  if (Po(e)) return e.pathname;
  let t = new URL(e);
  return (
    (t.search = ``),
    (t.hash = ``),
    [`80`, `443`].includes(t.port) && (t.port = ``),
    (t.password &&= `%filtered%`),
    (t.username &&= `%filtered%`),
    t.toString()
  );
}
function Lo(e) {
  if (!e) return {};
  let t = e.match(
    /^(([^:/?#]+):)?(\/\/([^/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))?$/
  );
  if (!t) return {};
  let n = t[6] || ``,
    r = t[8] || ``;
  return {
    host: t[4],
    path: t[5],
    protocol: t[2],
    search: n,
    hash: r,
    relative: t[5] + n + r,
  };
}
function Ro(e) {
  return e.split(/[?#]/, 1)[0];
}
function G(e, t = !0) {
  if (e.startsWith(`data:`)) {
    let n = e.match(/^data:([^;,]+)/),
      r = n ? n[1] : `text/plain`,
      i = e.includes(`;base64,`),
      a = e.indexOf(`,`),
      o = ``;
    if (t && a !== -1) {
      let t = e.slice(a + 1);
      o = t.length > 10 ? `${t.slice(0, 10)}... [truncated]` : t;
    }
    return `data:${r}${i ? `,base64` : ``}${o ? `,${o}` : ``}`;
  }
  return e;
}
function zo(e) {
  `aggregates` in e
    ? e.attrs?.ip_address === void 0 &&
      (e.attrs = { ...e.attrs, ip_address: `{{auto}}` })
    : e.ipAddress === void 0 && (e.ipAddress = `{{auto}}`);
}
function Bo(e, t, r = [t], i = `npm`) {
  let a = ((e._metadata = e._metadata || {}).sdk = e._metadata.sdk || {});
  a.name ||
    ((a.name = `sentry.javascript.${t}`),
    (a.packages = r.map((e) => ({ name: `${i}:@sentry/${e}`, version: n }))),
    (a.version = n));
}
function Vo(e = {}) {
  let t = e.client || F();
  if (!ha() || !t) return {};
  let n = Ht(r());
  if (n.getTraceData) return n.getTraceData(e);
  let i = e.scope || N(),
    a = e.span || V(),
    o = Er(a) && !H(t.getOptions());
  if (!a && Yt()) return {};
  let s = a && !o ? Xn(a) : Ho(i),
    c = En(a ? Mr(a) : jr(t, i));
  if (!Vn.test(s))
    return (
      v.warn(`Invalid sentry-trace data. Cannot generate trace data`),
      {}
    );
  let l = { "sentry-trace": s, baggage: c };
  return (
    e.propagateTraceparent && (l.traceparent = a && !o ? Zn(a) : Uo(i)),
    l
  );
}
function Ho(e) {
  let {
    traceId: t,
    sampled: n,
    propagationSpanId: r,
  } = e.getPropagationContext();
  return Wn(t, r, n);
}
function Uo(e) {
  let {
    traceId: t,
    sampled: n,
    propagationSpanId: r,
  } = e.getPropagationContext();
  return Gn(t, r, n);
}
var Wo = 100;
function Go(e, t) {
  let n = F(),
    r = P();
  if (!n) return;
  let { beforeBreadcrumb: i = null, maxBreadcrumbs: a = Wo } = n.getOptions();
  if (a <= 0) return;
  let o = { timestamp: pt(), ...e },
    s = i ? l(() => i(o, t)) : o;
  s !== null &&
    (n.emit && n.emit(`beforeAddBreadcrumb`, s, t), r.addBreadcrumb(s, a));
}
var Ko = `FunctionToString`,
  qo = new WeakMap(),
  Jo = W(() => ({
    name: Ko,
    setupOnce() {
      let e = Function.prototype.toString;
      try {
        Function.prototype.toString = new Proxy(e, {
          apply(e, t, n) {
            let r = Ne(t),
              i = t;
            try {
              qo.has(F()) && r && (i = r);
            } catch {}
            return Reflect.apply(e, i, n);
          },
        });
      } catch {}
    },
    setup(e) {
      qo.set(e, !0);
    },
  })),
  Yo = [
    /^Script error\.?$/,
    /^Javascript error: Script error\.? on line 0$/,
    /^ResizeObserver loop completed with undelivered notifications.$/,
    /^Cannot redefine property: googletag$/,
    /^Can't find variable: gmo$/,
    /^undefined is not an object \(evaluating 'a\.[A-Z]'\)$/,
    /can't redefine non-configurable property "solana"/,
    /vv\(\)\.getRestrictions is not a function/,
    /Can't find variable: _AutofillCallbackHandler/,
    /Object Not Found Matching Id:\d+, MethodName:simulateEvent/,
    /^Java exception was raised during method invocation$/,
  ],
  Xo = `EventFilters`,
  Zo = W((e = {}) => {
    let t;
    return {
      name: Xo,
      setup(n) {
        t = $o(e, n.getOptions());
      },
      processEvent(n, r, i) {
        return ((t ||= $o(e, i.getOptions())), es(n, t) ? null : n);
      },
    };
  }),
  Qo = W((e = {}) => ({ ...Zo(e), name: `InboundFilters` }));
function $o(e = {}, t = {}) {
  return {
    allowUrls: [...(e.allowUrls || []), ...(t.allowUrls || [])],
    denyUrls: [...(e.denyUrls || []), ...(t.denyUrls || [])],
    ignoreErrors: [
      ...(e.ignoreErrors || []),
      ...(t.ignoreErrors || []),
      ...(e.disableErrorDefaults ? [] : Yo),
    ],
    ignoreTransactions: [
      ...(e.ignoreTransactions || []),
      ...(t.ignoreTransactions || []),
    ],
  };
}
function es(t, n) {
  if (!t.type) {
    if (ts(t, n.ignoreErrors))
      return (
        e &&
          v.warn(`Event dropped due to being matched by \`ignoreErrors\` option.
Event: ${st(t)}`),
        !0
      );
    if (ss(t))
      return (
        e &&
          v.warn(`Event dropped due to not having an error message, error type or stacktrace.
Event: ${st(t)}`),
        !0
      );
    if (rs(t, n.denyUrls))
      return (
        e &&
          v.warn(`Event dropped due to being matched by \`denyUrls\` option.
Event: ${st(t)}.
Url: ${os(t)}`),
        !0
      );
    if (!is(t, n.allowUrls))
      return (
        e &&
          v.warn(`Event dropped due to not being matched by \`allowUrls\` option.
Event: ${st(t)}.
Url: ${os(t)}`),
        !0
      );
  } else if (t.type === `transaction` && ns(t, n.ignoreTransactions))
    return (
      e &&
        v.warn(`Event dropped due to being matched by \`ignoreTransactions\` option.
Event: ${st(t)}`),
      !0
    );
  return !1;
}
function ts(e, t) {
  return t?.length ? ro(e).some((e) => D(e, t)) : !1;
}
function ns(e, t) {
  if (!t?.length) return !1;
  let n = e.transaction;
  return n ? D(n, t) : !1;
}
function rs(e, t) {
  if (!t?.length) return !1;
  let n = os(e);
  return n ? D(n, t) : !1;
}
function is(e, t) {
  if (!t?.length) return !0;
  let n = os(e);
  return !n || D(n, t);
}
function as(e = []) {
  for (let t = e.length - 1; t >= 0; t--) {
    let n = e[t];
    if (n && n.filename !== `<anonymous>` && n.filename !== `[native code]`)
      return n.filename || null;
  }
  return null;
}
function os(t) {
  try {
    let e = [...(t.exception?.values ?? [])]
      .reverse()
      .find(
        (e) => e.mechanism?.parent_id === void 0 && e.stacktrace?.frames?.length
      )?.stacktrace?.frames;
    return e ? as(e) : null;
  } catch {
    return (e && v.error(`Cannot extract url for event ${st(t)}`), null);
  }
}
function ss(e) {
  return e.exception?.values?.length
    ? !e.message &&
        !e.exception.values.some(
          (e) => e.stacktrace || (e.type && e.type !== `Error`) || e.value
        )
    : !1;
}
function cs(e, t, n, r, i, a) {
  if (!i.exception?.values || !a || !Ae(a.originalException, Error)) return;
  let o =
    i.exception.values.length > 0
      ? i.exception.values[i.exception.values.length - 1]
      : void 0;
  o &&
    (i.exception.values = ls(
      e,
      t,
      r,
      a.originalException,
      n,
      i.exception.values,
      o,
      0
    ));
}
function ls(e, t, n, r, i, a, o, s) {
  if (a.length >= n + 1) return a;
  let c = [...a];
  if (Ae(r[i], Error)) {
    ds(o, s, r);
    let a = e(t, r[i]),
      l = c.length;
    (fs(a, i, l, s), (c = ls(e, t, n, r[i], i, [a, ...c], a, l)));
  }
  return (
    us(r) &&
      r.errors.forEach((a, l) => {
        if (Ae(a, Error)) {
          ds(o, s, r);
          let u = e(t, a),
            d = c.length;
          (fs(u, `errors[${l}]`, d, s),
            (c = ls(e, t, n, a, i, [u, ...c], u, d)));
        }
      }),
    c
  );
}
function us(e) {
  return Array.isArray(e.errors);
}
function ds(e, t, n) {
  e.mechanism = {
    handled: !0,
    type: `auto.core.linked_errors`,
    ...(us(n) && { is_exception_group: !0 }),
    ...e.mechanism,
    exception_id: t,
  };
}
function fs(e, t, n, r) {
  e.mechanism = {
    handled: !0,
    ...e.mechanism,
    type: `chained`,
    source: t,
    exception_id: n,
    parent_id: r,
  };
}
function ps(e) {
  return (
    _e(e) &&
    `__sentry_fetch_url_host__` in e &&
    typeof e.__sentry_fetch_url_host__ == `string`
  );
}
function ms(e) {
  return ps(e) ? `${e.message} (${e.__sentry_fetch_url_host__})` : e.message;
}
var hs = new Set([]);
function gs(e) {
  let t = `console`,
    n = x(t, e);
  return (S(t, vs), n);
}
var _s = new Set();
function vs() {
  `console` in t &&
    o.forEach(function (n) {
      _s.has(n) ||
        !(n in t.console) ||
        (_s.add(n),
        w(t.console, n, function (r) {
          return (
            (c[n] = r),
            function (...r) {
              let i = r[0],
                a = c[n],
                o = hs.size && typeof i == `string` && D(i, hs);
              (o || C(`console`, { args: r, level: n }),
                (!o || (e && v.isEnabled())) && a?.apply(t.console, r));
            }
          );
        }));
    });
}
function ys(e) {
  return e === `warn`
    ? `warning`
    : [`fatal`, `error`, `warning`, `log`, `info`, `debug`].includes(e)
      ? e
      : `log`;
}
var bs = `Dedupe`,
  xs = W(() => {
    let t;
    return {
      name: bs,
      processEvent(n) {
        if (n.type) return n;
        try {
          if (Ss(n, t))
            return (
              e &&
                v.warn(
                  `Event dropped due to being a duplicate of previously captured event.`
                ),
              null
            );
        } catch {}
        return (t = n);
      },
    };
  });
function Ss(e, t) {
  return t ? !!(Cs(e, t) || ws(e, t)) : !1;
}
function Cs(e, t) {
  let n = e.message,
    r = t.message;
  return !(
    (!n && !r) ||
    (n && !r) ||
    (!n && r) ||
    n !== r ||
    !Es(e, t) ||
    !Ts(e, t)
  );
}
function ws(e, t) {
  let n = Ds(t),
    r = Ds(e);
  return !(
    !n ||
    !r ||
    n.type !== r.type ||
    n.value !== r.value ||
    !Es(e, t) ||
    !Ts(e, t)
  );
}
function Ts(e, t) {
  let n = ce(e),
    r = ce(t);
  if (!n && !r) return !0;
  if ((n && !r) || (!n && r) || ((n = n), (r = r), r.length !== n.length))
    return !1;
  for (let e = 0; e < r.length; e++) {
    let t = r[e],
      i = n[e];
    if (
      t.filename !== i.filename ||
      t.lineno !== i.lineno ||
      t.colno !== i.colno ||
      t.function !== i.function
    )
      return !1;
  }
  return !0;
}
function Es(e, t) {
  let n = e.fingerprint,
    r = t.fingerprint;
  if (!n && !r) return !0;
  if ((n && !r) || (!n && r)) return !1;
  ((n = n), (r = r));
  try {
    return n.join(``) === r.join(``);
  } catch {
    return !1;
  }
}
function Ds(e) {
  return e.exception?.values?.[0];
}
var Os = `ConversationId`,
  ks = W(() => ({
    name: Os,
    setup(e) {
      e.on(`spanStart`, (e) => {
        let t = N().getScopeData(),
          n = P().getScopeData(),
          r = t.conversationId || n.conversationId;
        if (r) {
          let { op: t, data: n, description: i } = z(e);
          if (
            !t?.startsWith(`gen_ai.`) &&
            !n[`ai.operationId`] &&
            !i?.startsWith(`ai.`)
          )
            return;
          e.setAttribute(dn, r);
        }
      });
    },
  }));
function As(e, t, n, r, i) {
  if (!e.fetchData) return;
  let { method: a, url: o } = e.fetchData,
    s = H() && t(o);
  if (e.endTimestamp) {
    let t = e.fetchData.__span;
    if (!t) return;
    let n = r[t];
    n && (s && (Ns(n, e), js(n, e, i)), delete r[t]);
    return;
  }
  let { spanOrigin: c = `auto.http.browser`, propagateTraceparent: l = !1 } =
      typeof i == `object` ? i : { spanOrigin: i },
    u = F(),
    d = !!V(),
    f = d || (!!u && U(u)),
    p = s && f ? di(Ls(o, a, c)) : new Tr(),
    m = Ci(p) && d ? void 0 : p;
  if (
    (s && !f && u?.recordDroppedEvent(`no_parent_span`, `span`),
    (e.fetchData.__span = p.spanContext().spanId),
    (r[p.spanContext().spanId] = p),
    n(e.fetchData.url))
  ) {
    let t = e.args[0],
      n = { ...(e.args[1] || {}) },
      r = Ms(t, n, H() && f ? m : void 0, l);
    r && ((e.args[1] = n), (n.headers = r));
  }
  if (u) {
    let t = {
      input: e.args,
      response: e.response,
      startTimestamp: e.startTimestamp,
      endTimestamp: e.endTimestamp,
    };
    u.emit(`beforeOutgoingRequestSpan`, p, t);
  }
  return p;
}
function js(e, t, n) {
  (typeof n == `object` && n ? n.onRequestSpanEnd : void 0)?.(e, {
    headers: t.response?.headers,
    error: t.error,
  });
}
function Ms(e, t, n, r) {
  let i = Vo({ span: n, propagateTraceparent: r }),
    a = i[`sentry-trace`],
    o = i.baggage,
    s = i.traceparent;
  if (!a) return;
  let c = t.headers || (je(e) ? e.headers : void 0);
  if (!c) return { ...i };
  if (Fs(c)) {
    let e = new Headers(c);
    if (
      (e.get(`sentry-trace`) || e.set(`sentry-trace`, a),
      r && s && !e.get(`traceparent`) && e.set(`traceparent`, s),
      o)
    ) {
      let t = e.get(`baggage`);
      t ? Ps(t) || e.set(`baggage`, `${t},${o}`) : e.set(`baggage`, o);
    }
    return e;
  } else if (Is(c)) {
    let e = [...c];
    (e.find((e) => e[0] === `sentry-trace`) || e.push([`sentry-trace`, a]),
      r &&
        s &&
        !e.find((e) => e[0] === `traceparent`) &&
        e.push([`traceparent`, s]));
    let t = c.find(
      (e) => e[0] === `baggage` && typeof e[1] == `string` && Ps(e[1])
    );
    return (o && !t && e.push([`baggage`, o]), e);
  } else {
    let e = `sentry-trace` in c ? c[`sentry-trace`] : void 0,
      t = `traceparent` in c ? c.traceparent : void 0,
      n = `baggage` in c ? c.baggage : void 0,
      i = n ? (Array.isArray(n) ? [...n] : [n]) : [],
      l = n && (Array.isArray(n) ? n.find((e) => Ps(e)) : Ps(n));
    o && !l && i.push(o);
    let u = Object.assign({}, c, {
      "sentry-trace": e ?? a,
      baggage: i.length > 0 ? i.join(`,`) : void 0,
    });
    return (r && s && !t && (u.traceparent = s), u);
  }
}
function Ns(e, t) {
  if (t.response) {
    pn(e, t.response.status);
    let n = t.response?.headers?.get(`content-length`);
    if (n) {
      let t = parseInt(n);
      t > 0 && e.setAttribute(`http.response_content_length`, t);
    }
  } else t.error && e.setStatus({ code: 2, message: `internal_error` });
  e.end();
}
function Ps(e) {
  return (
    typeof e == `string` && e.split(`,`).some((e) => e.trim().startsWith(wn))
  );
}
function Fs(e) {
  return typeof Headers < `u` && Ae(e, Headers);
}
function Is(e) {
  return Array.isArray(e)
    ? e.every(
        (e) => Array.isArray(e) && e.length === 2 && typeof e[0] == `string`
      )
    : !1;
}
function Ls(e, t, n) {
  if (e.startsWith(`data:`))
    return { name: `${t} ${G(e)}`, attributes: Rs(e, void 0, t, n) };
  let r = Fo(e);
  return { name: `${t} ${r ? Io(r) : e}`, attributes: Rs(e, r, t, n) };
}
function Rs(e, t, n, r) {
  let i = {
    url: G(e),
    type: `fetch`,
    "http.method": n,
    [L]: r,
    [I]: `http.client`,
  };
  return (
    t &&
      (Po(t) || ((i[`http.url`] = G(t.href)), (i[`server.address`] = t.host)),
      t.search && (i[`http.query`] = t.search),
      t.hash && (i[`http.fragment`] = t.hash)),
    i
  );
}
function zs(e) {
  if (e !== void 0) {
    if (e >= 400 && e < 500) return `warning`;
    if (e >= 500) return `error`;
  }
}
var Bs = t;
function Vs() {
  return `history` in Bs && !!Bs.history;
}
function Hs() {
  if (!(`fetch` in Bs)) return !1;
  try {
    return (new Headers(), new Request(`data:,`), new Response(), !0);
  } catch {
    return !1;
  }
}
function Us(e) {
  return (
    e && /^function\s+\w+\(\)\s+\{\s+\[native code\]\s+\}$/.test(e.toString())
  );
}
function Ws() {
  if (typeof EdgeRuntime == `string`) return !0;
  if (!Hs()) return !1;
  if (Us(Bs.fetch)) return !0;
  let t = !1,
    n = Bs.document;
  if (n && typeof n.createElement == `function`)
    try {
      let e = n.createElement(`iframe`);
      ((e.hidden = !0),
        n.head.appendChild(e),
        e.contentWindow?.fetch && (t = Us(e.contentWindow.fetch)),
        n.head.removeChild(e));
    } catch (t) {
      e &&
        v.warn(
          `Could not create sandbox iframe for pure fetch check, bailing to window.fetch: `,
          t
        );
    }
  return t;
}
function Gs(e, t) {
  let n = `fetch`,
    r = x(n, e);
  return (S(n, () => qs(void 0, t)), r);
}
function Ks(e) {
  let t = `fetch-body-resolved`,
    n = x(t, e);
  return (S(t, () => qs(Ys)), n);
}
function qs(e, n = !1) {
  (n && !Ws()) ||
    w(t, `fetch`, function (n) {
      return function (...r) {
        let i = Error(),
          { method: a, url: o } = Qs(r),
          s = {
            args: r,
            fetchData: { method: a, url: o },
            startTimestamp: k() * 1e3,
            virtualError: i,
            headers: $s(r),
          };
        return (
          e || C(`fetch`, { ...s }),
          n.apply(t, r).then(
            async (t) => (
              e
                ? e(t)
                : C(`fetch`, { ...s, endTimestamp: k() * 1e3, response: t }),
              t
            ),
            (e) => {
              (C(`fetch`, { ...s, endTimestamp: k() * 1e3, error: e }),
                _e(e) &&
                  e.stack === void 0 &&
                  ((e.stack = i.stack), T(e, `framesToPop`, 1)));
              let t = F()?.getOptions().enhanceFetchErrorMessages ?? `always`;
              if (
                t !== !1 &&
                e instanceof TypeError &&
                (e.message === `Failed to fetch` ||
                  e.message === `Load failed` ||
                  e.message ===
                    `NetworkError when attempting to fetch resource.`)
              )
                try {
                  let n = new URL(s.fetchData.url).host;
                  t === `always`
                    ? (e.message = `${e.message} (${n})`)
                    : T(e, `__sentry_fetch_url_host__`, n);
                } catch {}
              throw e;
            }
          )
        );
      };
    });
}
async function Js(e, t) {
  if (e?.body) {
    let n = e.body,
      r = n.getReader(),
      i = setTimeout(() => {
        n.cancel().then(null, () => {});
      }, 90 * 1e3),
      a = !0;
    for (; a;) {
      let e;
      try {
        e = setTimeout(() => {
          n.cancel().then(null, () => {});
        }, 5e3);
        let { done: i } = await r.read();
        (clearTimeout(e), i && (t(), (a = !1)));
      } catch {
        a = !1;
      } finally {
        clearTimeout(e);
      }
    }
    (clearTimeout(i), r.releaseLock(), n.cancel().then(null, () => {}));
  }
}
function Ys(e) {
  let t;
  try {
    t = e.clone();
  } catch {
    return;
  }
  Js(t, () => {
    C(`fetch-body-resolved`, { endTimestamp: k() * 1e3, response: e });
  });
}
function Xs(e, t) {
  return Ee(e) && !!e[t];
}
function Zs(e) {
  return typeof e == `string`
    ? e
    : e
      ? Xs(e, `url`)
        ? e.url
        : e.toString
          ? e.toString()
          : ``
      : ``;
}
function Qs(e) {
  if (e.length === 0) return { method: `GET`, url: `` };
  if (e.length === 2) {
    let [t, n] = e;
    return {
      url: Zs(t),
      method: Xs(n, `method`)
        ? String(n.method).toUpperCase()
        : je(t) && Xs(t, `method`)
          ? String(t.method).toUpperCase()
          : `GET`,
    };
  }
  let t = e[0];
  return {
    url: Zs(t),
    method: Xs(t, `method`) ? String(t.method).toUpperCase() : `GET`,
  };
}
function $s(e) {
  let [t, n] = e;
  try {
    if (typeof n == `object` && n && `headers` in n && n.headers)
      return new Headers(n.headers);
    if (je(t)) return new Headers(t.headers);
  } catch {}
}
var ec = t;
function tc() {
  try {
    return ec.document.location.href;
  } catch {
    return ``;
  }
}
function nc(e, t = 5) {
  if (!ec.HTMLElement) return null;
  let n = e;
  for (let e = 0; e < t; e++) {
    if (!n) return null;
    if (n instanceof HTMLElement) {
      if (n.dataset.sentryComponent) return n.dataset.sentryComponent;
      if (n.dataset.sentryElement) return n.dataset.sentryElement;
    }
    n = n.parentNode;
  }
  return null;
}
var K = t,
  rc = 0;
function ic() {
  return rc > 0;
}
function ac() {
  (rc++,
    setTimeout(() => {
      rc--;
    }));
}
function oc(e, n = {}) {
  function r(e) {
    return typeof e == `function`;
  }
  if (!r(e)) return e;
  try {
    if (Object.prototype.hasOwnProperty.call(e, `__sentry_wrapped__`)) {
      let t = e.__sentry_wrapped__;
      return typeof t == `function` ? t : e;
    }
    if (Ne(e)) return e;
  } catch {
    return e;
  }
  let i = function (...r) {
    t._sentryWrappedDepth = (t._sentryWrappedDepth || 0) + 1;
    try {
      let t = r.map((e) => oc(e, n));
      return e.apply(this, t);
    } catch (e) {
      throw (
        ac(),
        Zt((t) => {
          (t.addEventProcessor(
            (e) => (
              n.mechanism && (ct(e, void 0, void 0), lt(e, n.mechanism)),
              (e.extra = { ...e.extra, arguments: r }),
              e
            )
          ),
            pa(e));
        }),
        e
      );
    } finally {
      t._sentryWrappedDepth = (t._sentryWrappedDepth || 0) - 1;
    }
  };
  try {
    for (let t in e)
      Object.prototype.hasOwnProperty.call(e, t) && (i[t] = e[t]);
  } catch {}
  (Me(i, e), T(e, `__sentry_wrapped__`, i));
  try {
    Object.getOwnPropertyDescriptor(i, `name`).configurable &&
      Object.defineProperty(i, "name", {
        get() {
          return e.name;
        },
      });
  } catch {}
  return i;
}
function sc() {
  let e = tc(),
    { referrer: t } = K.document || {},
    { userAgent: n } = K.navigator || {};
  return {
    url: e,
    headers: { ...(t && { Referer: t }), ...(n && { "User-Agent": n }) },
  };
}
function cc(e, t) {
  let n = dc(e, t),
    r = { type: gc(t), value: _c(t) };
  return (
    n.length && (r.stacktrace = { frames: n }),
    r.type === void 0 &&
      r.value === `` &&
      (r.value = `Unrecoverable error caught`),
    r
  );
}
function lc(e, t, n, r) {
  let i = F()?.getOptions().normalizeDepth,
    a = wc(t),
    o = { __serialized__: qe(t, i) };
  if (a) return { exception: { values: [cc(e, a)] }, extra: o };
  let s = {
    exception: {
      values: [
        {
          type: De(t) ? t.constructor.name : r ? `UnhandledRejection` : `Error`,
          value: Sc(t, { isUnhandledRejection: r }),
        },
      ],
    },
    extra: o,
  };
  if (n) {
    let t = dc(e, n);
    t.length && (s.exception.values[0].stacktrace = { frames: t });
  }
  return s;
}
function uc(e, t) {
  return { exception: { values: [cc(e, t)] } };
}
function dc(e, t) {
  let n = t.stacktrace || t.stack || ``,
    r = pc(t),
    i = mc(t);
  try {
    return e(n, r, i);
  } catch {}
  return [];
}
var fc = /Minified React error #\d+;/i;
function pc(e) {
  return e && fc.test(e.message) ? 1 : 0;
}
function mc(e) {
  return typeof e.framesToPop == `number` ? e.framesToPop : 0;
}
function hc(e) {
  return (
    typeof WebAssembly < `u` &&
    WebAssembly.Exception !== void 0 &&
    e instanceof WebAssembly.Exception
  );
}
function gc(e) {
  let t = e?.name;
  return !t && hc(e)
    ? e.message && Array.isArray(e.message) && e.message.length == 2
      ? e.message[0]
      : `WebAssembly.Exception`
    : t;
}
function _c(e) {
  let t = e?.message;
  return hc(e)
    ? Array.isArray(e.message) && e.message.length == 2
      ? e.message[1]
      : `wasm exception`
    : t
      ? t.error && typeof t.error.message == `string`
        ? ms(t.error)
        : ms(e)
      : `No error message`;
}
function vc(e, t, n, r) {
  let i = bc(e, t, n?.syntheticException || void 0, r);
  return (
    lt(i),
    (i.level = `error`),
    n?.event_id && (i.event_id = n.event_id),
    Ki(i)
  );
}
function yc(e, t, n = `info`, r, i) {
  let a = xc(e, t, r?.syntheticException || void 0, i);
  return ((a.level = n), r?.event_id && (a.event_id = r.event_id), Ki(a));
}
function bc(e, t, n, r, i) {
  let a;
  if (ye(t) && t.error) return uc(e, t.error);
  if (be(t) || xe(t)) {
    let i = t;
    if (`stack` in t) {
      a = uc(e, t);
      let i = a.exception?.values?.[0];
      if (r && n && i && !i.stacktrace) {
        let t = dc(e, n);
        t.length && ((i.stacktrace = { frames: t }), lt(a, { synthetic: !0 }));
      }
    } else {
      let t = i.name || (be(i) ? `DOMError` : `DOMException`),
        o = i.message ? `${t}: ${i.message}` : t;
      ((a = xc(e, o, n, r)), ct(a, o));
    }
    return (
      `code` in i && (a.tags = { ...a.tags, "DOMException.code": `${i.code}` }),
      a
    );
  }
  return _e(t)
    ? uc(e, t)
    : Te(t) || De(t)
      ? ((a = lc(e, t, n, i)), lt(a, { synthetic: !0 }), a)
      : ((a = xc(e, t, n, r)),
        ct(a, `${t}`, void 0),
        lt(a, { synthetic: !0 }),
        a);
}
function xc(e, t, n, r) {
  let i = {};
  if (r && n) {
    let r = dc(e, n);
    (r.length &&
      (i.exception = { values: [{ value: t, stacktrace: { frames: r } }] }),
      lt(i, { synthetic: !0 }));
  }
  if (Ce(t)) {
    let { __sentry_template_string__: e, __sentry_template_values__: n } = t;
    return ((i.logentry = { message: e, params: n }), i);
  }
  return ((i.message = t), i);
}
function Sc(e, { isUnhandledRejection: t }) {
  let n = Ie(e),
    r = t ? `promise rejection` : `exception`;
  return ye(e)
    ? `Event \`ErrorEvent\` captured as ${r} with message \`${e.message}\``
    : De(e)
      ? `Event \`${Cc(e)}\` (type=${e.type}) captured as ${r}`
      : `Object captured as ${r} with keys: ${n}`;
}
function Cc(e) {
  try {
    let t = Object.getPrototypeOf(e);
    return t ? t.constructor.name : void 0;
  } catch {}
}
function wc(e) {
  return Object.values(e).find((e) => e instanceof Error);
}
var Tc = class extends xo {
  constructor(e) {
    let t = Ec(e);
    (Bo(t, `browser`, [`browser`], K.SENTRY_SDK_SOURCE || Ma()), super(t));
    let { userInfo: n } = this.getDataCollectionOptions();
    t._metadata?.sdk &&
      (t._metadata.sdk.settings = {
        infer_ip: n ? `auto` : `never`,
        ...t._metadata.sdk.settings,
      });
    let { sendClientReports: r } = this._options;
    (K.document &&
      K.document.addEventListener(`visibilitychange`, () => {
        K.document.visibilityState === `hidden` &&
          (r && this._flushOutcomes(),
          queueMicrotask(() => {
            this.flush();
          }));
      }),
      n && this.on(`beforeSendSession`, zo));
  }
  eventFromException(e, t) {
    return vc(this._options.stackParser, e, t, this._options.attachStacktrace);
  }
  eventFromMessage(e, t = `info`, n) {
    return yc(
      this._options.stackParser,
      e,
      t,
      n,
      this._options.attachStacktrace
    );
  }
  _prepareEvent(e, t, n, r) {
    return (
      (e.platform = e.platform || `javascript`),
      super._prepareEvent(e, t, n, r)
    );
  }
};
function Ec(e) {
  return {
    release:
      typeof __SENTRY_RELEASE__ == `string`
        ? __SENTRY_RELEASE__
        : K.SENTRY_RELEASE?.id,
    sendClientReports: !0,
    parentSpanIsAlwaysRootSpan: !0,
    ...e,
  };
}
var Dc = typeof __SENTRY_DEBUG__ > `u` || __SENTRY_DEBUG__,
  q = t,
  Oc = (e, t) => (e > t[1] ? `poor` : e > t[0] ? `needs-improvement` : `good`),
  kc = (e, t, n, r) => {
    let i, a;
    return (o) => {
      t.value >= 0 &&
        (o || r) &&
        ((a = t.value - (i ?? 0)),
        (a || i === void 0) &&
          ((i = t.value), (t.delta = a), (t.rating = Oc(t.value, n)), e(t)));
    };
  },
  Ac = (e = !0) => {
    let t = q.performance?.getEntriesByType?.(`navigation`)[0];
    if (!e || (t && t.responseStart > 0 && t.responseStart < performance.now()))
      return t;
  },
  jc = () => Ac()?.activationStart ?? 0;
function Mc(e, t, n) {
  q.document && q.addEventListener(e, t, n);
}
function Nc(e, t, n) {
  q.document && q.removeEventListener(e, t, n);
}
var Pc = -1,
  Fc = new Set(),
  Ic = () =>
    q.document?.visibilityState === `hidden` && !q.document?.prerendering
      ? 0
      : 1 / 0,
  Lc = (e) => {
    if (zc(e) && Pc > -1) {
      if (e.type === `visibilitychange` || e.type === `pagehide`)
        for (let e of Fc) e();
      isFinite(Pc) ||
        ((Pc = e.type === `visibilitychange` ? e.timeStamp : 0),
        Nc(`prerenderingchange`, Lc, !0));
    }
  },
  Rc = () => {
    if (q.document && Pc < 0) {
      let e = jc();
      ((Pc =
        (q.document.prerendering
          ? void 0
          : globalThis.performance
              .getEntriesByType(`visibility-state`)
              .filter((t) => t.name === `hidden` && t.startTime > e)[0]
              ?.startTime) ?? Ic()),
        Mc(`visibilitychange`, Lc, !0),
        Mc(`pagehide`, Lc, !0),
        Mc(`prerenderingchange`, Lc, !0));
    }
    return {
      get firstHiddenTime() {
        return Pc;
      },
      onHidden(e) {
        Fc.add(e);
      },
    };
  };
function zc(e) {
  return e.type === `pagehide` || q.document?.visibilityState === `hidden`;
}
var Bc = () =>
    `v5-${Date.now()}-${Math.floor(Math.random() * 8999999999999) + 0xe8d4a51000}`,
  Vc = (e, t = -1) => {
    let n = Ac(),
      r = `navigate`;
    return (
      n &&
        (q.document?.prerendering || jc() > 0
          ? (r = `prerender`)
          : q.document?.wasDiscarded
            ? (r = `restore`)
            : n.type && (r = n.type.replace(/_/g, `-`))),
      {
        name: e,
        value: t,
        rating: `good`,
        delta: 0,
        entries: [],
        id: Bc(),
        navigationType: r,
      }
    );
  },
  Hc = new WeakMap();
function Uc(e, t) {
  try {
    return (Hc.get(e) || Hc.set(e, new t()), Hc.get(e));
  } catch {
    return new t();
  }
}
var Wc = class {
    constructor() {
      ((this._sessionValue = 0), (this._sessionEntries = []));
    }
    _processEntry(e) {
      if (e.hadRecentInput) return;
      let t = this._sessionEntries[0],
        n = this._sessionEntries[this._sessionEntries.length - 1];
      (this._sessionValue &&
      t &&
      n &&
      e.startTime - n.startTime < 1e3 &&
      e.startTime - t.startTime < 5e3
        ? ((this._sessionValue += e.value), this._sessionEntries.push(e))
        : ((this._sessionValue = e.value), (this._sessionEntries = [e])),
        this._onAfterProcessingUnexpectedShift?.(e));
    }
  },
  Gc = (e, t, n = {}) => {
    try {
      if (PerformanceObserver.supportedEntryTypes.includes(e)) {
        let r = new PerformanceObserver((e) => {
          Promise.resolve().then(() => {
            t(e.getEntries());
          });
        });
        return (r.observe({ type: e, buffered: !0, ...n }), r);
      }
    } catch {}
  },
  Kc = (e) => {
    let t = !1;
    return () => {
      t ||= (e(), !0);
    };
  },
  qc = (e) => {
    q.document?.prerendering
      ? addEventListener(`prerenderingchange`, () => e(), !0)
      : e();
  },
  Jc = [1800, 3e3],
  Yc = (e, t = {}) => {
    qc(() => {
      let n = Rc(),
        r = Vc(`FCP`),
        i,
        a = Gc(`paint`, (e) => {
          for (let t of e)
            t.name === `first-contentful-paint` &&
              (a.disconnect(),
              t.startTime < n.firstHiddenTime &&
                ((r.value = Math.max(t.startTime - jc(), 0)),
                r.entries.push(t),
                i(!0)));
        });
      a && (i = kc(e, r, Jc, t.reportAllChanges));
    });
  },
  Xc = [0.1, 0.25],
  Zc = (e, t = {}) => {
    Yc(
      Kc(() => {
        let n = Vc(`CLS`, 0),
          r,
          i = Rc(),
          a = Uc(t, Wc),
          o = (e) => {
            for (let t of e) a._processEntry(t);
            a._sessionValue > n.value &&
              ((n.value = a._sessionValue),
              (n.entries = a._sessionEntries),
              r());
          },
          s = Gc(`layout-shift`, o);
        s &&
          ((r = kc(e, n, Xc, t.reportAllChanges)),
          i.onHidden(() => {
            (o(s.takeRecords()), r(!0));
          }),
          q?.setTimeout?.(r));
      })
    );
  },
  Qc = 0,
  $c = 1 / 0,
  el = 0,
  tl = (e) => {
    e.forEach((e) => {
      e.interactionId &&
        (($c = Math.min($c, e.interactionId)),
        (el = Math.max(el, e.interactionId)),
        (Qc = el ? (el - $c) / 7 + 1 : 0));
    });
  },
  nl,
  rl = () => (nl ? Qc : performance.interactionCount || 0),
  il = () => {
    `interactionCount` in performance ||
      nl ||
      (nl = Gc(`event`, tl, {
        type: `event`,
        buffered: !0,
        durationThreshold: 0,
      }));
  },
  al = 10,
  ol = 0,
  sl = () => rl() - ol,
  cl = class {
    constructor() {
      ((this._longestInteractionList = []),
        (this._longestInteractionMap = new Map()));
    }
    _resetInteractions() {
      ((ol = rl()),
        (this._longestInteractionList.length = 0),
        this._longestInteractionMap.clear());
    }
    _estimateP98LongestInteraction() {
      let e = Math.min(
        this._longestInteractionList.length - 1,
        Math.floor(sl() / 50)
      );
      return this._longestInteractionList[e];
    }
    _processEntry(e) {
      if (
        (this._onBeforeProcessingEntry?.(e),
        !(e.interactionId || e.entryType === `first-input`))
      )
        return;
      let t = this._longestInteractionList.at(-1),
        n = this._longestInteractionMap.get(e.interactionId);
      if (
        n ||
        this._longestInteractionList.length < al ||
        e.duration > t._latency
      ) {
        if (
          (n
            ? e.duration > n._latency
              ? ((n.entries = [e]), (n._latency = e.duration))
              : e.duration === n._latency &&
                e.startTime === n.entries[0].startTime &&
                n.entries.push(e)
            : ((n = {
                id: e.interactionId,
                entries: [e],
                _latency: e.duration,
              }),
              this._longestInteractionMap.set(n.id, n),
              this._longestInteractionList.push(n)),
          this._longestInteractionList.sort((e, t) => t._latency - e._latency),
          this._longestInteractionList.length > al)
        ) {
          let e = this._longestInteractionList.splice(al);
          for (let t of e) this._longestInteractionMap.delete(t.id);
        }
        this._onAfterProcessingINPCandidate?.(n);
      }
    }
  },
  ll = (e) => {
    let t = q.requestIdleCallback || q.setTimeout;
    q.document?.visibilityState === `hidden`
      ? e()
      : ((e = Kc(e)),
        Mc(`visibilitychange`, e, { once: !0, capture: !0 }),
        Mc(`pagehide`, e, { once: !0, capture: !0 }),
        t(() => {
          (e(),
            Nc(`visibilitychange`, e, { capture: !0 }),
            Nc(`pagehide`, e, { capture: !0 }));
        }));
  },
  ul = [200, 500],
  dl = 40,
  fl = (e, t = {}) => {
    if (!(
      globalThis.PerformanceEventTiming &&
      `interactionId` in PerformanceEventTiming.prototype
    ))
      return;
    let n = Rc();
    qc(() => {
      il();
      let r = Vc(`INP`),
        i,
        a = Uc(t, cl),
        o = (e) => {
          ll(() => {
            for (let t of e) a._processEntry(t);
            let t = a._estimateP98LongestInteraction();
            t &&
              t._latency !== r.value &&
              ((r.value = t._latency), (r.entries = t.entries), i());
          });
        },
        s = Gc(`event`, o, { durationThreshold: t.durationThreshold ?? dl });
      ((i = kc(e, r, ul, t.reportAllChanges)),
        s &&
          (s.observe({ type: `first-input`, buffered: !0 }),
          n.onHidden(() => {
            (o(s.takeRecords()), i(!0));
          })));
    });
  },
  pl = class {
    _processEntry(e) {
      this._onBeforeProcessingEntry?.(e);
    }
  },
  ml = [2500, 4e3],
  hl = (e, t = {}) => {
    qc(() => {
      let n = Rc(),
        r = Vc(`LCP`),
        i,
        a = Uc(t, pl),
        o = (e) => {
          t.reportAllChanges || (e = e.slice(-1));
          for (let t of e)
            (a._processEntry(t),
              t.startTime < n.firstHiddenTime &&
                ((r.value = Math.max(t.startTime - jc(), 0)),
                (r.entries = [t]),
                i()));
        },
        s = Gc(`largest-contentful-paint`, o);
      if (s) {
        i = kc(e, r, ml, t.reportAllChanges);
        let n = Kc(() => {
            (o(s.takeRecords()), s.disconnect(), i(!0));
          }),
          a = (e) => {
            e.isTrusted && (ll(n), Nc(e.type, a, { capture: !0 }));
          };
        for (let e of [`keydown`, `click`, `visibilitychange`])
          Mc(e, a, { capture: !0 });
      }
    });
  },
  gl = [800, 1800],
  _l = (e) => {
    q.document?.prerendering
      ? qc(() => _l(e))
      : q.document?.readyState === `complete`
        ? setTimeout(e)
        : addEventListener(`load`, () => _l(e), !0);
  },
  vl = (e, t = {}) => {
    let n = Vc(`TTFB`),
      r = kc(e, n, gl, t.reportAllChanges);
    _l(() => {
      let e = Ac();
      e &&
        ((n.value = Math.max(e.responseStart - jc(), 0)),
        (n.entries = [e]),
        r(!0));
    });
  },
  yl = {},
  bl = {},
  xl,
  Sl,
  Cl,
  wl;
function Tl(e, t = !1) {
  return Fl(`cls`, e, jl, xl, t);
}
function El(e, t = !1) {
  return Fl(`lcp`, e, Ml, Sl, t);
}
function Dl(e) {
  return Fl(`ttfb`, e, Nl, Cl);
}
function Ol(e) {
  return Fl(`inp`, e, Pl, wl);
}
function kl(e, t) {
  return (Ll(e, t), bl[e] || (Il(e), (bl[e] = !0)), Rl(e, t));
}
function Al(e, t) {
  let n = yl[e];
  if (n?.length)
    for (let r of n)
      try {
        r(t);
      } catch (t) {
        Dc &&
          v.error(
            `Error while triggering instrumentation handler.
Type: ${e}
Name: ${y(r)}
Error:`,
            t
          );
      }
}
function jl() {
  return Zc(
    (e) => {
      (Al(`cls`, { metric: e }), (xl = e));
    },
    { reportAllChanges: !0 }
  );
}
function Ml() {
  return hl(
    (e) => {
      (Al(`lcp`, { metric: e }), (Sl = e));
    },
    { reportAllChanges: !0 }
  );
}
function Nl() {
  return vl((e) => {
    (Al(`ttfb`, { metric: e }), (Cl = e));
  });
}
function Pl() {
  return fl((e) => {
    (Al(`inp`, { metric: e }), (wl = e));
  });
}
function Fl(e, t, n, r, i = !1) {
  Ll(e, t);
  let a;
  return (
    bl[e] || ((a = n()), (bl[e] = !0)),
    r && t({ metric: r }),
    Rl(e, t, i ? a : void 0)
  );
}
function Il(e) {
  let t = {};
  (e === `event` && (t.durationThreshold = 0),
    Gc(
      e,
      (t) => {
        Al(e, { entries: t });
      },
      t
    ));
}
function Ll(e, t) {
  ((yl[e] = yl[e] || []), yl[e].push(t));
}
function Rl(e, t, n) {
  return () => {
    n && n();
    let r = yl[e];
    if (!r) return;
    let i = r.indexOf(t);
    i !== -1 && r.splice(i, 1);
  };
}
function zl(e) {
  return `duration` in e;
}
var Bl = 80,
  Vl = {};
try {
  (typeof Node < `u` &&
    (Vl.parentNode = Object.getOwnPropertyDescriptor(
      Node.prototype,
      `parentNode`
    ).get),
    typeof Element < `u` &&
      ((Vl.tagName = Object.getOwnPropertyDescriptor(
        Element.prototype,
        `tagName`
      ).get),
      (Vl.id = Object.getOwnPropertyDescriptor(Element.prototype, `id`).get),
      (Vl.className = Object.getOwnPropertyDescriptor(
        Element.prototype,
        `className`
      ).get),
      (Vl.getAttribute = Element.prototype.getAttribute)),
    typeof HTMLElement < `u` &&
      (Vl.dataset = Object.getOwnPropertyDescriptor(
        HTMLElement.prototype,
        `dataset`
      ).get));
} catch {}
function Hl(e, t, n) {
  let r = Vl[t];
  if (r)
    try {
      return r.call(e, n);
    } catch {}
  let i = e[t];
  return typeof i == `function` ? i.call(e, n) : i;
}
function J(e, t = {}) {
  if (!e) return `<unknown>`;
  try {
    let n = e,
      r = [],
      i = 0,
      a = 0,
      o,
      s = Array.isArray(t) ? t : t.keyAttrs,
      c = (!Array.isArray(t) && t.maxStringLength) || Bl;
    for (
      ;
      n &&
      i++ < 5 &&
      ((o = Ul(n, s)),
      !(o === `html` || (i > 1 && a + r.length * 3 + o.length >= c)));
    )
      (r.push(o), (a += o.length), (n = Hl(n, `parentNode`)));
    return r.reverse().join(` > `);
  } catch {
    return `<unknown>`;
  }
}
function Ul(e, t) {
  let n = [],
    r = Hl(e, `tagName`);
  if (!r) return ``;
  if (typeof HTMLElement < `u` && e instanceof HTMLElement) {
    let t = Hl(e, `dataset`);
    if (t) {
      if (t.sentryComponent) return t.sentryComponent;
      if (t.sentryElement) return t.sentryElement;
    }
  }
  n.push(r.toLowerCase());
  let i = t?.length
    ? t
        .filter((t) => Hl(e, `getAttribute`, t))
        .map((t) => [t, Hl(e, `getAttribute`, t)])
    : null;
  if (i?.length)
    i.forEach((e) => {
      n.push(`[${e[0]}="${e[1]}"]`);
    });
  else {
    let t = Hl(e, `id`);
    t && n.push(`#${t}`);
    let r = Hl(e, `className`);
    if (r && Se(r)) {
      let e = r.split(/\s+/);
      for (let t of e) n.push(`.${t}`);
    }
  }
  for (let t of [`aria-label`, `type`, `name`, `title`, `alt`]) {
    let r = Hl(e, `getAttribute`, t);
    r && n.push(`[${t}="${r}"]`);
  }
  return n.join(``);
}
var Wl = (e) => {
  let t = (t) => {
    (t.type === `pagehide` || q.document?.visibilityState === `hidden`) && e(t);
  };
  (Mc(`visibilitychange`, t, { capture: !0, once: !0 }),
    Mc(`pagehide`, t, { capture: !0, once: !0 }));
};
function Gl(e) {
  return typeof e == `number` && isFinite(e);
}
function Kl(e, t, n, { ...r }) {
  let i = z(e).start_timestamp;
  return (
    i &&
      i > t &&
      typeof e.updateStartTime == `function` &&
      e.updateStartTime(t),
    pi(e, () => {
      let e = di({ startTime: t, ...r });
      return (e && e.end(n), e);
    })
  );
}
function ql(e) {
  let t = F();
  if (!t) return;
  let { name: n, transaction: r, attributes: i, startTime: a } = e,
    { release: o, environment: s } = t.getOptions(),
    { userInfo: c } = t.getDataCollectionOptions(),
    l = t.getIntegrationByName(`Replay`)?.getReplayId(),
    u = N(),
    d = u.getUser(),
    f = d === void 0 ? void 0 : d.email || d.id || d.ip_address,
    p;
  try {
    p = u.getScopeData().contexts.profile.profile_id;
  } catch {}
  return di({
    name: n,
    attributes: {
      release: o,
      environment: s,
      user: f || void 0,
      profile_id: p || void 0,
      replay_id: l || void 0,
      transaction: r,
      "user_agent.original": q.navigator?.userAgent,
      "client.address": c ? `{{auto}}` : void 0,
      ...i,
    },
    startTime: a,
    experimental: { standalone: !0 },
  });
}
function Jl() {
  return q.addEventListener && q.performance;
}
function Y(e) {
  return e / 1e3;
}
function Yl(e) {
  let t = `unknown`,
    n = `unknown`,
    r = ``;
  for (let i of e) {
    if (i === `/`) {
      [t, n] = e.split(`/`);
      break;
    }
    if (!isNaN(Number(i))) {
      ((t = r === `h` ? `http` : r), (n = e.split(r)[1]));
      break;
    }
    r += i;
  }
  return (r === e && (t = r), { name: t, version: n });
}
function Xl(e) {
  try {
    return PerformanceObserver.supportedEntryTypes.includes(e);
  } catch {
    return !1;
  }
}
function Zl(e, t) {
  let n,
    r = !1;
  function i(e) {
    (!r && n && t(e, n.spanContext().spanId, n), (r = !0));
  }
  Wl(() => {
    i(`pagehide`);
  });
  let a = e.on(`beforeStartNavigationSpan`, (e, t) => {
      t?.isRedirect || (i(`navigation`), a(), o());
    }),
    o = e.on(`afterStartPageLoadSpan`, (e) => {
      ((n = e), o());
    });
}
function Ql(e) {
  let t = 0,
    n;
  if (!Xl(`layout-shift`)) return;
  let r = Tl(({ metric: e }) => {
    let r = e.entries[e.entries.length - 1];
    r && ((t = e.value), (n = r));
  }, !0);
  Zl(e, (e, i) => {
    ($l(t, n, i, e), r());
  });
}
function $l(e, t, n, r) {
  Dc && v.log(`Sending CLS span (${e})`);
  let i = t ? Y((A() || 0) + t.startTime) : k(),
    a = N().getScopeData().transactionName,
    o = t ? J(t.sources[0]?.node) : `Layout shift`,
    s = {
      [L]: `auto.http.browser.cls`,
      [I]: `ui.webvital.cls`,
      [cn]: 0,
      "sentry.pageload.span_id": n,
      "sentry.report_event": r,
    };
  t?.sources &&
    t.sources.forEach((e, t) => {
      s[`cls.source.${t + 1}`] = J(e.node);
    });
  let c = ql({ name: o, transaction: a, attributes: s, startTime: i });
  c && (c.addEvent(`cls`, { [rn]: ``, [an]: e }), c.end(i));
}
function eu(e) {
  return e != null && e > 0 && e <= 6e4;
}
function tu(e) {
  let t = 0,
    n;
  if (!Xl(`largest-contentful-paint`)) return;
  let r = El(({ metric: e }) => {
    let r = e.entries[e.entries.length - 1];
    !r || !eu(e.value) || ((t = e.value), (n = r));
  }, !0);
  Zl(e, (e, i) => {
    (nu(t, n, i, e), r());
  });
}
function nu(e, t, n, r) {
  if (!eu(e)) return;
  Dc && v.log(`Sending LCP span (${e})`);
  let i = Y((A() || 0) + (t?.startTime || 0)),
    a = N().getScopeData().transactionName,
    o = t ? J(t.element) : `Largest contentful paint`,
    s = {
      [L]: `auto.http.browser.lcp`,
      [I]: `ui.webvital.lcp`,
      [cn]: 0,
      "sentry.pageload.span_id": n,
      "sentry.report_event": r,
    };
  t &&
    (t.element && (s[`lcp.element`] = J(t.element)),
    t.id && (s[`lcp.id`] = t.id),
    t.url && (s[`lcp.url`] = t.url),
    t.loadTime != null && (s[`lcp.loadTime`] = t.loadTime),
    t.renderTime != null && (s[`lcp.renderTime`] = t.renderTime),
    t.size != null && (s[`lcp.size`] = t.size));
  let c = ql({ name: o, transaction: a, attributes: s, startTime: i });
  c && (c.addEvent(`lcp`, { [rn]: `millisecond`, [an]: e }), c.end(i));
}
function X(e) {
  return e && ((A() || performance.timeOrigin) + e) / 1e3;
}
function ru(e) {
  let t = {};
  if (e.nextHopProtocol != null) {
    let { name: n, version: r } = Yl(e.nextHopProtocol);
    ((t[`network.protocol.version`] = r), (t[`network.protocol.name`] = n));
  }
  return A() || Jl()?.timeOrigin
    ? iu({
        ...t,
        "http.request.redirect_start": X(e.redirectStart),
        "http.request.redirect_end": X(e.redirectEnd),
        "http.request.worker_start": X(e.workerStart),
        "http.request.fetch_start": X(e.fetchStart),
        "http.request.domain_lookup_start": X(e.domainLookupStart),
        "http.request.domain_lookup_end": X(e.domainLookupEnd),
        "http.request.connect_start": X(e.connectStart),
        "http.request.secure_connection_start": X(e.secureConnectionStart),
        "http.request.connection_end": X(e.connectEnd),
        "http.request.request_start": X(e.requestStart),
        "http.request.response_start": X(e.responseStart),
        "http.request.response_end": X(e.responseEnd),
        "http.request.time_to_first_byte":
          e.responseStart == null ? void 0 : e.responseStart / 1e3,
      })
    : t;
}
function iu(e) {
  return Object.fromEntries(Object.entries(e).filter(([, e]) => e != null));
}
var au = 2147483647,
  ou = 0,
  Z = {},
  Q,
  su;
function cu({
  recordClsStandaloneSpans: e,
  recordLcpStandaloneSpans: t,
  client: n,
}) {
  let r = Jl();
  if (r && A()) {
    r.mark && q.performance.mark(`sentry-tracing-init`);
    let i = t ? tu(n) : t === !1 ? pu() : void 0,
      a = e ? Ql(n) : e === !1 ? fu() : void 0,
      o = mu(),
      s = hu();
    return () => {
      (o(), s(), i?.(), a?.());
    };
  }
  return () => void 0;
}
function lu() {
  kl(`longtask`, ({ entries: e }) => {
    let t = V();
    if (!t) return;
    let { op: n, start_timestamp: r } = z(t);
    for (let i of e) {
      let e = Y(A() + i.startTime),
        a = Y(i.duration);
      (n === `navigation` && r && e < r) ||
        Kl(t, e, e + a, {
          name: `Main UI thread blocked`,
          op: `ui.long-task`,
          attributes: { [L]: `auto.ui.browser.metrics` },
        });
    }
  });
}
function uu() {
  new PerformanceObserver((e) => {
    let t = V();
    if (t)
      for (let n of e.getEntries()) {
        if (!n.scripts[0]) continue;
        let e = Y(A() + n.startTime),
          { start_timestamp: r, op: i } = z(t);
        if (i === `navigation` && r && e < r) continue;
        let a = Y(n.duration),
          o = { [L]: `auto.ui.browser.metrics` },
          {
            invoker: s,
            invokerType: c,
            sourceURL: l,
            sourceFunctionName: u,
            sourceCharPosition: d,
          } = n.scripts[0];
        ((o[`browser.script.invoker`] = s),
          (o[`browser.script.invoker_type`] = c),
          l && (o[`code.filepath`] = l),
          u && (o[`code.function`] = u),
          d !== -1 && (o[`browser.script.source_char_position`] = d),
          Kl(t, e, e + a, {
            name: `Main UI thread blocked`,
            op: `ui.long-animation-frame`,
            attributes: o,
          }));
      }
  }).observe({ type: `long-animation-frame`, buffered: !0 });
}
function du() {
  kl(`event`, ({ entries: e }) => {
    let t = V();
    if (t) {
      for (let n of e)
        if (n.name === `click`) {
          let e = Y(A() + n.startTime),
            r = Y(n.duration),
            i = {
              name: J(n.target),
              op: `ui.interaction.${n.name}`,
              startTime: e,
              attributes: { [L]: `auto.ui.browser.metrics` },
            },
            a = nc(n.target);
          (a && (i.attributes[`ui.component_name`] = a), Kl(t, e, e + r, i));
        }
    }
  });
}
function fu() {
  return Tl(({ metric: e }) => {
    let t = e.entries[e.entries.length - 1];
    t && ((Z.cls = { value: e.value, unit: `` }), (su = t));
  }, !0);
}
function pu() {
  return El(({ metric: e }) => {
    let t = e.entries[e.entries.length - 1];
    !t ||
      !eu(e.value) ||
      ((Z.lcp = { value: e.value, unit: `millisecond` }), (Q = t));
  }, !0);
}
function mu() {
  return Dl(({ metric: e }) => {
    e.entries[e.entries.length - 1] &&
      (Z.ttfb = { value: e.value, unit: `millisecond` });
  });
}
function hu() {
  return kl(`paint`, ({ entries: e }) => {
    let t = Rc();
    for (let n of e) {
      let e = n.startTime < t.firstHiddenTime;
      (n.name === `first-paint` &&
        e &&
        (Z.fp = { value: n.startTime, unit: `millisecond` }),
        n.name === `first-contentful-paint` &&
          e &&
          (Z.fcp = { value: n.startTime, unit: `millisecond` }));
    }
  });
}
function gu(e, t) {
  let n = Jl(),
    r = A();
  if (!n?.getEntries || !r) return;
  let {
      spanStreamingEnabled: i,
      ignorePerformanceApiSpans: a,
      ignoreResourceSpans: o,
    } = t,
    s = Y(r),
    c = n.getEntries(),
    { op: l, start_timestamp: u } = z(e);
  (c.slice(ou).forEach((t) => {
    let n = Y(t.startTime),
      r = Y(Math.max(0, t.duration));
    if (!(l === `navigation` && u && s + n < u))
      switch (t.entryType) {
        case `navigation`:
          Su(e, t, s);
          break;
        case `mark`:
        case `paint`:
        case `measure`:
          bu(e, t, n, r, s, a);
          break;
        case `resource`:
          Eu(e, t, t.name, n, r, s, o);
          break;
      }
  }),
    (ou = Math.max(c.length - 1, 0)),
    Du(e, i));
}
function _u(e, t) {
  let n = A();
  if (!Jl()?.getEntries || !n) {
    vu();
    return;
  }
  let {
      spanStreamingEnabled: r,
      recordClsOnPageloadSpan: i,
      recordLcpOnPageloadSpan: a,
    } = t,
    o = Y(n);
  if (z(e).op === `pageload`) {
    if ((Au(Z), r)) {
      let t = (t, n, r) => {
        let i = r ?? `browser.web_vital.${t}.value`;
        (e.setAttribute(i, n),
          Dc &&
            v.log(
              `Setting web vital attribute`,
              { [i]: n },
              `on pageload span`
            ));
      };
      ([`ttfb`, `fp`, `fcp`].forEach((e) => {
        Z[e] && t(e, Z[e].value);
      }),
        Z[`ttfb.requestTime`] &&
          t(
            `ttfb.requestTime`,
            Z[`ttfb.requestTime`].value,
            `browser.web_vital.ttfb.request_time`
          ));
    } else
      (i || delete Z.cls,
        a || delete Z.lcp,
        Object.entries(Z).forEach(([t, n]) => {
          ei(t, n.value, n.unit, e);
        }),
        Ou(e, t));
    (e.setAttribute(
      r ? `browser.performance.time_origin` : `performance.timeOrigin`,
      o
    ),
      e.setAttribute(
        r
          ? `browser.performance.navigation.activation_start`
          : `performance.activationStart`,
        jc()
      ));
  }
  vu();
}
function vu() {
  ((Q = void 0), (su = void 0), (Z = {}));
}
function yu(e) {
  if (e?.entryType === `measure`)
    try {
      return e.detail.devtools.track === `Components ⚛`;
    } catch {
      return;
    }
}
function bu(e, t, n, r, i, a) {
  if (yu(t) || ([`mark`, `measure`].includes(t.entryType) && D(t.name, a)))
    return;
  let o = Ac(!1),
    s = Y(o ? o.requestStart : 0),
    c = i + Math.max(n, s),
    l = i + n,
    u = l + r,
    d = { [L]: `auto.resource.browser.metrics` };
  (c !== l &&
    ((d[`sentry.browser.measure_happened_before_request`] = !0),
    (d[`sentry.browser.measure_start_time`] = c)),
    xu(d, t),
    c <= u && Kl(e, c, u, { name: t.name, op: t.entryType, attributes: d }));
}
function xu(e, t) {
  try {
    let n = t.detail;
    if (!n) return;
    if (typeof n == `object`) {
      for (let [t, r] of Object.entries(n))
        if (r && we(r)) e[`sentry.browser.measure.detail.${t}`] = r;
        else if (r !== void 0)
          try {
            e[`sentry.browser.measure.detail.${t}`] = JSON.stringify(r);
          } catch {}
      return;
    }
    if (we(n)) {
      e[`sentry.browser.measure.detail`] = n;
      return;
    }
    try {
      e[`sentry.browser.measure.detail`] = JSON.stringify(n);
    } catch {}
  } catch {}
}
function Su(e, t, n) {
  ([
    `unloadEvent`,
    `redirect`,
    `domContentLoadedEvent`,
    `loadEvent`,
    `connect`,
  ].forEach((r) => {
    Cu(e, t, r, n);
  }),
    Cu(e, t, `secureConnection`, n, `TLS/SSL`),
    Cu(e, t, `fetch`, n, `cache`),
    Cu(e, t, `domainLookup`, n, `DNS`),
    Tu(e, t, n));
}
function Cu(e, t, n, r, i = n) {
  let a = t[wu(n)],
    o = t[`${n}Start`];
  !o ||
    !a ||
    Kl(e, r + Y(o), r + Y(a), {
      op: `browser.${i}`,
      name: t.name,
      attributes: {
        [L]: `auto.ui.browser.metrics`,
        ...(n === `redirect` && t.redirectCount != null
          ? { "http.redirect_count": t.redirectCount }
          : {}),
      },
    });
}
function wu(e) {
  return e === `secureConnection`
    ? `connectEnd`
    : e === `fetch`
      ? `domainLookupStart`
      : `${e}End`;
}
function Tu(e, t, n) {
  let r = n + Y(t.requestStart),
    i = n + Y(t.responseEnd),
    a = n + Y(t.responseStart);
  t.responseEnd &&
    (Kl(e, r, i, {
      op: `browser.request`,
      name: t.name,
      attributes: { [L]: `auto.ui.browser.metrics` },
    }),
    Kl(e, a, i, {
      op: `browser.response`,
      name: t.name,
      attributes: { [L]: `auto.ui.browser.metrics` },
    }));
}
function Eu(e, t, n, r, i, a, o) {
  if (t.initiatorType === `xmlhttprequest` || t.initiatorType === `fetch`)
    return;
  let s = t.initiatorType ? `resource.${t.initiatorType}` : `resource.other`;
  if (o?.includes(s)) return;
  let c = { [L]: `auto.resource.browser.metrics` },
    l = Lo(n);
  (l.protocol && (c[`url.scheme`] = l.protocol.split(`:`).pop()),
    l.host && (c[`server.address`] = l.host),
    (c[`url.same_origin`] = n.includes(q.location.origin)),
    (c[ln] = n),
    ku(t, c, [
      [`responseStatus`, `http.response.status_code`],
      [`transferSize`, `http.response_transfer_size`],
      [`encodedBodySize`, `http.response_content_length`],
      [`decodedBodySize`, `http.decoded_response_content_length`],
      [`renderBlockingStatus`, `resource.render_blocking_status`],
      [`deliveryType`, `http.response_delivery_type`],
    ]));
  let u = { ...c, ...ru(t) },
    d = a + r;
  Kl(e, d, d + i, {
    name: n.replace(q.location.origin, ``),
    op: s,
    attributes: u,
  });
}
function Du(e, t) {
  let n = q.navigator;
  if (!n) return;
  let r = n.connection;
  (r &&
    (r.effectiveType &&
      e.setAttribute(
        t ? `network.connection.effective_type` : `effectiveConnectionType`,
        r.effectiveType
      ),
    r.type &&
      e.setAttribute(t ? `network.connection.type` : `connectionType`, r.type),
    Gl(r.rtt) &&
      (t
        ? e.setAttribute(`network.connection.rtt`, r.rtt)
        : z(e).op === `pageload` &&
          ei(`connection.rtt`, r.rtt, `millisecond`))),
    Gl(n.deviceMemory) &&
      (t
        ? e.setAttribute(`device.memory.estimated_capacity`, n.deviceMemory)
        : e.setAttribute(`deviceMemory`, `${n.deviceMemory} GB`)),
    Gl(n.hardwareConcurrency) &&
      (t
        ? e.setAttribute(`device.processor_count`, n.hardwareConcurrency)
        : e.setAttribute(
            `hardwareConcurrency`,
            String(n.hardwareConcurrency)
          )));
}
function Ou(e, t) {
  (Q &&
    t.recordLcpOnPageloadSpan &&
    (Q.element && e.setAttribute(`lcp.element`, J(Q.element)),
    Q.id && e.setAttribute(`lcp.id`, Q.id),
    Q.url && e.setAttribute(`lcp.url`, Q.url.trim().slice(0, 200)),
    Q.loadTime != null && e.setAttribute(`lcp.loadTime`, Q.loadTime),
    Q.renderTime != null && e.setAttribute(`lcp.renderTime`, Q.renderTime),
    e.setAttribute(`lcp.size`, Q.size)),
    su?.sources &&
      t.recordClsOnPageloadSpan &&
      su.sources.forEach((t, n) =>
        e.setAttribute(`cls.source.${n + 1}`, J(t.node))
      ));
}
function ku(e, t, n) {
  n.forEach(([n, r]) => {
    let i = e[n];
    i != null &&
      ((typeof i == `number` && i < au) || typeof i == `string`) &&
      (t[r] = i);
  });
}
function Au(e) {
  let t = Ac(!1);
  if (!t) return;
  let { responseStart: n, requestStart: r } = t;
  r <= n && (e[`ttfb.requestTime`] = { value: n - r, unit: `millisecond` });
}
var ju = [],
  Mu = new Map(),
  Nu = new Map();
function Pu() {
  if (Jl() && A()) {
    let e = Iu();
    return () => {
      e();
    };
  }
  return () => void 0;
}
var Fu = {
  click: `click`,
  pointerdown: `click`,
  pointerup: `click`,
  mousedown: `click`,
  mouseup: `click`,
  touchstart: `click`,
  touchend: `click`,
  mouseover: `hover`,
  mouseout: `hover`,
  mouseenter: `hover`,
  mouseleave: `hover`,
  pointerover: `hover`,
  pointerout: `hover`,
  pointerenter: `hover`,
  pointerleave: `hover`,
  dragstart: `drag`,
  dragend: `drag`,
  drag: `drag`,
  dragenter: `drag`,
  dragleave: `drag`,
  dragover: `drag`,
  drop: `drag`,
  keydown: `press`,
  keyup: `press`,
  keypress: `press`,
  input: `press`,
};
function Iu() {
  return Ol(Lu);
}
var Lu = ({ metric: e }) => {
  if (e.value == null) return;
  let t = Y(e.value);
  if (t > 60) return;
  let n = e.entries.find((t) => t.duration === e.value && Fu[t.name]);
  if (!n) return;
  let { interactionId: r } = n,
    i = Fu[n.name],
    a = Y(A() + n.startTime),
    o = V(),
    s = o ? B(o) : void 0,
    c = r == null ? void 0 : Mu.get(r),
    l = c?.span || s,
    u = l ? z(l).description : N().getScopeData().transactionName,
    d = ql({
      name: c?.elementName || J(n.target),
      transaction: u,
      attributes: {
        [L]: `auto.http.browser.inp`,
        [I]: `ui.interaction.${i}`,
        [cn]: n.duration,
      },
      startTime: a,
    });
  d &&
    (d.addEvent(`inp`, { [rn]: `millisecond`, [an]: e.value }), d.end(a + t));
};
function Ru(e) {
  return e == null ? void 0 : Mu.get(e);
}
function zu() {
  let e = Object.keys(Fu);
  Pa() &&
    e.forEach((e) => {
      q.addEventListener(e, t, { capture: !0, passive: !0 });
    });
  function t(e) {
    let t = e.target;
    if (!t) return;
    let n = J(t),
      r = Math.round(e.timeStamp);
    if ((Nu.set(r, n), Nu.size > 50)) {
      let e = Nu.keys().next().value;
      e !== void 0 && Nu.delete(e);
    }
  }
  function n(e) {
    let t = Math.round(e.startTime),
      n = Nu.get(t);
    if (!n)
      for (let e = -5; e <= 5; e++) {
        let r = Nu.get(t + e);
        if (r) {
          n = r;
          break;
        }
      }
    return n || `<unknown>`;
  }
  let r = ({ entries: e }) => {
    let t = V(),
      r = t && B(t);
    e.forEach((e) => {
      if (!zl(e)) return;
      let t = e.interactionId;
      if (t == null || Mu.has(t)) return;
      let i = e.target ? J(e.target) : n(e);
      if (ju.length > 10) {
        let e = ju.shift();
        Mu.delete(e);
      }
      (ju.push(t), Mu.set(t, { span: r, elementName: i }));
    });
  };
  (kl(`event`, r), kl(`first-input`, r));
}
function Bu(e) {
  let {
      name: t,
      op: n,
      origin: r,
      metricName: i,
      value: a,
      attributes: o,
      parentSpan: s,
      reportEvent: c,
      startTime: l,
      endTime: u,
    } = e,
    d = N().getScopeData().transactionName,
    f = {
      [L]: r,
      [I]: n,
      [cn]: 0,
      [`browser.web_vital.${i}.value`]: a,
      [Vi]: d,
      [Bi]: d,
      "user_agent.original": q.navigator?.userAgent,
      ...o,
    };
  (s &&
    tr(s).attributes?.[`sentry.op`] === `pageload` &&
    (f[`sentry.pageload.span_id`] = s.spanContext().spanId),
    c && (f[`browser.web_vital.${i}.report_event`] = c));
  let p = di({ name: t, attributes: f, startTime: l, parentSpan: s });
  p && p.end(u ?? l);
}
function Vu(e) {
  let t = 0,
    n;
  if (!Xl(`largest-contentful-paint`)) return;
  let r = El(({ metric: e }) => {
    let r = e.entries[e.entries.length - 1];
    !r || !eu(e.value) || ((t = e.value), (n = r));
  }, !0);
  Zl(e, (e, i, a) => {
    (Hu(t, n, a, e), r());
  });
}
function Hu(e, t, n, r) {
  if (!eu(e)) return;
  Dc && v.log(`Sending LCP span (${e})`);
  let i = A() || 0,
    a = Y(i),
    o = Y(i + (t?.startTime || 0)),
    s = t ? J(t.element) : `Largest contentful paint`,
    c = {};
  (t?.element && (c[`browser.web_vital.lcp.element`] = J(t.element)),
    t?.id && (c[`browser.web_vital.lcp.id`] = t.id),
    t?.url && (c[`browser.web_vital.lcp.url`] = t.url),
    t?.loadTime != null && (c[`browser.web_vital.lcp.load_time`] = t.loadTime),
    t?.renderTime != null &&
      (c[`browser.web_vital.lcp.render_time`] = t.renderTime),
    t?.size != null && (c[`browser.web_vital.lcp.size`] = t.size),
    Bu({
      name: s,
      op: `ui.webvital.lcp`,
      origin: `auto.http.browser.lcp`,
      metricName: `lcp`,
      value: e,
      attributes: c,
      parentSpan: n,
      reportEvent: r,
      startTime: a,
      endTime: o,
    }));
}
function Uu(e) {
  let t = 0,
    n;
  if (!Xl(`layout-shift`)) return;
  let r = Tl(({ metric: e }) => {
    let r = e.entries[e.entries.length - 1];
    r && ((t = e.value), (n = r));
  }, !0);
  Zl(e, (e, i, a) => {
    (Wu(t, n, a, e), r());
  });
}
function Wu(e, t, n, r) {
  Dc && v.log(`Sending CLS span (${e})`);
  let i = t ? Y((A() || 0) + t.startTime) : k(),
    a = t ? J(t.sources[0]?.node) : `Layout shift`,
    o = {};
  (t?.sources &&
    t.sources.forEach((e, t) => {
      o[`browser.web_vital.cls.source.${t + 1}`] = J(e.node);
    }),
    Bu({
      name: a,
      op: `ui.webvital.cls`,
      origin: `auto.http.browser.cls`,
      metricName: `cls`,
      value: e,
      attributes: o,
      parentSpan: n,
      reportEvent: r,
      startTime: i,
    }));
}
function Gu() {
  !Jl() ||
    !A() ||
    Ol(({ metric: e }) => {
      if (e.value == null || Y(e.value) > 60) return;
      let t = e.entries.find((t) => t.duration === e.value && Fu[t.name]);
      t && Ku(e.value, t);
    });
}
function Ku(e, t) {
  Dc && v.log(`Sending INP span (${e})`);
  let n = Y(A() + t.startTime),
    r = Y(e),
    i = Fu[t.name],
    a = Ru(t.interactionId),
    o = V(),
    s = o ? B(o) : void 0,
    c = a?.span || s,
    l = c ? tr(c).name : N().getScopeData().transactionName;
  Bu({
    name: a?.elementName || J(t.target),
    op: `ui.interaction.${i}`,
    origin: `auto.http.browser.inp`,
    metricName: `inp`,
    value: e,
    attributes: { [cn]: t.duration, [Vi]: l, [Bi]: l },
    startTime: n,
    endTime: n + r,
    parentSpan: c,
  });
}
var qu = 1e3,
  Ju,
  Yu,
  Xu;
function Zu(e) {
  (x(`dom`, e), S(`dom`, Qu));
}
function Qu() {
  if (!q.document) return;
  let e = C.bind(null, `dom`),
    t = td(e, !0);
  (q.document.addEventListener(`click`, t, !1),
    q.document.addEventListener(`keypress`, t, !1),
    [`EventTarget`, `Node`].forEach((t) => {
      let n = q[t]?.prototype;
      n?.hasOwnProperty?.(`addEventListener`) &&
        (w(n, `addEventListener`, function (t) {
          return function (n, r, i) {
            if (n === `click` || n == `keypress`)
              try {
                let r = (this.__sentry_instrumentation_handlers__ =
                    this.__sentry_instrumentation_handlers__ || {}),
                  a = (r[n] = r[n] || { refCount: 0 });
                if (!a.handler) {
                  let r = td(e);
                  ((a.handler = r), t.call(this, n, r, i));
                }
                a.refCount++;
              } catch {}
            return t.call(this, n, r, i);
          };
        }),
        w(n, `removeEventListener`, function (e) {
          return function (t, n, r) {
            if (t === `click` || t == `keypress`)
              try {
                let n = this.__sentry_instrumentation_handlers__ || {},
                  i = n[t];
                i &&
                  (i.refCount--,
                  i.refCount <= 0 &&
                    (e.call(this, t, i.handler, r),
                    (i.handler = void 0),
                    delete n[t]),
                  Object.keys(n).length === 0 &&
                    delete this.__sentry_instrumentation_handlers__);
              } catch {}
            return e.call(this, t, n, r);
          };
        }));
    }));
}
function $u(e) {
  if (e.type !== Yu) return !1;
  try {
    if (!e.target || e.target._sentryId !== Xu) return !1;
  } catch {}
  return !0;
}
function ed(e, t) {
  return e === `keypress`
    ? !t?.tagName ||
        !(
          t.tagName === `INPUT` ||
          t.tagName === `TEXTAREA` ||
          t.isContentEditable
        )
    : !1;
}
function td(e, t = !1) {
  return (n) => {
    if (!n || n._sentryCaptured) return;
    let r = nd(n);
    if (ed(n.type, r)) return;
    (T(n, `_sentryCaptured`, !0), r && !r._sentryId && T(r, `_sentryId`, O()));
    let i = n.type === `keypress` ? `input` : n.type;
    ($u(n) ||
      (e({ event: n, name: i, global: t }),
      (Yu = n.type),
      (Xu = r ? r._sentryId : void 0)),
      clearTimeout(Ju),
      (Ju = q.setTimeout(() => {
        ((Xu = void 0), (Yu = void 0));
      }, qu)));
  };
}
function nd(e) {
  try {
    return e.target;
  } catch {
    return null;
  }
}
var rd;
function id(e) {
  let t = `history`;
  (x(t, e), S(t, ad));
}
function ad() {
  if (
    (q.addEventListener(`popstate`, () => {
      let e = q.location.href,
        t = rd;
      ((rd = e), t !== e && C(`history`, { from: t, to: e }));
    }),
    !Vs())
  )
    return;
  function e(e) {
    return function (...t) {
      let n = t.length > 2 ? t[2] : void 0;
      if (n) {
        let r = rd,
          i = od(String(n));
        if (((rd = i), r === i)) return e.apply(this, t);
        C(`history`, { from: r, to: i });
      }
      return e.apply(this, t);
    };
  }
  (w(q.history, `pushState`, e), w(q.history, `replaceState`, e));
}
function od(e) {
  try {
    return new URL(e, q.location.origin).toString();
  } catch {
    return e;
  }
}
var sd = {};
function cd(e) {
  let t = sd[e];
  if (t) return t;
  let n = q[e];
  if (Us(n)) return (sd[e] = n.bind(q));
  let r = q.document;
  if (r && typeof r.createElement == `function`)
    try {
      let t = r.createElement(`iframe`);
      ((t.hidden = !0), r.head.appendChild(t));
      let i = t.contentWindow;
      (i?.[e] && (n = i[e]), r.head.removeChild(t));
    } catch (t) {
      Dc &&
        v.warn(
          `Could not create sandbox iframe for ${e} check, bailing to window.${e}: `,
          t
        );
    }
  return n && (sd[e] = n.bind(q));
}
function ld(e) {
  sd[e] = void 0;
}
var ud = `__sentry_xhr_v3__`;
function dd(e) {
  (x(`xhr`, e), S(`xhr`, fd));
}
function fd() {
  if (!q.XMLHttpRequest) return;
  let e = XMLHttpRequest.prototype;
  ((e.open = new Proxy(e.open, {
    apply(e, t, n) {
      let r = Error(),
        i = k() * 1e3,
        a = Se(n[0]) ? n[0].toUpperCase() : void 0,
        o = pd(n[1]);
      if (!a || !o) return e.apply(t, n);
      ((t[ud] = { method: a, url: o, request_headers: {} }),
        a === `POST` &&
          o.match(/sentry_key/) &&
          (t.__sentry_own_request__ = !0));
      let s = () => {
        let e = t[ud];
        if (e && t.readyState === 4) {
          try {
            e.status_code = t.status;
          } catch {}
          (C(`xhr`, {
            endTimestamp: k() * 1e3,
            startTimestamp: i,
            xhr: t,
            virtualError: r,
          }),
            t.removeEventListener(`readystatechange`, s));
        }
      };
      return (
        `onreadystatechange` in t && typeof t.onreadystatechange == `function`
          ? (t.onreadystatechange = new Proxy(t.onreadystatechange, {
              apply(e, t, n) {
                return (s(), e.apply(t, n));
              },
            }))
          : t.addEventListener(`readystatechange`, s),
        (t.setRequestHeader = new Proxy(t.setRequestHeader, {
          apply(e, t, n) {
            let [r, i] = n,
              a = t[ud];
            return (
              a && Se(r) && Se(i) && (a.request_headers[r.toLowerCase()] = i),
              e.apply(t, n)
            );
          },
        })),
        e.apply(t, n)
      );
    },
  })),
    (e.send = new Proxy(e.send, {
      apply(e, t, n) {
        let r = t[ud];
        return r
          ? (n[0] !== void 0 && (r.body = n[0]),
            C(`xhr`, { startTimestamp: k() * 1e3, xhr: t }),
            e.apply(t, n))
          : e.apply(t, n);
      },
    })));
}
function pd(e) {
  if (Se(e)) return e;
  try {
    return e.toString();
  } catch {}
}
function md(e) {
  let t;
  try {
    t = e.getAllResponseHeaders();
  } catch (t) {
    return (Dc && v.error(t, `Failed to get xhr response headers`, e), {});
  }
  return t
    ? t
        .split(
          `\r
`
        )
        .reduce((e, t) => {
          let [n, r] = t.split(`: `);
          return (r && (e[n.toLowerCase()] = r), e);
        }, {})
    : {};
}
function hd(e) {
  if (typeof Element > `u`) return !1;
  try {
    return e instanceof Element;
  } catch {
    return !1;
  }
}
var gd = 40;
function _d(e, t = cd(`fetch`)) {
  let n = 0,
    r = 0;
  async function i(i) {
    let a = i.body.length;
    ((n += a), r++);
    let o = {
      body: i.body,
      method: `POST`,
      referrerPolicy: `strict-origin`,
      headers: e.headers,
      keepalive: n <= 6e4 && r < 15,
      ...e.fetchOptions,
    };
    try {
      let n = await t(e.url, o);
      return {
        statusCode: n.status,
        headers: {
          "x-sentry-rate-limits": n.headers.get(`X-Sentry-Rate-Limits`),
          "retry-after": n.headers.get(`Retry-After`),
        },
      };
    } catch (e) {
      throw (ld(`fetch`), e);
    } finally {
      ((n -= a), r--);
    }
  }
  return to(e, i, Ya(e.bufferSize || gd));
}
var $ = typeof __SENTRY_DEBUG__ > `u` || __SENTRY_DEBUG__,
  vd = 30,
  yd = 50;
function bd(e, t, n, r) {
  let i = { filename: e, function: t === `<anonymous>` ? `?` : t, in_app: !0 };
  return (n !== void 0 && (i.lineno = n), r !== void 0 && (i.colno = r), i);
}
var xd = /^\s*at (\S+?)(?::(\d+))(?::(\d+))\s*$/i,
  Sd =
    /^\s*at (?:(.+?\)(?: \[.+\])?|.*?) ?\((?:address at )?)?(?:async )?((?:<anonymous>|[-a-z]+:|.*bundle|\/)?.*?)(?::(\d+))?(?::(\d+))?\)?\s*$/i,
  Cd = /\((\S*)(?::(\d+))(?::(\d+))\)/,
  wd = /at (.+?) ?\(data:(.+?),/,
  Td = [
    vd,
    (e) => {
      let t = e.match(wd);
      if (t) return { filename: `<data:${t[2]}>`, function: t[1] };
      let n = xd.exec(e);
      if (n) {
        let [, e, t, r] = n;
        return bd(e, `?`, +t, +r);
      }
      let r = Sd.exec(e);
      if (r) {
        if (r[2]?.indexOf(`eval`) === 0) {
          let e = Cd.exec(r[2]);
          e && ((r[2] = e[1]), (r[3] = e[2]), (r[4] = e[3]));
        }
        let [e, t] = kd(r[1] || `?`, r[2]);
        return bd(t, e, r[3] ? +r[3] : void 0, r[4] ? +r[4] : void 0);
      }
    },
  ],
  Ed =
    /^\s*(.*?)(?:\((.*?)\))?(?:^|@)?((?:[-a-z]+)?:\/.*?|\[native code\]|[^@]*(?:bundle|\d+\.js)|\/[\w\-. /=]+)(?::(\d+))?(?::(\d+))?\s*$/i,
  Dd = /(\S+) line (\d+)(?: > eval line \d+)* > eval/i,
  Od = re(Td, [
    yd,
    (e) => {
      let t = Ed.exec(e);
      if (t) {
        if (t[3] && t[3].indexOf(` > eval`) > -1) {
          let e = Dd.exec(t[3]);
          e &&
            ((t[1] = t[1] || `eval`),
            (t[3] = e[1]),
            (t[4] = e[2]),
            (t[5] = ``));
        }
        let e = t[3],
          n = t[1] || `?`;
        return (
          ([n, e] = kd(n, e)),
          bd(e, n, t[4] ? +t[4] : void 0, t[5] ? +t[5] : void 0)
        );
      }
    },
  ]),
  kd = (e, t) => {
    let n = e.indexOf(`safari-extension`) !== -1,
      r = e.indexOf(`safari-web-extension`) !== -1;
    return n || r
      ? [
          e.indexOf(`@`) === -1 ? `?` : e.split(`@`)[0],
          n ? `safari-extension:${t}` : `safari-web-extension:${t}`,
        ]
      : [e, t];
  },
  Ad = 1024,
  jd = `Breadcrumbs`,
  Md = W((e = {}) => {
    let t = {
      console: !0,
      dom: !0,
      fetch: !0,
      history: !0,
      sentry: !0,
      xhr: !0,
      ...e,
    };
    return {
      name: jd,
      setup(e) {
        (t.console && gs(Fd(e)),
          t.dom && Zu(Pd(e, t.dom)),
          t.xhr && dd(Id(e)),
          t.fetch && Gs(Ld(e)),
          t.history && id(Rd(e)),
          t.sentry && e.on(`beforeSendEvent`, Nd(e)));
      },
    };
  });
function Nd(e) {
  return function (t) {
    F() === e &&
      Go(
        {
          category: `sentry.${t.type === `transaction` ? `transaction` : `event`}`,
          event_id: t.event_id,
          level: t.level,
          message: st(t),
        },
        { event: t }
      );
  };
}
function Pd(e, t) {
  return function (n) {
    if (F() !== e) return;
    let r,
      i,
      a = typeof t == `object` ? t.serializeAttribute : void 0,
      o =
        typeof t == `object` && typeof t.maxStringLength == `number`
          ? t.maxStringLength
          : void 0;
    (o &&
      o > Ad &&
      ($ &&
        v.warn(
          `\`dom.maxStringLength\` cannot exceed ${Ad}, but a value of ${o} was configured. Sentry will use ${Ad} instead.`
        ),
      (o = Ad)),
      typeof a == `string` && (a = [a]));
    try {
      let e = n.event,
        t = zd(e) ? e.target : e;
      ((r = J(t, { keyAttrs: a, maxStringLength: o })), (i = nc(t)));
    } catch {
      r = `<unknown>`;
    }
    if (r.length === 0) return;
    let s = { category: `ui.${n.name}`, message: r };
    (i && (s.data = { "ui.component_name": i }),
      Go(s, { event: n.event, name: n.name, global: n.global }));
  };
}
function Fd(e) {
  return function (t) {
    if (F() !== e) return;
    let n = {
      category: `console`,
      data: { arguments: t.args, logger: `console` },
      level: ys(t.level),
      message: tt(t.args, ` `),
    };
    if (t.level === `assert`)
      if (t.args[0] === !1)
        ((n.message = `Assertion failed: ${tt(t.args.slice(1), ` `) || `console.assert`}`),
          (n.data.arguments = t.args.slice(1)));
      else return;
    Go(n, { input: t.args, level: t.level });
  };
}
function Id(e) {
  return function (t) {
    if (F() !== e) return;
    let { startTimestamp: n, endTimestamp: r } = t,
      i = t.xhr[ud];
    if (!n || !r || !i) return;
    let { method: a, url: o, status_code: s, body: c } = i,
      l = { method: a, url: o, status_code: s },
      u = { xhr: t.xhr, input: c, startTimestamp: n, endTimestamp: r },
      d = { category: `xhr`, data: l, type: `http`, level: zs(s) };
    (e.emit(`beforeOutgoingRequestBreadcrumb`, d, u), Go(d, u));
  };
}
function Ld(e) {
  return function (t) {
    if (F() !== e) return;
    let { startTimestamp: n, endTimestamp: r } = t;
    if (
      r &&
      !(t.fetchData.url.match(/sentry_key/) && t.fetchData.method === `POST`)
    )
      if (t.error) {
        let i = {
            data: t.error,
            input: t.args,
            startTimestamp: n,
            endTimestamp: r,
          },
          a = {
            category: `fetch`,
            data: t.fetchData,
            level: `error`,
            type: `http`,
          };
        (e.emit(`beforeOutgoingRequestBreadcrumb`, a, i), Go(a, i));
      } else {
        let i = t.response,
          a = { ...t.fetchData, status_code: i?.status },
          o = {
            input: t.args,
            response: i,
            startTimestamp: n,
            endTimestamp: r,
          },
          s = {
            category: `fetch`,
            data: a,
            type: `http`,
            level: zs(a.status_code),
          };
        (e.emit(`beforeOutgoingRequestBreadcrumb`, s, o), Go(s, o));
      }
  };
}
function Rd(e) {
  return function (t) {
    if (F() !== e) return;
    let n = t.from,
      r = t.to,
      i = Lo(K.location.href),
      a = n ? Lo(n) : void 0,
      o = Lo(r);
    (a?.path || (a = i),
      i.protocol === o.protocol && i.host === o.host && (r = o.relative),
      i.protocol === a.protocol && i.host === a.host && (n = a.relative),
      Go({ category: `navigation`, data: { from: n, to: r } }));
  };
}
function zd(e) {
  return !!e && !!e.target;
}
var Bd =
    `EventTarget,Window,Node,ApplicationCache,AudioTrackList,BroadcastChannel,ChannelMergerNode,CryptoOperation,EventSource,FileReader,HTMLUnknownElement,IDBDatabase,IDBRequest,IDBTransaction,KeyOperation,MediaController,MessagePort,ModalWindow,Notification,SVGElementInstance,Screen,SharedWorker,TextTrack,TextTrackCue,TextTrackList,WebSocket,WebSocketWorker,Worker,XMLHttpRequest,XMLHttpRequestEventTarget,XMLHttpRequestUpload`.split(
      `,`
    ),
  Vd = `BrowserApiErrors`,
  Hd = W((e = {}) => {
    let t = {
      XMLHttpRequest: !0,
      eventTarget: !0,
      requestAnimationFrame: !0,
      setInterval: !0,
      setTimeout: !0,
      unregisterOriginalCallbacks: !1,
      ...e,
    };
    return {
      name: Vd,
      setupOnce() {
        (t.setTimeout && w(K, `setTimeout`, Ud),
          t.setInterval && w(K, `setInterval`, Ud),
          t.requestAnimationFrame && w(K, `requestAnimationFrame`, Wd),
          t.XMLHttpRequest &&
            `XMLHttpRequest` in K &&
            w(XMLHttpRequest.prototype, `send`, Gd));
        let e = t.eventTarget;
        e && (Array.isArray(e) ? e : Bd).forEach((e) => Kd(e, t));
      },
    };
  });
function Ud(e) {
  return function (...t) {
    let n = t[0];
    return (
      (t[0] = oc(n, {
        mechanism: {
          handled: !1,
          type: `auto.browser.browserapierrors.${y(e)}`,
        },
      })),
      e.apply(this, t)
    );
  };
}
function Wd(e) {
  return function (t) {
    return e.apply(this, [
      oc(t, {
        mechanism: {
          data: { handler: y(e) },
          handled: !1,
          type: `auto.browser.browserapierrors.requestAnimationFrame`,
        },
      }),
    ]);
  };
}
function Gd(e) {
  return function (...t) {
    let n = this;
    return (
      [`onload`, `onerror`, `onprogress`, `onreadystatechange`].forEach((e) => {
        e in n &&
          typeof n[e] == `function` &&
          w(n, e, function (t) {
            let n = {
                mechanism: {
                  data: { handler: y(t) },
                  handled: !1,
                  type: `auto.browser.browserapierrors.xhr.${e}`,
                },
              },
              r = Ne(t);
            return (r && (n.mechanism.data.handler = y(r)), oc(t, n));
          });
      }),
      e.apply(this, t)
    );
  };
}
function Kd(e, t) {
  let n = K[e]?.prototype;
  n?.hasOwnProperty?.(`addEventListener`) &&
    (w(n, `addEventListener`, function (n) {
      return function (r, i, a) {
        try {
          qd(i) &&
            (i.handleEvent = oc(i.handleEvent, {
              mechanism: {
                data: { handler: y(i), target: e },
                handled: !1,
                type: `auto.browser.browserapierrors.handleEvent`,
              },
            }));
        } catch {}
        return (
          t.unregisterOriginalCallbacks && Jd(this, r, i),
          n.apply(this, [
            r,
            oc(i, {
              mechanism: {
                data: { handler: y(i), target: e },
                handled: !1,
                type: `auto.browser.browserapierrors.addEventListener`,
              },
            }),
            a,
          ])
        );
      };
    }),
    w(n, `removeEventListener`, function (e) {
      return function (t, n, r) {
        try {
          if (Object.prototype.hasOwnProperty.call(n, `__sentry_wrapped__`)) {
            let i = n.__sentry_wrapped__;
            i && e.call(this, t, i, r);
          }
        } catch {}
        return e.call(this, t, n, r);
      };
    }));
}
function qd(e) {
  return typeof e.handleEvent == `function`;
}
function Jd(e, t, n) {
  e &&
    typeof e == `object` &&
    `removeEventListener` in e &&
    typeof e.removeEventListener == `function` &&
    e.removeEventListener(t, n);
}
var Yd = W((e = {}) => {
    let t = e.lifecycle ?? `route`;
    return {
      name: `BrowserSession`,
      setupOnce() {
        if (K.document === void 0) {
          $ &&
            v.warn(
              "Using the `browserSessionIntegration` in non-browser environments is not supported."
            );
          return;
        }
        ga({ ignoreDuration: !0 });
        let e = !1;
        ll(() => {
          e ||= (ya(), !0);
        });
        let n = P(),
          r = n.getUser();
        (n.addScopeListener((t) => {
          let n = t.getUser();
          (r?.id !== n?.id || r?.ip_address !== n?.ip_address) &&
            ((r = n), e && ya());
        }),
          t === `route` &&
            id(({ from: t, to: n }) => {
              t !== n && (ga({ ignoreDuration: !0 }), ya(), (e = !0));
            }));
      },
    };
  }),
  Xd = `CultureContext`,
  Zd = W(() => ({
    name: Xd,
    preprocessEvent(e) {
      let t = Qd();
      t &&
        (e.contexts = {
          ...e.contexts,
          culture: { ...t, ...e.contexts?.culture },
        });
    },
    processSegmentSpan(e) {
      let t = Qd();
      t &&
        Hi(e, {
          "culture.locale": t.locale,
          "culture.timezone": t.timezone,
          "culture.calendar": t.calendar,
        });
    },
  }));
function Qd() {
  try {
    let e = K.Intl;
    if (!e) return;
    let t = e.DateTimeFormat().resolvedOptions();
    return { locale: t.locale, timezone: t.timeZone, calendar: t.calendar };
  } catch {
    return;
  }
}
var $d = `GlobalHandlers`,
  ef = W((e = {}) => {
    let t = { onerror: !0, onunhandledrejection: !0, ...e };
    return {
      name: $d,
      setupOnce() {
        Error.stackTraceLimit = 50;
      },
      setup(e) {
        (t.onerror && (tf(e), sf(`onerror`)),
          t.onunhandledrejection && (nf(e), sf(`onunhandledrejection`)));
      },
    };
  });
function tf(e) {
  de((t) => {
    let { stackParser: n, attachStacktrace: r } = cf();
    if (F() !== e || ic()) return;
    let { msg: i, url: a, line: o, column: s, error: c } = t,
      l = of(bc(n, c || i, void 0, r, !1), a, o, s);
    ((l.level = `error`),
      ma(l, {
        originalException: c,
        mechanism: {
          handled: !1,
          type: `auto.browser.global_handlers.onerror`,
        },
      }));
  });
}
function nf(e) {
  me((t) => {
    let { stackParser: n, attachStacktrace: r } = cf();
    if (F() !== e || ic()) return;
    let i = rf(t),
      a = we(i) ? af(i) : bc(n, i, void 0, r, !0);
    ((a.level = `error`),
      ma(a, {
        originalException: i,
        mechanism: {
          handled: !1,
          type: `auto.browser.global_handlers.onunhandledrejection`,
        },
      }));
  });
}
function rf(e) {
  if (we(e)) return e;
  try {
    if (`reason` in e) return e.reason;
    if (`detail` in e && `reason` in e.detail) return e.detail.reason;
  } catch {}
  return e;
}
function af(e) {
  return {
    exception: {
      values: [
        {
          type: `UnhandledRejection`,
          value: `Non-Error promise rejection captured with value: ${String(e)}`,
        },
      ],
    },
  };
}
function of(e, t, n, r) {
  let i = (e.exception = e.exception || {}),
    a = (i.values = i.values || []),
    o = (a[0] = a[0] || {}),
    s = (o.stacktrace = o.stacktrace || {}),
    c = (s.frames = s.frames || []);
  return (
    c.length === 0 &&
      c.push({
        colno: r,
        lineno: n,
        filename: lf(t) ?? tc(),
        function: `?`,
        in_app: !0,
      }),
    e
  );
}
function sf(e) {
  $ && v.log(`Global Handler attached: ${e}`);
}
function cf() {
  return F()?.getOptions() || { stackParser: () => [], attachStacktrace: !1 };
}
function lf(e) {
  if (!(!Se(e) || e.length === 0))
    return e.startsWith(`data:`) ? `<${G(e, !1)}>` : e;
}
var uf = W(() => ({
    name: `HttpContext`,
    preprocessEvent(e) {
      if (!K.navigator && !K.location && !K.document) return;
      let t = sc(),
        n = { ...t.headers, ...e.request?.headers };
      e.request = { ...t, ...e.request, headers: n };
    },
    processSegmentSpan(e) {
      let t = e.attributes?.[I];
      if (!K.navigator && !K.location && !K.document) return;
      let n = sc();
      Hi(e, {
        "url.full": t === `http.client` ? void 0 : n.url,
        "http.request.header.user_agent": n.headers[`User-Agent`],
        "http.request.header.referer": n.headers.Referer,
      });
    },
  })),
  df = `cause`,
  ff = 5,
  pf = `LinkedErrors`,
  mf = W((e = {}) => {
    let t = e.limit || ff,
      n = e.key || df;
    return {
      name: pf,
      preprocessEvent(e, r, i) {
        cs(cc, i.getOptions().stackParser, n, t, e, r);
      },
    };
  }),
  hf = /^HTML(\w*)Element$/;
function gf(e) {
  if (typeof window < `u` && e === window) return `[Window]`;
  if (typeof document < `u` && e === document) return `[Document]`;
  if (hd(e)) {
    let t = _f(e);
    if (hf.test(t)) return `[HTMLElement: ${J(e)}]`;
  }
}
function _f(e) {
  let t = Object.getPrototypeOf(e);
  return t?.constructor ? t.constructor.name : `null prototype`;
}
function vf() {
  return yf()
    ? ($ &&
        l(() => {
          console.error(
            `[Sentry] You cannot use Sentry.init() in a browser extension, see: https://docs.sentry.io/platforms/javascript/best-practices/browser-extensions/`
          );
        }),
      !0)
    : !1;
}
function yf() {
  if (K.window === void 0) return !1;
  let e = K;
  if (e.nw || !(e.chrome || e.browser)?.runtime?.id) return !1;
  let t = tc();
  return !(
    K === K.top &&
    /^(?:chrome-extension|moz-extension|ms-browser-extension|safari-web-extension):\/\//.test(
      t
    )
  );
}
function bf(e) {
  return [Qo(), Jo(), ks(), Hd(), Md(), ef(), mf(), xs(), uf(), Zd(), Yd()];
}
function xf(e = {}) {
  let t = !e.skipBrowserExtensionCheck && vf(),
    n = e.defaultIntegrations == null ? bf() : e.defaultIntegrations,
    r = {
      ...e,
      enabled: !t && e.enabled,
      stackParser: ie(e.stackParser || Od),
      integrations: Da({
        integrations: e.integrations,
        defaultIntegrations: n,
      }),
      transport: e.transport || _d,
    };
  return (Ke(gf), jo(Tc, r));
}
function Sf(e) {
  return e.split(`,`).some((e) => e.trim().startsWith(`sentry-`));
}
function Cf(e) {
  try {
    return new URL(e, K.location.origin).href;
  } catch {
    return;
  }
}
function wf(e) {
  return (
    e.entryType === `resource` &&
    `initiatorType` in e &&
    typeof e.nextHopProtocol == `string` &&
    (e.initiatorType === `fetch` || e.initiatorType === `xmlhttprequest`)
  );
}
function Tf(e) {
  try {
    return new Headers(e);
  } catch {
    return;
  }
}
var Ef = {
  traceFetch: !0,
  traceXHR: !0,
  enableHTTPTimings: !0,
  trackFetchStreamPerformance: !1,
};
function Df(e, t) {
  let {
      traceFetch: n,
      traceXHR: r,
      shouldCreateSpanForRequest: i,
      enableHTTPTimings: a,
      tracePropagationTargets: o,
      onRequestSpanStart: s,
      onRequestSpanEnd: c,
    } = { ...Ef, ...t },
    l = typeof i == `function` ? i : (e) => !0,
    u = (e) => Af(e, o),
    d = {},
    f = e.getOptions().propagateTraceparent;
  (n &&
    Gs((t) => {
      let n = As(t, l, u, d, { propagateTraceparent: f, onRequestSpanEnd: c });
      if (n) {
        let r = Cf(t.fetchData.url),
          i = r ? Lo(r).host : void 0,
          o = r ? G(r) : void 0;
        (n.setAttributes({ "http.url": o, "url.full": o, "server.address": i }),
          a && kf(n, e),
          s?.(n, { headers: t.headers }));
      }
    }),
    r &&
      dd((t) => {
        let n = jf(t, l, u, d, f, c);
        n &&
          (a && kf(n, e),
          s?.(n, { headers: Tf(t.xhr.__sentry_xhr_v3__?.request_headers) }));
      }));
}
var Of = 300;
function kf(e, t) {
  let { url: n } = z(e).data;
  if (!n || typeof n != `string`) return;
  let r = () => void setTimeout(i);
  if (U(t)) {
    let t = e.end.bind(e);
    e.end = (e) => {
      let n = e ?? k(),
        a = !1,
        o = () => {
          a || ((a = !0), setTimeout(i), t(n), clearTimeout(s));
        };
      r = o;
      let s = setTimeout(o, Of);
    };
  }
  let i = kl(`resource`, ({ entries: t }) => {
    t.forEach((t) => {
      wf(t) && t.name.endsWith(n) && (e.setAttributes(ru(t)), r());
    });
  });
}
function Af(e, t) {
  let n = tc();
  if (n) {
    let r, i;
    try {
      ((r = new URL(e, n)), (i = new URL(n).origin));
    } catch {
      return !1;
    }
    let a = r.origin === i;
    return t ? D(r.toString(), t) || (a && D(r.pathname, t)) : a;
  } else {
    let n = !!e.match(/^\/(?!\/)/);
    return t ? D(e, t) : n;
  }
}
function jf(e, t, n, r, i, a) {
  let o = e.xhr,
    s = o?.[ud];
  if (!o || o.__sentry_own_request__ || !s) return;
  let { url: c, method: l } = s,
    u = H() && t(c);
  if (e.endTimestamp) {
    let t = o.__sentry_xhr_span_id__;
    if (!t) return;
    let n = r[t];
    n &&
      (u &&
        s.status_code !== void 0 &&
        (pn(n, s.status_code),
        n.end(),
        a?.(n, { headers: Tf(md(o)), error: e.error })),
      delete r[t]);
    return;
  }
  let d = Cf(c),
    f = Lo(d || c),
    p = d ? G(d) : void 0,
    m = G(Ro(c)),
    h = F(),
    g = !!V(),
    _ = g || (!!h && U(h)),
    v =
      u && _
        ? di({
            name: `${l} ${m}`,
            attributes: {
              url: G(c),
              type: `xhr`,
              "http.method": l,
              "http.url": p,
              "url.full": p,
              "server.address": f?.host,
              [L]: `auto.http.browser`,
              [I]: `http.client`,
              ...(f?.search && { "http.query": f?.search }),
              ...(f?.hash && { "http.fragment": f?.hash }),
            },
          })
        : new Tr(),
    ee = Ci(v) && g ? void 0 : v;
  return (
    u && !_ && h?.recordDroppedEvent(`no_parent_span`, `span`),
    (o.__sentry_xhr_span_id__ = v.spanContext().spanId),
    (r[o.__sentry_xhr_span_id__] = v),
    n(c) && Mf(o, H() && _ ? ee : void 0, i),
    h && h.emit(`beforeOutgoingRequestSpan`, v, e),
    v
  );
}
function Mf(e, t, n) {
  let {
    "sentry-trace": r,
    baggage: i,
    traceparent: a,
  } = Vo({ span: t, propagateTraceparent: n });
  r && Nf(e, r, i, a);
}
function Nf(e, t, n, r) {
  let i = e.__sentry_xhr_v3__?.request_headers;
  if (!(i?.[`sentry-trace`] || !e.setRequestHeader))
    try {
      if (
        (e.setRequestHeader(`sentry-trace`, t),
        r && !i?.traceparent && e.setRequestHeader(`traceparent`, r),
        n)
      ) {
        let t = i?.baggage;
        (!t || !Sf(t)) && e.setRequestHeader(`baggage`, n);
      }
    } catch {}
}
var Pf = new WeakMap(),
  Ff = new WeakMap(),
  If = 9e4,
  Lf = [`text/event-stream`, `application/x-ndjson`, `application/stream+json`],
  Rf = W(() => ({
    name: `FetchStreamPerformance`,
    setup() {
      (Ks((e) => {
        if (e.response) {
          let t = Pf.get(e.response);
          if (t && e.endTimestamp) {
            t.end(e.endTimestamp);
            let n = Ff.get(e.response);
            n && clearTimeout(n);
          }
        }
      }),
        Gs((e) => {
          if (e.endTimestamp && e.response) {
            let t = e.response.headers?.get(`content-type`) || ``;
            if (
              e.response.headers?.get(`content-length`) ||
              !Lf.some((e) => t.startsWith(e))
            )
              return;
            let n = e.fetchData?.url || ``,
              r = e.fetchData?.method || `GET`,
              i = Fo(n),
              a = di({
                name: `${r} ${n.startsWith(`data:`) ? G(n) : i ? Io(i) : n}`,
                startTime: e.endTimestamp,
                attributes: {
                  url: G(n),
                  "http.method": r,
                  type: `fetch`,
                  [I]: `http.client.stream`,
                  [L]: `auto.http.browser.stream`,
                },
              });
            Pf.set(e.response, a);
            let o = setTimeout(() => {
              a.isRecording() && a.end();
            }, If);
            Ff.set(e.response, o);
          }
        }));
    },
  })),
  zf = `WebVitals`,
  Bf = W((e = {}) => {
    let t = new Set(e.ignore ?? []);
    return {
      name: zf,
      setup(n) {
        let r = U(n),
          { enableStandaloneClsSpans: i, enableStandaloneLcpSpans: a } =
            e._experiments ?? {},
          o = r || t.has(`cls`) ? void 0 : i || !1,
          s = r || t.has(`lcp`) ? void 0 : a || !1,
          c = cu({
            recordClsStandaloneSpans: o,
            recordLcpStandaloneSpans: s,
            client: n,
          }),
          l = new WeakSet();
        (n.on(`afterStartPageLoadSpan`, (e) => {
          l.add(e);
        }),
          n.on(`spanEnd`, (e) => {
            l.delete(e) &&
              (c(),
              _u(e, {
                recordClsOnPageloadSpan: o === !1,
                recordLcpOnPageloadSpan: s === !1,
                spanStreamingEnabled: r,
              }));
          }),
          r
            ? (t.has(`lcp`) || Vu(n),
              t.has(`cls`) || Uu(n),
              t.has(`inp`) || Gu())
            : t.has(`inp`) || Pu());
      },
      afterAllSetup() {
        t.has(`inp`) || zu();
      },
    };
  });
function Vf() {
  K.document
    ? K.document.addEventListener(`visibilitychange`, () => {
        let e = V();
        if (!e) return;
        let t = B(e);
        if (K.document.hidden && t) {
          let e = `cancelled`,
            { op: n, status: r } = z(t);
          ($ &&
            v.log(
              `[Tracing] Transaction: ${e} -> since tab moved to the background, op: ${n}`
            ),
            r || t.setStatus({ code: 2, message: e }),
            t.setAttribute(`sentry.cancellation_reason`, `document.hidden`),
            t.end());
        }
      })
    : $ &&
      v.warn(
        `[Tracing] Could not set up background tab detection due to lack of global document`
      );
}
var Hf = `sentry_previous_trace`,
  Uf = `sentry.previous_trace`;
function Wf(e, { linkPreviousTrace: t, consistentTraceSampling: n }) {
  let r = t === `session-storage`,
    i = r ? qf() : void 0;
  e.on(`spanStart`, (e) => {
    if (B(e) !== e) return;
    let t = N().getPropagationContext();
    ((i = Gf(i, e, t)), r && Kf(i));
  });
  let a = !0;
  n &&
    e.on(`beforeSampling`, (e) => {
      if (!i) return;
      let t = N(),
        n = t.getPropagationContext();
      if (a && n.parentSpanId) {
        a = !1;
        return;
      }
      (t.setPropagationContext({
        ...n,
        dsc: {
          ...n.dsc,
          sample_rate: String(i.sampleRate),
          sampled: String(Jf(i.spanContext)),
        },
        sampleRand: i.sampleRand,
      }),
        (e.parentSampled = Jf(i.spanContext)),
        (e.parentSampleRate = i.sampleRate),
        (e.spanAttributes = { ...e.spanAttributes, [tn]: i.sampleRate }));
    });
}
function Gf(e, t, n) {
  let r = z(t);
  function i() {
    try {
      let e = Number(r.data?.[`sentry.sample_rate`] ?? n.dsc?.sample_rate);
      return Number.isNaN(e) ? 0 : e;
    } catch {
      return 0;
    }
  }
  let a = {
    spanContext: t.spanContext(),
    startTimestamp: r.start_timestamp,
    sampleRate: i(),
    sampleRand: n.sampleRand,
  };
  if (!e) return a;
  let o = e.spanContext;
  return o.traceId === r.trace_id
    ? e
    : (Date.now() / 1e3 - e.startTimestamp <= 3600 &&
        ($ &&
          v.log(
            `Adding previous_trace \`${JSON.stringify(o)}\` link to span \`${JSON.stringify({ op: r.op, ...t.spanContext() })}\``
          ),
        t.addLink({ context: o, attributes: { [un]: `previous_trace` } }),
        t.setAttribute(Uf, `${o.traceId}-${o.spanId}-${+!!Jf(o)}`)),
      a);
}
function Kf(e) {
  try {
    K.sessionStorage.setItem(Hf, JSON.stringify(e));
  } catch (e) {
    $ && v.warn(`Could not store previous trace in sessionStorage`, e);
  }
}
function qf() {
  try {
    let e = K.sessionStorage?.getItem(Hf);
    return JSON.parse(e);
  } catch {
    return;
  }
}
function Jf(e) {
  return e.traceFlags === 1;
}
var Yf = `BrowserTracing`,
  Xf =
    /Googlebot|Google-InspectionTool|Storebot-Google|Bingbot|Slurp|DuckDuckBot|Baiduspider|YandexBot|Facebot|facebookexternalhit|LinkedInBot|Twitterbot|Applebot/i;
function Zf() {
  let e = K.navigator;
  return e?.userAgent ? Xf.test(e.userAgent) : !1;
}
var Qf = {
    ...wi,
    instrumentNavigation: !0,
    instrumentPageLoad: !0,
    markBackgroundSpan: !0,
    enableLongTask: !0,
    enableLongAnimationFrame: !0,
    enableInp: !0,
    ignoreResourceSpans: [],
    ignorePerformanceApiSpans: [],
    detectRedirects: !0,
    linkPreviousTrace: `in-memory`,
    consistentTraceSampling: !1,
    enableReportPageLoaded: !1,
    _experiments: {},
    ...Ef,
  },
  $f = (e = {}) => {
    `enableElementTiming` in e &&
      l(() => {
        console.warn(
          "[Sentry] `enableElementTiming` is deprecated and no longer has any effect. Use the standalone `elementTimingIntegration` instead."
        );
      });
    let n = { name: void 0, source: void 0 },
      r = K.document,
      {
        enableInp: i,
        enableLongTask: a,
        enableLongAnimationFrame: o,
        _experiments: {
          enableInteractions: s,
          enableStandaloneClsSpans: c,
          enableStandaloneLcpSpans: u,
        },
        beforeStartSpan: d,
        idleTimeout: f,
        finalTimeout: p,
        childSpanTimeout: m,
        markBackgroundSpan: h,
        traceFetch: g,
        traceXHR: _,
        trackFetchStreamPerformance: ee,
        shouldCreateSpanForRequest: te,
        enableHTTPTimings: ne,
        ignoreResourceSpans: re,
        ignorePerformanceApiSpans: ie,
        instrumentPageLoad: ae,
        instrumentNavigation: oe,
        detectRedirects: se,
        linkPreviousTrace: y,
        consistentTraceSampling: ce,
        enableReportPageLoaded: b,
        onRequestSpanStart: le,
        onRequestSpanEnd: x,
      } = { ...Qf, ...e },
      S = Zf(),
      C,
      ue;
    function de(e, t, i = !0, a) {
      let o = t.op === `pageload`,
        s = t.name,
        c = d ? d(t) : t,
        l = Fo(a || tc()),
        u = {
          ...(l?.pathname && { "url.path": l.pathname }),
          ...(l && !Po(l) && { "url.full": l.href }),
          ...c.attributes,
        };
      if ((s !== c.name && (u[$t] = `custom`), (c.attributes = u), !i)) {
        let e = pt();
        di({ ...c, startTime: e }).end(e);
        return;
      }
      ((n.name = c.name), (n.source = u[$t]));
      let h = ki(c, {
        idleTimeout: f,
        finalTimeout: p,
        childSpanTimeout: m,
        disableAutoFinish: o,
        beforeSpanEnd: (t) => {
          (gu(t, {
            ignoreResourceSpans: re,
            ignorePerformanceApiSpans: ie,
            spanStreamingEnabled: U(e),
          }),
            sp(e, void 0));
          let n = N(),
            r = n.getPropagationContext();
          (n.setPropagationContext({
            ...r,
            traceId: h.spanContext().traceId,
            sampled: or(h),
            dsc: Mr(t),
          }),
            o && (ue = void 0));
        },
        trimIdleSpanEndTimestamp: !b,
      });
      (o && b && (ue = h), sp(e, h));
      function g() {
        r &&
          [`interactive`, `complete`].includes(r.readyState) &&
          (e.emit(`idleSpanEnableAutoFinish`, h),
          r.removeEventListener(`readystatechange`, g));
      }
      o && !b && r && (r.addEventListener(`readystatechange`, g), g());
    }
    return {
      name: Yf,
      setup(e) {
        if (S) {
          $ &&
            v.log(
              `[Tracing] Skipping browserTracingIntegration setup for bot user agent.`
            );
          return;
        }
        if (
          (vr(),
          o &&
          t.PerformanceObserver &&
          PerformanceObserver.supportedEntryTypes?.includes(
            `long-animation-frame`
          )
            ? uu()
            : a && lu(),
          s && du(),
          se && r)
        ) {
          let e = () => {
            C = k();
          };
          (addEventListener(`click`, e, { capture: !0 }),
            addEventListener(`keydown`, e, { capture: !0, passive: !0 }));
        }
        function n() {
          let t = op(e);
          t &&
            !z(t).timestamp &&
            ($ &&
              v.log(
                `[Tracing] Finishing current active span with op: ${z(t).op}`
              ),
            t.setAttribute(nn, `cancelled`),
            t.end());
        }
        (e.on(`startNavigationSpan`, (t, r) => {
          if (F() !== e) return;
          if (r?.isRedirect) {
            ($ &&
              v.warn(
                `[Tracing] Detected redirect, navigation span will not be the root span, but a child span.`
              ),
              de(e, { op: `navigation.redirect`, ...t }, !1, r.url));
            return;
          }
          ((C = void 0),
            n(),
            P().setPropagationContext({
              traceId: j(),
              sampleRand: Math.random(),
              propagationSpanId: H() ? void 0 : M(),
            }));
          let i = N();
          (i.setPropagationContext({
            traceId: j(),
            sampleRand: Math.random(),
            propagationSpanId: H() ? void 0 : M(),
          }),
            i.setSDKProcessingMetadata({ normalizedRequest: void 0 }),
            de(
              e,
              {
                op: `navigation`,
                ...t,
                parentSpan: null,
                forceTransaction: !0,
              },
              !0,
              r?.url
            ));
        }),
          e.on(`startPageLoadSpan`, (t, r = {}) => {
            if (F() !== e) return;
            n();
            let i = Un(
                r.sentryTrace || np(`sentry-trace`) || rp(`sentry-trace`),
                r.baggage || np(`baggage`) || rp(`baggage`)
              ),
              a = N();
            (a.setPropagationContext(i),
              H() || (a.getPropagationContext().propagationSpanId = M()),
              a.setSDKProcessingMetadata({ normalizedRequest: sc() }),
              de(e, { op: `pageload`, ...t }));
          }),
          e.on(`endPageloadSpan`, () => {
            b && ue && (ue.setAttribute(nn, `reportPageLoaded`), ue.end());
          }));
      },
      afterAllSetup(e) {
        if (S) return;
        e.addIntegration &&
          !e.getIntegrationByName?.(`WebVitals`) &&
          e.addIntegration(
            Bf({
              ignore: i ? [] : [`inp`],
              _experiments: {
                enableStandaloneClsSpans: c,
                enableStandaloneLcpSpans: u,
              },
            })
          );
        let t = tc();
        if (
          (y !== `off` &&
            Wf(e, { linkPreviousTrace: y, consistentTraceSampling: ce }),
          K.location)
        ) {
          if (ae) {
            let t = A();
            ep(e, {
              name: K.location.pathname,
              startTime: t ? t / 1e3 : void 0,
              attributes: { [$t]: `url`, [L]: `auto.pageload.browser` },
            });
          }
          oe &&
            id(({ to: n, from: r }) => {
              if (r === void 0 && t?.indexOf(n) !== -1) {
                t = void 0;
                return;
              }
              t = void 0;
              let i = Fo(n),
                a = op(e),
                o = a && se && lp(a, C);
              tp(
                e,
                {
                  name: i?.pathname || K.location.pathname,
                  attributes: { [$t]: `url`, [L]: `auto.navigation.browser` },
                },
                { url: n, isRedirect: o }
              );
            });
        }
        (h && Vf(),
          s && ip(e, f, p, m, n),
          Df(e, {
            traceFetch: g,
            traceXHR: _,
            tracePropagationTargets: e.getOptions().tracePropagationTargets,
            shouldCreateSpanForRequest: te,
            enableHTTPTimings: ne,
            onRequestSpanStart: le,
            onRequestSpanEnd: x,
          }),
          ee && e.addIntegration(Rf()));
      },
    };
  };
function ep(e, t, n) {
  (e.emit(`startPageLoadSpan`, t, n), N().setTransactionName(t.name));
  let r = op(e);
  return (r && e.emit(`afterStartPageLoadSpan`, r), r);
}
function tp(e, t, n) {
  let { url: r, isRedirect: i } = n || {};
  (e.emit(`beforeStartNavigationSpan`, t, { isRedirect: i, url: r }),
    e.emit(`startNavigationSpan`, t, { isRedirect: i, url: r }));
  let a = N();
  return (
    a.setTransactionName(t.name),
    r &&
      !i &&
      a.setSDKProcessingMetadata({ normalizedRequest: { ...sc(), url: r } }),
    op(e)
  );
}
function np(e) {
  return (
    K.document?.querySelector(`meta[name=${e}]`)?.getAttribute(`content`) ||
    void 0
  );
}
function rp(e) {
  return K.performance
    ?.getEntriesByType?.(`navigation`)[0]
    ?.serverTiming?.find((t) => t.name === e)?.description;
}
function ip(e, t, n, r, i) {
  let a = K.document,
    o;
  a &&
    addEventListener(
      `click`,
      () => {
        let a = `ui.action.click`,
          s = op(e);
        if (s) {
          let e = z(s).op;
          if ([`navigation`, `pageload`].includes(e)) {
            $ &&
              v.warn(
                `[Tracing] Did not create ${a} span because a pageload or navigation span is in progress.`
              );
            return;
          }
        }
        if (
          ((o &&=
            (o.setAttribute(nn, `interactionInterrupted`), o.end(), void 0)),
          !i.name)
        ) {
          $ &&
            v.warn(
              `[Tracing] Did not create ${a} transaction because _latestRouteName is missing.`
            );
          return;
        }
        o = ki(
          { name: i.name, op: a, attributes: { [$t]: i.source || `url` } },
          { idleTimeout: t, finalTimeout: n, childSpanTimeout: r }
        );
      },
      { capture: !0 }
    );
}
var ap = `_sentry_idleSpan`;
function op(e) {
  return e[ap];
}
function sp(e, t) {
  T(e, ap, t);
}
var cp = 1.5;
function lp(e, t) {
  let n = z(e),
    r = pt();
  return !(r - n.start_timestamp > cp || (t && r - t <= cp));
}
var up = `jawla-offline`,
  dp = `outbox`,
  fp = 1;
function pp() {
  return new Promise((e, t) => {
    let n = indexedDB.open(up, fp);
    ((n.onupgradeneeded = () => {
      let e = n.result;
      if (!e.objectStoreNames.contains(dp)) {
        let t = e.createObjectStore(dp, { keyPath: `id` });
        (t.createIndex(`status`, `status`),
          t.createIndex(`createdAt`, `createdAt`));
      }
    }),
      (n.onsuccess = () => e(n.result)),
      (n.onerror = () => t(n.error)));
  });
}
function mp() {
  return crypto?.randomUUID
    ? crypto.randomUUID()
    : `xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx`.replace(/[xy]/g, (e) => {
        let t = (Math.random() * 16) | 0;
        return (e === `x` ? t : (t & 3) | 8).toString(16);
      });
}
function hp(e, t, n) {
  return pp().then(
    (e) =>
      new Promise((r, i) => {
        let a = n(e.transaction(dp, t).objectStore(dp));
        ((a.onsuccess = () => r(a.result)), (a.onerror = () => i(a.error)));
      })
  );
}
async function gp(e, t) {
  let n = {
    id: mp(),
    type: e,
    payload: t,
    status: `pending`,
    error: null,
    createdAt: Date.now(),
  };
  return (await hp(dp, `readwrite`, (e) => e.add(n)), n);
}
function _p() {
  return hp(dp, `readonly`, (e) => e.getAll());
}
async function vp() {
  return (await _p()).filter((e) => e.status === `pending`);
}
async function yp() {
  return (await _p()).filter((e) => e.status === `failed`);
}
function bp(e) {
  return hp(dp, `readwrite`, (t) => t.delete(e));
}
async function xp(e, t, n = null) {
  let r = await hp(dp, `readonly`, (t) => t.get(e));
  r &&
    ((r.status = t), (r.error = n), await hp(dp, `readwrite`, (e) => e.put(r)));
}
function Sp(e, t) {
  return xp(e, `failed`, t);
}
function Cp(e) {
  return xp(e, `pending`, null);
}
var wp = !1;
function Tp() {
  let e = document.cookie.match(/(?:^|;\s*)XSRF-TOKEN=([^;]+)/);
  return e
    ? decodeURIComponent(e[1])
    : document
        .querySelector(`meta[name="csrf-token"]`)
        ?.getAttribute(`content`) || ``;
}
async function Ep() {
  let e = await _p(),
    t = e.filter((e) => e.status === `pending`).length,
    n = e.filter((e) => e.status === `failed`).length;
  window.dispatchEvent(
    new CustomEvent(`jawla-sync-status`, { detail: { pending: t, failed: n } })
  );
}
async function Dp() {
  if (wp || !navigator.onLine) return;
  let e = await vp();
  if (!e.length) {
    await Ep();
    return;
  }
  wp = !0;
  try {
    let t = await fetch(`/app/sync`, {
      method: `POST`,
      headers: {
        "Content-Type": `application/json`,
        Accept: `application/json`,
        "X-XSRF-TOKEN": Tp(),
        "X-Requested-With": `XMLHttpRequest`,
      },
      body: JSON.stringify({
        operations: e.map((e) => ({
          key: e.id,
          type: e.type,
          payload: e.payload,
        })),
      }),
    });
    if (!t.ok) return;
    let n = await t.json();
    for (let e of n.results || [])
      e.status === `applied` || e.status === `duplicate`
        ? await bp(e.key)
        : [`failed`, `invalid`, `unsupported`].includes(e.status) &&
          (await Sp(e.key, e.error || e.status));
  } catch {
  } finally {
    ((wp = !1), await Ep());
  }
}
async function Op(e, t) {
  let n = await gp(e, t);
  return (await Ep(), navigator.onLine && Dp(), n);
}
async function kp(e) {
  (await Cp(e), await Ep(), Dp());
}
function Ap() {
  (window.addEventListener(`online`, Dp),
    document.addEventListener(`visibilitychange`, () => {
      document.hidden || Dp();
    }),
    Dp());
}
((window.jawlaSync = {
  enqueue: Op,
  flush: Dp,
  retry: kp,
  discard: (e) => bp(e).then(Ep),
  all: _p,
  pending: vp,
  failed: yp,
}),
  document.readyState === `loading`
    ? document.addEventListener(`DOMContentLoaded`, Ap)
    : Ap());
var jp = document.querySelector(`meta[name="sentry-dsn"]`)?.content;
jp &&
  xf({
    dsn: jp,
    environment:
      document.querySelector(`meta[name="sentry-environment"]`)?.content ||
      `production`,
    tracesSampleRate: 0.1,
    replaysSessionSampleRate: 0,
    replaysOnErrorSampleRate: 0.5,
    integrations: [$f()],
  });
var Mp = `jawla.bluetooth-printer`,
  Np = [
    65504,
    `49535343-fe7d-4ae5-8fa9-9fafd205e455`,
    `0000ae30-0000-1000-8000-00805f9b34fb`,
  ],
  Pp = [
    `0000ffe1-0000-1000-8000-00805f9b34fb`,
    `49535343-8841-43f4-a8d4-ecbe34729bb3`,
    `0000ae01-0000-1000-8000-00805f9b34fb`,
  ];
function Fp(e) {
  return new Promise((t) => setTimeout(t, e));
}
function Ip(e, t = 180) {
  let n = [];
  for (let r = 0; r < e.length; r += t) n.push(e.slice(r, r + t));
  return n;
}
window.jawlaAutocomplete = ({ options: e, noResultsText: t }) => ({
  open: !1,
  search: ``,
  highlighted: -1,
  selected: ``,
  options: e,
  noResultsText: t,
  init() {
    ((this.selected = this.$refs.hidden?.value
      ? String(this.$refs.hidden.value)
      : ``),
      (this.search = this.labelFor(this.selected)));
  },
  get filtered() {
    if (!this.search) return this.options;
    let e = this.search.toLowerCase();
    return this.options.filter((t) => t.label.toLowerCase().includes(e));
  },
  labelFor(e) {
    let t = this.options.find((t) => String(t.value) === String(e));
    return t ? t.label : ``;
  },
  syncHidden() {
    this.$refs.hidden &&
      ((this.$refs.hidden.value = this.selected),
      this.$refs.hidden.dispatchEvent(new Event(`input`, { bubbles: !0 })),
      this.$refs.hidden.dispatchEvent(new Event(`change`, { bubbles: !0 })));
  },
  choose(e) {
    ((this.selected = String(e.value)),
      (this.search = e.label),
      this.syncHidden(),
      (this.open = !1),
      (this.highlighted = -1));
  },
  clear() {
    ((this.selected = ``),
      (this.search = ``),
      this.syncHidden(),
      (this.open = !1),
      (this.highlighted = -1));
  },
  closeList() {
    ((this.open = !1),
      (this.highlighted = -1),
      (this.search = this.labelFor(this.selected)));
  },
  move(e) {
    if (!this.open) {
      this.open = !0;
      return;
    }
    let t = this.filtered.length;
    t !== 0 && (this.highlighted = (this.highlighted + e + t) % t);
  },
  enter() {
    this.open &&
      this.highlighted >= 0 &&
      this.filtered[this.highlighted] &&
      this.choose(this.filtered[this.highlighted]);
  },
});
function Lp(e) {
  return Array.from(new TextEncoder().encode(e));
}
function Rp(e) {
  let t = Lp(e),
    n = t.length + 3;
  return [
    29,
    40,
    107,
    4,
    0,
    49,
    65,
    50,
    0,
    29,
    40,
    107,
    3,
    0,
    49,
    67,
    6,
    29,
    40,
    107,
    3,
    0,
    49,
    69,
    48,
    27,
    97,
    1,
    29,
    40,
    107,
    n % 256,
    Math.floor(n / 256),
    49,
    80,
    48,
    ...t,
    29,
    40,
    107,
    3,
    0,
    49,
    81,
    48,
    27,
    97,
    0,
  ];
}
function zp(e) {
  let t = [27, 64];
  for (let n of e.lines || []) t.push(...Lp(`${n}\n`));
  return (
    e.qr &&
      (t.push(
        ...Lp(`
`)
      ),
      t.push(...Rp(e.qr)),
      t.push(
        ...Lp(`
`)
      )),
    t.push(29, 86, 66, 0),
    t.push(
      ...Lp(`


`)
    ),
    new Uint8Array(t)
  );
}
async function Bp(e) {
  let t = [];
  for (let n of Np)
    try {
      t.push(await e.getPrimaryService(n));
    } catch {}
  let n = t.length > 0 ? t : await e.getPrimaryServices();
  for (let e of n) {
    for (let t of Pp)
      try {
        let n = await e.getCharacteristic(t);
        if (n.properties.write || n.properties.writeWithoutResponse) return n;
      } catch {}
    let t = await e.getCharacteristics();
    for (let e of t)
      if (e.properties.write || e.properties.writeWithoutResponse) return e;
  }
  throw Error(`No writable printer characteristic found.`);
}
async function Vp() {
  let e = window.localStorage.getItem(Mp);
  if (!e || !navigator.bluetooth?.getDevices) return null;
  let t = null;
  try {
    ({ id: t } = JSON.parse(e));
  } catch {
    return (window.localStorage.removeItem(Mp), null);
  }
  return (
    (await navigator.bluetooth.getDevices()).find((e) => e.id === t) || null
  );
}
((window.jawlaBluetoothPrinter = ({ payload: e }) => ({
  payload: e,
  busy: !1,
  supported: !!navigator.bluetooth,
  hasSavedPrinter: !!window.localStorage.getItem(Mp),
  message: navigator.bluetooth
    ? document.documentElement.lang === `ar`
      ? `اطبع مباشرة أو قم بإقران الطابعة أولاً.`
      : `Print now or pair a printer first.`
    : document.documentElement.lang === `ar`
      ? `هذا المتصفح لا يدعم Bluetooth للطباعة. استخدم ملف PDF.`
      : `This browser does not support Bluetooth printing. Use the PDF instead.`,
  async pairPrinter() {
    if (this.supported) {
      ((this.busy = !0),
        (this.message =
          document.documentElement.lang === `ar`
            ? `جاري إقران الطابعة…`
            : `Pairing printer…`));
      try {
        let e = await navigator.bluetooth.requestDevice({
          acceptAllDevices: !0,
          optionalServices: Np,
        });
        (window.localStorage.setItem(
          Mp,
          JSON.stringify({ id: e.id, name: e.name || `Bluetooth printer` })
        ),
          (this.hasSavedPrinter = !0),
          (this.message =
            document.documentElement.lang === `ar`
              ? `تم حفظ الطابعة: ${e.name || `Bluetooth printer`}`
              : `Saved printer: ${e.name || `Bluetooth printer`}`));
      } catch (e) {
        this.message =
          e?.message ||
          (document.documentElement.lang === `ar`
            ? `تعذر إقران الطابعة.`
            : `Could not pair the printer.`);
      } finally {
        this.busy = !1;
      }
    }
  },
  forgetPrinter() {
    (window.localStorage.removeItem(Mp),
      (this.hasSavedPrinter = !1),
      (this.message =
        document.documentElement.lang === `ar`
          ? `تم مسح الطابعة المحفوظة.`
          : `Saved printer cleared.`));
  },
  async printDocument() {
    if (this.supported) {
      if (!this.payload?.lines?.length) {
        this.message =
          document.documentElement.lang === `ar`
            ? `لا توجد بيانات للطباعة.`
            : `Nothing to print.`;
        return;
      }
      ((this.busy = !0),
        (this.message =
          document.documentElement.lang === `ar`
            ? `جاري تجهيز الطباعة…`
            : `Preparing print…`));
      try {
        let e = await Vp();
        if (((e ||= (await this.pairPrinter(), await Vp())), !e))
          throw Error(
            document.documentElement.lang === `ar`
              ? `اختر طابعة أولاً.`
              : `Choose a printer first.`
          );
        let t = await Bp(e.gatt?.connected ? e.gatt : await e.gatt.connect()),
          n = zp(this.payload);
        for (let e of Ip(n))
          (typeof t.writeValueWithoutResponse == `function`
            ? await t.writeValueWithoutResponse(e)
            : await t.writeValue(e),
            await Fp(30));
        this.message =
          document.documentElement.lang === `ar`
            ? `تم إرسال الطباعة إلى الطابعة.`
            : `Print sent to the printer.`;
      } catch (e) {
        this.message =
          e?.message ||
          (document.documentElement.lang === `ar`
            ? `فشلت الطباعة.`
            : `Printing failed.`);
      } finally {
        this.busy = !1;
      }
    }
  },
})),
  (window.jawlaAutocompleteRegistry = window.jawlaAutocompleteRegistry || {}),
  (window.jawlaAutocompleteInit = function (e) {
    let t = window.jawlaAutocompleteRegistry[e] || [],
      n = window.jawlaAutocomplete({ options: t, noResultsText: `No results` }),
      r = document.getElementById(e);
    r && r.__x && Object.assign(r.__x.$data, n);
  }),
  (window.jawlaAutocompleteSync = function (e, t) {
    let n = document.getElementById(e);
    n &&
      n.__x &&
      ((n.__x.$data.selected = t),
      (n.__x.$data.search = n.__x.$data.labelFor(t)),
      n.__x.$data.syncHidden());
  }),
  (window.jawlaAutocompleteFinalize = function (e) {
    let t = document.getElementById(e);
    t && t.__x && t.__x.$data.closeList();
  }));
