...
| -------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| P0       | SEC-001: Session cookies may not be secure on staging | Client sessions could be intercepted over HTTPS if `SESSION_SECURE_COOKIE` isn't set | `config/session.php:172` defaults to secure only when `APP_ENV=production`      | FIX-001: Set `SESSION_SECURE_COOKIE=true` on Railway staging |
| P1       | SEC-002: CORS origin list may block staging API calls | PWA fetch requests from the staging domain could be rejected                         | `config/cors.php:6-9` — fixed to use env vars but `APP_STAGING_URL` must be set | FIX-002: Set `APP_STAGING_URL` env var on Railway            |
| P1       | DEP-001: No verified backup/restore process           | Data loss risk if staging DB is corrupted or reset                                   | No backup scripts or restore documentation found                                | FIX-003: Document and test Railway DB backup/restore         |
